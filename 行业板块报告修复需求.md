...
>
> 当前决策：**完整四维模式仍执行原 P0 门槛；`SOLO_OWNER_RESEARCH_V1` 的领导力必须真实计算，仅资金保持 `UNKNOWN`；完整排名置空，项目所有者精确签署后发布 `RESEARCH_ONLY/WAIT` 报告。**

## 〇、V1.2 一人项目治理覆盖决策

项目所有者于 2026-09-05 书面确认 `SOLO_OWNER_RESEARCH_V1`，用于 2026-09-04 首次恢复研究发布。本节及第 10、14、16、17、19 节的一人治理条款在该模式下优先于旧企业级完整四维条款；旧条款继续保留，适用于未来显式启用的完整四维模式。

- 唯一生产输入是 2026-09-04 `READY_FOR_REPORT` 行业逐日任务，行业价格事实达到 90/90 且逐项回读为 `VALID`。
- 资金与领导力没有可信生产事实时必须为 `UNKNOWN`，不得填零、插值、沿用旧链路数据或参与评分。
- `complete_score`、`complete_rank` 必须为空；只允许展示明确标注为“部分研究排名”的结果。
- 模型状态固定为 `RESEARCH_ONLY`，交易状态固定为 `WAIT`，不构成投资建议或交易指令。
- 发布前由项目所有者对不可变 `owner_approval_request.json` 的精确 SHA-256 作带外确认，再生成 signoff、恢复清单和正式 release。
- 10 个交易日双跑、20 个交易日独立性观察和五行业人工抽查降为 P2 跟踪项，不阻断本模式首次发布，也不得被写成已经完成。

本次审批请求 SHA-256 为 `90b9332668fe64aa7b4440a4577bfadbc8894afe80372b9cf8010b5726772e8a`；正式 release ID 为 `86143c84b8a4b925984b2bc3ced5dbcb40bc13e42fd73832930474e69a81326e`，root SHA-256 为 `9179db98969c7ad511e0a4916352e5ae5faa669ecd278f406cc35e8f83a90dd8`。发布物见[2026-09-04 行业板块研究报告](A股板块潜力与龙头图形股研究报告_20260904.html)与[2026-08-26 至 2026-09-01 数据勘误](A股板块报告数据勘误_20260826_20260901.html)。
...
#### 10.1.1 `SOLO_OWNER_RESEARCH_V1` schema 10

不可变 release 目录必须精确包含以下 8 个工件，根目录同名工件与 release 目录逐字节一致；根目录另有指向该 release 的 `PUBLISHED.json`：

```text
A股板块潜力与龙头图形股研究报告_20260904.html
A股板块报告数据勘误_20260826_20260901.html
report_payload.json
partial_ranking.json
five_industry_recalculation.json
owner_approval_request.json
owner_signoff.json
solo_recovery_manifest.json
```

`owner_approval_request.json` 的精确 SHA-256 必须经项目所有者带外确认；signoff、恢复清单、release ID、root hash 及全部输入身份必须可由发布验证器回读。

#### 10.1.2 旧企业级完整四维 release
...
### 14.1 `SOLO_OWNER_RESEARCH_V1` 恢复门槛

首次研究发布必须同时满足：

- [x] 报告日期为 2026-09-04，引用唯一且已达到 `READY_FOR_REPORT` 的 `job_run_id`，不存在临时联网取数、`latest` 或最近成功日回退。
- [x] 当前分类版本行业集合达到 90/90，价格与相对强度质量逐项为 `VALID`，并能从已归档输入复算。
- [x] 资金与领导力逐项为 `UNKNOWN`，相关值为空、参与评分数为 0，且 release 中不存在旧资金/领导力 artifact。
- [x] 生成前后重新固定行情 STORE、readiness、交易日历、行业任务、公式注册表、形态报告及输入哈希，防止检查后替换。
- [x] 模型状态为 `RESEARCH_ONLY`、交易状态为 `WAIT`；资金与领导力不参与评分。
- [x] `complete_score` 与 `complete_rank` 全部为空，仅展示带 `partial_research_rank` 标签的部分研究排名。
- [x] 2026-08-26 至 2026-09-01 资金历史缺口按“证据不可得”发布勘误，不插值、不伪造修正值。
- [x] 项目所有者已对精确 `owner_approval_request.json` SHA-256 作带外确认，signoff 与恢复清单可独立验证。
- [x] release 清单、root hash、release ID、报告字节及发布目标均可回读。
...
- 预期行业代码集合与价格有效事实集合不一致；或缺失资金/领导力未在一人治理模式中逐项标为 `UNKNOWN` 并排除出评分。
- 报告可以在 `daily_ingest_status != READY_FOR_REPORT` 时启动，或可回退读取前一交易日/最近成功快照。
- 资金 N 日值仍直接来自无业务日期的供应商周期快照。
- 任一实际参与评分的核心维度不可从已归档输入复算。
- 历史回填可以读到报告截止日之后的数据。
- 子集分位仍伪装成全行业分位。
- 已启用领导力频道时，龙头事件账本与前后快照不一致；未启用时却未明确标为 `UNKNOWN`。
- 未知/无效仍可被转为 `PASS/FAIL` 或数值0。
- 数据审计失败时仍能将 `publish_status` 标为 `PASS` 或 `OWNER_CONFIRMED_RESEARCH_RELEASE`。
- 修正版无法定位原版、输入版本或生成代码。
...
### 16.1 `SOLO_OWNER_RESEARCH_V1`

本项目在一人治理模式下由项目所有者承担数据、方法、开发、测试、报告与发布责任。有效签署不是角色名列表，而是：项目所有者对精确审批请求 SHA-256 的带外确认、不可变 `owner_signoff.json`、完整审批图、恢复清单、release ID/root hash 以及发布后远端字节回读。代理不得自行新增角色、审计周期或发布门槛。
...
项目所有者于 2026-09-05 书面确认 D-01 至 D-08 采用下表结论。首次一人治理 release 对尚无可信生产事实的资金与领导力一律执行 `UNKNOWN`；以后变更必须有新的所有者书面记录，不得由开发根据当前输出反推。
...
| D-04 | 价格与相对强度是否各计1票 | 可共同进入部分研究得分，但高相关性必须作为 P2 警告披露；独立性观察完成前不得宣称为两个独立完整投票 |
...
### 19.1 `SOLO_OWNER_RESEARCH_V1` 首次恢复发布

本模式只有在第 14.1 节全部条件满足、正式 release 独立验证通过、三份授权发布路径与远端 blob 逐字节回读一致后，才能宣布 2026-09-04 首次研究发布完成。10 个交易日双跑、20 个交易日独立性观察和五行业人工抽查保留为 P2 后续项；它们不阻断本次发布，也不得在完成报告中被误写为已完成。

### 19.2 旧企业级完整四维恢复

完整四维修复只有在以下条件全部满足后才能宣布完成：
