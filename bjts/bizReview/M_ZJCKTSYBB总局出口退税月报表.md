# M_ZJCKTSYBB 总局出口退税月报表——系统功能分析整理

> 文档编号：`FUN-M_ZJCKTSYBB`
> 当前版本：V0.1（静态资料还原稿）
> 整理日期：2026-09-03
...
1. **[事实]** `M_ZJCKTSYBB` 是“日常管理 → 报表管理 → 总局出口退税月报表”路径下的有效叶子菜单，静态菜单组件为空，登录后由报表菜单接口动态绑定通用组件 `bblist`。需求资料说明：包括出口退（免）税情况月报表（主表）、暂缓退（免）税统计月报表、不予办理退（免）税统计月报表、未申报出口退（免）税情况统计表、企业不欲申报出口退税情况统计月报表、出口退税返还情况月报表、涉嫌骗税移送稽查情况统计表、专设机构设置及专职人员配备年度统计表、出口退（免）税情况分析月报表（本月情况）、出口退（免）税情况分析月报表（累计情况）
...
| 项目 | 内容 | 证据状态 |
|---|---|---|
| 功能编号 | `FUN-M_ZJCKTSYBB` | 本文建立 |
| 菜单代码 | `M_ZJCKTSYBB` | [事实] |
| 功能名称 | 总局出口退税月报表 | [事实] |
| 菜单路径 | `M_ROOT → M_RCGL → M_HZBB → M_ZJCKTSYBB` | [事实]，菜单结构表 |
...
| 规则编号 | 规则 | 来源 | 改造要求/待确认 |
|---|---|---|---|
| `RULE-ZJCKTSYBB-001` | 授权菜单包含 `M_ZJCKTSYBB` 时才应显示并允许打开该功能 | 菜单表、`app.js` | 服务端接口必须独立鉴权，不能只依赖前端隐藏 |
| `RULE-ZJCKTSYBB-002` | 页面提供“【报表年月】：”等输入或筛选项 | 页面模板 | 后端需实施类型、长度、必填、日期区间及数据范围校验 |
| `RULE-ZJCKTSYBB-003` | 页面以当前登录用户税务机关初始化组织范围 | 前端源码 | 服务端须根据会话重算允许组织集合，拒绝越权机关代码 |
| `RULE-ZJCKTSYBB-004` | 页面可见操作包括“查询、导出套表▼ 原始数据、上报数据” | 页面模板 | 逐项确认角色、状态前置条件、并发控制和审计要求 |
| `RULE-ZJCKTSYBB-005` | 列表查询使用 `pageNo/pageSize` 或 jqGrid 分页事件 | 前端源码 | 统一页码基数、最大页大小、总数和空页返回约定 |
| `RULE-ZJCKTSYBB-006` | 页面会把列名与排序方向写入 `orderSql` 或排序参数 | 前端源码 | 服务端必须使用字段白名单映射，禁止直接拼接 SQL |
| `RULE-ZJCKTSYBB-007` | 组件初始化过程中会触发首次查询 | 前端源码 | 确认默认条件、默认数据范围及首屏性能基线 |
| `RULE-ZJCKTSYBB-008` | 包括出口退（免）税情况月报表（主表）、暂缓退（免）税统计月报表、不予办理退（免）税统计月报表、未申报出口退（免）税情况统计表、企业不欲申报出口退税情况统计月报表、出口退税返还情况月报表、涉嫌骗税移送稽查情况统计表、专设机构设置及专职人员配备年度统计表、出口退（免）税情况分析月报表（本月情况）、出口退（免）税情况分析月报表（累计情况） | 功能清单 | 作为业务验收口径；歧义项由业务部门确认 |
| `RULE-ZJCKTSYBB-009` | 功能包含“导出套表▼ 原始数据”能力 | 页面与接口源码 | 限制文件类型、大小/行数，防公式注入和敏感数据泄露并记录审计 |
| `RULE-ZJCKTSYBB-010` | 前端以字符串业务码 `"0"` 判定成功，失败时展示 `res.msg` | 前端源码 | 统一 HTTP 状态、业务码、错误信息和请求关联号 |
...
| 资产 | 关键位置/用途 |
|---|---|
| [`app.js`](../src/client/src/app.js) | 156 行菜单注册、页面组件/外部地址及授权过滤 |
...
|---|---|---|---|---|
| `REQ-ZJCKTSYBB-001` | 授权用户可从菜单路径打开功能 | `M_ZJCKTSYBB`、`bblist` | 菜单/角色配置 | `TC-01` 菜单可见性与接口越权校验 |
| `REQ-ZJCKTSYBB-002` | 查询、导出套表▼ 原始数据、上报数据 | `/bjtssw/tjbb/recevMain` | `TL_ADMIN.TJBB_TASK、TL_ADMIN.TJBB_TASK_SUB` | `TC-02` 正常、空结果及边界条件 |
| `REQ-ZJCKTSYBB-003` | 查询条件、列表字段和业务口径一致 | 主组件及直接接口 | `TL_ADMIN.TJBB_TASK、TL_ADMIN.TJBB_TASK_SUB` | `TC-03` 字段映射、分页排序及数据范围 |
| `REQ-ZJCKTSYBB-004` | 文件输入输出与页面数据、权限一致 | `/bjtssw/tjbb/saveSuitExcel` | 同列表/业务对象 | `TC-04` 文件格式、上限、脱敏和审计 |
| `REQ-ZJCKTSYBB-005` | 角色与税务机关数据范围受控 | `/auth/getmenu`、全部业务接口 | 权限对象待补证 | `TC-05` 横向/纵向越权与敏感字段校验 |
