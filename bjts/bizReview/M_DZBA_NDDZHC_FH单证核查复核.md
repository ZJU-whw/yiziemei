# M_DZBA_NDDZHC_FH 单证核查复核——系统功能分析整理

> 文档编号：`FUN-M_DZBA_NDDZHC_FH`
> 当前版本：V0.1（静态资料还原稿）
> 整理日期：2026-09-03
...
1. **[事实]** `M_DZBA_NDDZHC_FH` 是“日常管理 → 备案单证管理 → 年度单证核查 → 单证核查复核”路径下的有效叶子菜单，前端页面组件为 `dzhcDzfh`。需求资料说明：单证核查复核的操作和审核基本类同，复核环节支持批量通过，核查项目通过后则进入项目发放环节，回退则返回到审核环节。
...
| 项目 | 内容 | 证据状态 |
|---|---|---|
| 功能编号 | `FUN-M_DZBA_NDDZHC_FH` | 本文建立 |
| 菜单代码 | `M_DZBA_NDDZHC_FH` | [事实] |
| 功能名称 | 单证核查复核 | [事实] |
| 菜单路径 | `M_ROOT → M_RCGL → M_DZBA → M_DZBA_NDDZHC → M_DZBA_NDDZHC_FH` | [事实]，菜单结构表 |
| 页面入口 | 前端页面组件为 `dzhcDzfh` | [事实]，`app.js` |
| 功能类型 | 查询、流程处理 | [事实]，页面源码与需求资料 |
| 需求来源/年份 | 2021年杭州市税务局需求。 | [事实/待确认] |
| 原关系表服务标识 | `tl-bjts-swgl-dzba` | [事实]，功能—数据库关系表 |
| 授权角色 | `DZBA`、`DZBAFH`、`DZBAGL` | [事实]，菜单—角色关系表 |
...
| 规则编号 | 规则 | 来源 | 改造要求/待确认 |
|---|---|---|---|
| `RULE-DZBA_NDDZHC_FH-001` | 授权菜单包含 `M_DZBA_NDDZHC_FH` 时才应显示并允许打开该功能 | 菜单表、`app.js` | 服务端接口必须独立鉴权，不能只依赖前端隐藏 |
| `RULE-DZBA_NDDZHC_FH-002` | 页面提供“税务机关、纳税人识别号 企业名称 海关代码、项目状态、企业类型、管理等级、年度、企业规模、备案方式”等输入或筛选项 | 页面模板 | 后端需实施类型、长度、必填、日期区间及数据范围校验 |
| `RULE-DZBA_NDDZHC_FH-003` | 页面以当前登录用户税务机关初始化组织范围 | 前端源码 | 服务端须根据会话重算允许组织集合，拒绝越权机关代码 |
| `RULE-DZBA_NDDZHC_FH-004` | 页面可见操作包括“单证核查复核、查询、复核完成、退回、单证核查-出口业务、基础信息、批量退回、批量审核、核查完成、确定” | 页面模板 | 逐项确认角色、状态前置条件、并发控制和审计要求 |
| `RULE-DZBA_NDDZHC_FH-005` | 列表查询使用 `pageNo/pageSize` 或 jqGrid 分页事件 | 前端源码 | 统一页码基数、最大页大小、总数和空页返回约定 |
| `RULE-DZBA_NDDZHC_FH-006` | 页面会把列名与排序方向写入 `orderSql` 或排序参数 | 前端源码 | 服务端必须使用字段白名单映射，禁止直接拼接 SQL |
| `RULE-DZBA_NDDZHC_FH-007` | 组件初始化过程中会触发首次查询 | 前端源码 | 确认默认条件、默认数据范围及首屏性能基线 |
| `RULE-DZBA_NDDZHC_FH-008` | 单证核查复核的操作和审核基本类同，复核环节支持批量通过，核查项目通过后则进入项目发放环节，回退则返回到审核环节。 | 功能清单 | 作为业务验收口径；歧义项由业务部门确认 |
| `RULE-DZBA_NDDZHC_FH-009` | 功能包含“文件导入/导出”能力 | 页面与接口源码 | 限制文件类型、大小/行数，防公式注入和敏感数据泄露并记录审计 |
| `RULE-DZBA_NDDZHC_FH-010` | 前端以字符串业务码 `"0"` 判定成功，失败时展示 `res.msg` | 前端源码 | 统一 HTTP 状态、业务码、错误信息和请求关联号 |
...
| `/dzba/inspect/year/review/list` | POST | 年度单证核查复核 - 查询 | **当前服务工程未检出匹配路由** | 校验角色与数据范围，并保证幂等、事务一致性和操作审计 |
| `/dzba/inspect/year/project/baseinfo` | POST | 核查项目明细 - 基础信息 | **当前服务工程未检出匹配路由** | 校验参数、分页/排序白名单、行级权限、超时和空结果契约 |
| `/dzba/inspect/year/review/inspect/batch` | POST | 年度单证核查复核 - 复核完成 - 批量 | **当前服务工程未检出匹配路由** | 校验角色与数据范围，并保证幂等、事务一致性和操作审计 |
| `/dzba/inspect/year/review/inspect/single` | POST | 年度单证核查复核 - 复核完成 - 单笔 | **当前服务工程未检出匹配路由** | 校验角色与数据范围，并保证幂等、事务一致性和操作审计 |
| `/dzba/inspect/year/review/back/batch` | POST | 年度单证核查复核 - 退回 - 批量 | **当前服务工程未检出匹配路由** | 校验角色与数据范围，并保证幂等、事务一致性和操作审计 |
| `/dzba/inspect/year/review/back/single` | POST | 年度单证核查复核 - 退回 - 单笔 | **当前服务工程未检出匹配路由** | 校验角色与数据范围，并保证幂等、事务一致性和操作审计 |
...
| 资产 | 关键位置/用途 |
|---|---|
| [`app.js`](../src/client/src/app.js) | 86 行菜单注册、页面组件/外部地址及授权过滤 |
| [`dzhcDzfh.js`](<../src/client/src/page/单证备案/单证核查/dzhcDzfh.js>) | `dzhcDzfh` 主组件、查询状态、表格与操作逻辑 |
| [`dzhcDzfh.html`](<../src/client/src/page/单证备案/单证核查/dzhcDzfh.html>) | 页面输入、按钮、列表及弹窗结构 |
...
|---|---|---|---|---|
| `REQ-DZBA_NDDZHC_FH-001` | 授权用户可从菜单路径打开功能 | `M_DZBA_NDDZHC_FH`、`dzhcDzfh` | 菜单/角色配置 | `TC-01` 菜单可见性与接口越权校验 |
| `REQ-DZBA_NDDZHC_FH-002` | 单证核查复核、查询、复核完成、退回、单证核查-出口业务 | `/dzba/inspect/year/project/inspector/list` | `TL_ADMIN.EDOC_INSPECT_PROJECT` | `TC-02` 正常、空结果及边界条件 |
| `REQ-DZBA_NDDZHC_FH-003` | 查询条件、列表字段和业务口径一致 | 主组件及直接接口 | `TL_ADMIN.EDOC_INSPECT_PROJECT` | `TC-03` 字段映射、分页排序及数据范围 |
| `REQ-DZBA_NDDZHC_FH-004` | 文件输入输出与页面数据、权限一致 | `/dzba/inspect/year/project/file/view` | 同列表/业务对象 | `TC-04` 文件格式、上限、脱敏和审计 |
| `REQ-DZBA_NDDZHC_FH-005` | 角色与税务机关数据范围受控 | `/auth/getmenu`、全部业务接口 | 权限对象待补证 | `TC-05` 横向/纵向越权与敏感字段校验 |
