# M_JKM_PFGZ_SZ 健康码评分规则设置——系统功能分析整理

> 文档编号：`FUN-M_JKM_PFGZ_SZ`
> 当前版本：V0.1（静态资料还原稿）
> 整理日期：2026-09-03
...
1. **[事实]** `M_JKM_PFGZ_SZ` 是“风险管理 → 风险管理 → 指标及参数设置 → 健康码评分规则设置”路径下的有效叶子菜单，前端页面组件为 `jkmpfgz`。需求资料说明：健康码的评分规则，由业务分类、指标赋分、等级线设定等功能构成。 业务分类：主要划分为信用类、出口类、退税类、发票类、财务类和外部信息类六个维度。 指标赋分：分为两级管理，一级是全省统一的指标赋分规则，二级是以地市为单位对指标赋分的个性化调整。
...
| 项目 | 内容 | 证据状态 |
|---|---|---|
| 功能编号 | `FUN-M_JKM_PFGZ_SZ` | 本文建立 |
| 菜单代码 | `M_JKM_PFGZ_SZ` | [事实] |
| 功能名称 | 健康码评分规则设置 | [事实] |
| 菜单路径 | `M_ROOT → M_FXGL → M_SSZJ → M_ZDYSZ → M_JKM_PFGZ_SZ` | [事实]，菜单结构表 |
| 页面入口 | 前端页面组件为 `jkmpfgz` | [事实]，`app.js` |
...
| 规则编号 | 规则 | 来源 | 改造要求/待确认 |
|---|---|---|---|
| `RULE-JKM_PFGZ_SZ-001` | 授权菜单包含 `M_JKM_PFGZ_SZ` 时才应显示并允许打开该功能 | 菜单表、`app.js` | 服务端接口必须独立鉴权，不能只依赖前端隐藏 |
| `RULE-JKM_PFGZ_SZ-002` | 页面提供“税务机关”等输入或筛选项 | 页面模板 | 后端需实施类型、长度、必填、日期区间及数据范围校验 |
| `RULE-JKM_PFGZ_SZ-003` | 页面以当前登录用户税务机关初始化组织范围 | 前端源码 | 服务端须根据会话重算允许组织集合，拒绝越权机关代码 |
| `RULE-JKM_PFGZ_SZ-004` | 页面可见操作包括“保存” | 页面模板 | 逐项确认角色、状态前置条件、并发控制和审计要求 |
| `RULE-JKM_PFGZ_SZ-005` | 组件初始化过程中会触发首次查询 | 前端源码 | 确认默认条件、默认数据范围及首屏性能基线 |
| `RULE-JKM_PFGZ_SZ-006` | 业务分类：主要划分为信用类、出口类、退税类、发票类、财务类和外部信息类六个维度。 | 功能清单 | 作为业务验收口径；歧义项由业务部门确认 |
| `RULE-JKM_PFGZ_SZ-007` | 指标赋分：分为两级管理，一级是全省统一的指标赋分规则，二级是以地市为单位对指标赋分的个性化调整。 | 功能清单 | 作为业务验收口径；歧义项由业务部门确认 |
| `RULE-JKM_PFGZ_SZ-008` | 前端以字符串业务码 `"0"` 判定成功，失败时展示 `res.msg` | 前端源码 | 统一 HTTP 状态、业务码、错误信息和请求关联号 |
...
|---|---|---|---|---|
| `/sszj/jkmpd/pzList` | POST | search | **当前服务工程未检出匹配路由** | 校验参数、分页/排序白名单、行级权限、超时和空结果契约 |
| `/sszj/jkmpd/pzSave` | POST | 页面调用 | **当前服务工程未检出匹配路由** | 校验参数、分页/排序白名单、行级权限、超时和空结果契约 |
...
| 资产 | 关键位置/用途 |
|---|---|
| [`app.js`](../src/client/src/app.js) | 418 行菜单注册、页面组件/外部地址及授权过滤 |
| [`jkmpfgz.js`](<../src/client/src/page/三三智检/税务机关自定义/jkmpfgz.js>) | `jkmpfgz` 主组件、查询状态、表格与操作逻辑 |
| [`jkmpfgz.html`](<../src/client/src/page/三三智检/税务机关自定义/jkmpfgz.html>) | 页面输入、按钮、列表及弹窗结构 |
...
|---|---|---|---|---|
| `REQ-JKM_PFGZ_SZ-001` | 授权用户可从菜单路径打开功能 | `M_JKM_PFGZ_SZ`、`jkmpfgz` | 菜单/角色配置 | `TC-01` 菜单可见性与接口越权校验 |
| `REQ-JKM_PFGZ_SZ-002` | 保存 | `/sszj/jkmpd/pzList` | `TL_TSSH.JKGL_PZ_JKM、TL_TSSH.JKGL_PZ_JKM_SWJG、TL_TSSH.JKGL_PZ_ZB、TL_TSSH.JKGL_PZ_ZB_YCFF` | `TC-02` 正常、空结果及边界条件 |
| `REQ-JKM_PFGZ_SZ-003` | 查询条件、列表字段和业务口径一致 | 主组件及直接接口 | `TL_TSSH.JKGL_PZ_JKM、TL_TSSH.JKGL_PZ_JKM_SWJG、TL_TSSH.JKGL_PZ_ZB、TL_TSSH.JKGL_PZ_ZB_YCFF` | `TC-03` 字段映射、分页排序及数据范围 |
| `REQ-JKM_PFGZ_SZ-004` | 角色与税务机关数据范围受控 | `/auth/getmenu`、全部业务接口 | 权限对象待补证 | `TC-04` 横向/纵向越权与敏感字段校验 |
