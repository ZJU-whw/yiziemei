# M_SCQYHYFB 出口企业行业分布情况统计——系统功能分析整理
...
1. **[事实]** `M_SCQYHYFB` 是“日常管理 → 统计分析 → 出口企业行业分布情况统计”路径下的有效叶子菜单，前端页面组件为 `ckqyhyfb`。需求资料说明：该功能统计出口生产企业的行业、出口金额和退税金额，行业分类精确到主行业-子行业，以金三行业归类为准。便于掌握不同行业的出口和退税状况。 需要统计的数据包括行业、出口额、退税额、占比、上年同期金额、同比。
...
| 项目 | 内容 | 证据状态 |
|---|---|---|
| 功能编号 | `FUN-M_SCQYHYFB` | 本文建立 |
| 菜单代码 | `M_SCQYHYFB` | [事实] |
| 功能名称 | 出口企业行业分布情况统计 | [事实] |
| 菜单路径 | `M_ROOT → M_RCGL → M_TJFX → M_SCQYHYFB` | [事实]，菜单结构表 |
| 页面入口 | 前端页面组件为 `ckqyhyfb` | [事实]，`app.js` |
...
| 规则编号 | 规则 | 来源 | 改造要求/待确认 |
|---|---|---|---|
| `RULE-SCQYHYFB-001` | 授权菜单包含 `M_SCQYHYFB` 时才应显示并允许打开该功能 | 菜单表、`app.js` | 服务端接口必须独立鉴权，不能只依赖前端隐藏 |
| `RULE-SCQYHYFB-002` | 页面提供“退税机关、企业类型、生产企业、外贸企业、外综服企业、出口时间起、企业代码、出口贸易国”等输入或筛选项 | 页面模板 | 后端需实施类型、长度、必填、日期区间及数据范围校验 |
| `RULE-SCQYHYFB-003` | 页面以当前登录用户税务机关初始化组织范围 | 前端源码 | 服务端须根据会话重算允许组织集合，拒绝越权机关代码 |
| `RULE-SCQYHYFB-004` | 页面可见操作包括“查询、导出、统计图、统计表” | 页面模板 | 逐项确认角色、状态前置条件、并发控制和审计要求 |
| `RULE-SCQYHYFB-005` | 列表查询使用 `pageNo/pageSize` 或 jqGrid 分页事件 | 前端源码 | 统一页码基数、最大页大小、总数和空页返回约定 |
| `RULE-SCQYHYFB-006` | 页面会把列名与排序方向写入 `orderSql` 或排序参数 | 前端源码 | 服务端必须使用字段白名单映射，禁止直接拼接 SQL |
| `RULE-SCQYHYFB-007` | 组件初始化过程中会触发首次查询 | 前端源码 | 确认默认条件、默认数据范围及首屏性能基线 |
| `RULE-SCQYHYFB-008` | 需要统计的数据包括行业、出口额、退税额、占比、上年同期金额、同比。 | 功能清单 | 作为业务验收口径；歧义项由业务部门确认 |
| `RULE-SCQYHYFB-009` | 功能包含“导出”能力 | 页面与接口源码 | 限制文件类型、大小/行数，防公式注入和敏感数据泄露并记录审计 |
| `RULE-SCQYHYFB-010` | 前端以字符串业务码 `"0"` 判定成功，失败时展示 `res.msg` | 前端源码 | 统一 HTTP 状态、业务码、错误信息和请求关联号 |
...
| `TJBB_DT_D01007` | `TL_ADMIN` | 功能业务数据 | R | 功能—数据库关系表/页面源码 |
...
- **[事实]** `TL_ADMIN.TJBB_DT_D01007` 在本仓库 Oracle 对象脚本中定义为表，共 17 个字段，有主键，检出 0 个索引（其中 0 个唯一索引）。
...
flowchart LR
    ...
    B[业务对象<br/>TL_ADMIN.TJBB_DT_D01007<br/>TL_TSSH.CKTS_SB_MDT_CKMX] --> C
    C --> D[页面组件<br/>ckqyhyfb]
    D --> E[统计展示、查询、文件输出结果]
```

**[事实]** 功能—数据库关系表登记的基础对象为 `TL_TSSH.GLXT_BB_SHXT_DJXX`，业务对象为 `TL_ADMIN.TJBB_DT_D01007+TL_TSSH.CKTS_SB_MDT_CKMX+TL_TSSH.CKTS_SB_MTS_CKMX+TL_TSSH.CKTS_SB_WZF_CKMX`。
...
| 资产 | 关键位置/用途 |
|---|---|
| [`app.js`](../src/client/src/app.js) | 177 行菜单注册、页面组件/外部地址及授权过滤 |
| [`ckqyhyfb.js`](<../src/client/src/page/统计分析/ckqyhyfb.js>) | `ckqyhyfb` 主组件、查询状态、表格与操作逻辑 |
| [`ckqyhyfb.html`](<../src/client/src/page/统计分析/ckqyhyfb.html>) | 页面输入、按钮、列表及弹窗结构 |
...
|---|---|---|---|---|
| `REQ-SCQYHYFB-001` | 授权用户可从菜单路径打开功能 | `M_SCQYHYFB`、`ckqyhyfb` | 菜单/角色配置 | `TC-01` 菜单可见性与接口越权校验 |
| `REQ-SCQYHYFB-002` | 查询、导出、统计图、统计表 | `/bjtssw/tjbb/profile/header` | `TL_TSSH.GLXT_BB_SHXT_DJXX、TL_ADMIN.TJBB_DT_D01007、TL_TSSH.CKTS_SB_MDT_CKMX、TL_TSSH.CKTS_SB_MTS_CKMX` | `TC-02` 正常、空结果及边界条件 |
| `REQ-SCQYHYFB-003` | 查询条件、列表字段和业务口径一致 | 主组件及直接接口 | `TL_TSSH.GLXT_BB_SHXT_DJXX、TL_ADMIN.TJBB_DT_D01007、TL_TSSH.CKTS_SB_MDT_CKMX、TL_TSSH.CKTS_SB_MTS_CKMX` | `TC-03` 字段映射、分页排序及数据范围 |
| `REQ-SCQYHYFB-004` | 文件输入输出与页面数据、权限一致 | `/bjtssw/tjfx/loaddata/export` | 同列表/业务对象 | `TC-04` 文件格式、上限、脱敏和审计 |
| `REQ-SCQYHYFB-005` | 角色与税务机关数据范围受控 | `/auth/getmenu`、全部业务接口 | 权限对象待补证 | `TC-05` 横向/纵向越权与敏感字段校验 |
