create or replace function func_shzs_rwwp_bak_old2025(p_swjg_dm varchar2,p_gwdm varchar2,p_nsrsbh varchar2,p_lcswsxdm varchar2)
          ...
          select SFDM, SWRYMC into v_WTDXSFDM,v_WTDXMC
            FROM (select RY.SFDM, RY.swrymc, count(distinct(TK.nsrsbh)) as CNT
                    ...
                    LEFT JOIN SHZS_WP_TASK TK
                     ...
                     AND TO_CHAR(TK.Wpsj, 'yyyymmdd') = TO_CHAR(sysdate, 'yyyymmdd')
...
--DBMS_OUTPUT.put_line('随机分单=找到在岗接单人');
  ...
  o_wp.extend;
  o_wp(1) := TYPE_REC_SHZS_RWWP(v_WTDXSFDM,v_WTDXMC,p_nsrsbh,v_QYFZMC,v_JDMODE,v_ErrMsg);

--DBMS_OUTPUT.put_line('结束--'||v_ErrMsg||'-'||v_WTDXSFDM||v_WTDXMC);

  return o_wp;
end func_shzs_rwwp_bak_old2025;
/
