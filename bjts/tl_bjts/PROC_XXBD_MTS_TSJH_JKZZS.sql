CREATE OR REPLACE PROCEDURE PROC_XXBD_MTS_TSJH_JKZZS
/*
  编制人:严国平
  编制日期:202009
  功能:信息比对（外贸企业免退税）
  ...
  BEGIN
    INSERT INTO CKTS_XXBD_SHQ_YDXX(DJXH,SBYWB_DM,SSSQ,SBPC,SBID,ERR_LY,XH,ERR_OBJ,GLYWB1,GLYWB2,GLYWB3,GLYWZ1,GLYWZ2,YDCODE,ERR_LEV,ERR_MSG,PASS_FLAG,CRTIME)
         SELECT V_IN_DJXH,V_IN_SBYWBDM,V_IN_SSSQ,V_IN_SBPC,V_IN_SBID,'1',SEQ_XXBD_SHQ_YDID.NEXTVAL,LC_YDOBJECT,
                'CKTS_SB_MTS_TSJH_LSB',NULL,NULL,T.SBXH,T.GLH,'MTS_JHMX_JKZZS_WDZXX','E','海关进口增值税缴款书[' || T.JHPZH || ']无电子信息！','N',SYSDATE
           FROM CKTS_SB_MTS_TSJH_LSB T
          WHERE T.SBID=V_IN_SBID
            AND T.CKTMSPZLX_DM='04'
  ...
  BEGIN
    FOR LSF_XX IN (SELECT A.SBXH,A.GLH,
                    ...
                    WHERE A.SBID=V_IN_SBID
                      AND A.CKTMSPZLX_DM='04') LOOP
      BEGIN
        IF LSF_XX.BYTSBZ='Y' THEN
          INSERT INTO CKTS_XXBD_SHQ_YDXX(DJXH,SBYWB_DM,SSSQ,SBPC,SBID,ERR_LY,XH,ERR_OBJ,GLYWB1,GLYWB2,GLYWB3,GLYWZ1,GLYWZ2,YDCODE,ERR_LEV,ERR_MSG,PASS_FLAG,CRTIME)
               SELECT V_IN_DJXH,V_IN_SBYWBDM,V_IN_SSSQ,V_IN_SBPC,V_IN_SBID,'1',SEQ_XXBD_SHQ_YDID.NEXTVAL,LC_YDOBJECT,
                      'CKTS_SB_MTS_TSJH_LSB',NULL,NULL,LSF_XX.SBXH,LSF_XX.GLH,'MTS_JHMX_JKZZS_BYTS','E','海关进口增值税缴款书[' || LSF_XX.JHPZH || ']已被标注为“不予退税”！','N',SYSDATE
                 FROM DUAL;
          COMMIT;
        END IF;
        ...
        IF LSF_XX.HGJKJKSCLJG_DM IN ('001','200') THEN
          INSERT INTO CKTS_XXBD_SHQ_YDXX(DJXH,SBYWB_DM,SSSQ,SBPC,SBID,ERR_LY,XH,ERR_OBJ,GLYWB1,GLYWB2,GLYWB3,GLYWZ1,GLYWZ2,YDCODE,ERR_LEV,ERR_MSG,PASS_FLAG,CRTIME)
               SELECT V_IN_DJXH,V_IN_SBYWBDM,V_IN_SSSQ,V_IN_SBPC,V_IN_SBID,'1',SEQ_XXBD_SHQ_YDID.NEXTVAL,LC_YDOBJECT,
                      'CKTS_SB_MTS_TSJH_LSB',NULL,NULL,LSF_XX.SBXH,LSF_XX.GLH,'MTS_JHMX_JKZZS_YDK','E','海关进口增值税缴款书[' || LSF_XX.JHPZH || ']在电子信息中已勾选用于抵扣！','N',SYSDATE
                 FROM DUAL;
          COMMIT;
        END IF;
        ...
        IF LSF_XX.HGJKJKSCLJG_DM IN ('100','500') THEN
          INSERT INTO CKTS_XXBD_SHQ_YDXX(DJXH,SBYWB_DM,SSSQ,SBPC,SBID,ERR_LY,XH,ERR_OBJ,GLYWB1,GLYWB2,GLYWB3,GLYWZ1,GLYWZ2,YDCODE,ERR_LEV,ERR_MSG,PASS_FLAG,CRTIME)
               SELECT V_IN_DJXH,V_IN_SBYWBDM,V_IN_SSSQ,V_IN_SBPC,V_IN_SBID,'1',SEQ_XXBD_SHQ_YDID.NEXTVAL,LC_YDOBJECT,
                      'CKTS_SB_MTS_TSJH_LSB',NULL,NULL,LSF_XX.SBXH,LSF_XX.GLH,'MTS_JHMX_JKZZS_DUP','E','海关进口增值税缴款书[' || LSF_XX.JHPZH || ']在电子信息中重复申请退税！','Y',SYSDATE
                 FROM DUAL;
          COMMIT;
        END IF;
        ...
        IF LSF_XX.GHFNSRSBH_1<>LSF_XX.NSRSBH THEN
          INSERT INTO CKTS_XXBD_SHQ_YDXX(DJXH,SBYWB_DM,SSSQ,SBPC,SBID,ERR_LY,XH,ERR_OBJ,GLYWB1,GLYWB2,GLYWB3,GLYWZ1,GLYWZ2,YDCODE,ERR_LEV,ERR_MSG,PASS_FLAG,CRTIME)
               SELECT V_IN_DJXH,V_IN_SBYWBDM,V_IN_SSSQ,V_IN_SBPC,V_IN_SBID,'1',SEQ_XXBD_SHQ_YDID.NEXTVAL,LC_YDOBJECT,
                      'CKTS_SB_MTS_TSJH_LSB',NULL,NULL,LSF_XX.SBXH,LSF_XX.GLH,'MTS_JHMX_JKZZS_GHFNSR','E','海关进口增值税缴款书[' || LSF_XX.JHPZH || ']供货方纳税人[' || LSF_XX.GHFNSRSBH_1 || ']与电子信息中[' || LSF_XX.NSRSBH || ']不一致！','Y',SYSDATE
                 FROM DUAL;
          COMMIT;
        END IF;
      END;
