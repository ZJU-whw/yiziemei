CREATE OR REPLACE PROCEDURE PROC_XXBD_WZF
/*
  编制人:严国平
  编制日期:202009
  功能:信息比对（外综服代办退税）
...
BEGIN
  ...
  IF LC_WMZHFWQYBZ='N' THEN
    BEGIN
      V_OUT_STATUS :='26';
      V_OUT_MESSAGE:='出口企业未备案为外贸综合服务企业，不允许申报外贸综合服务企业代办退税！';
      RETURN;
    END;
  END IF;
  ...
  BEGIN
    PROC_XXBD_WZF_TSSB(V_IN_NSRDZDAH,V_IN_DJXH,V_IN_SBYWBDM,V_IN_SSSQ,V_IN_SBPC,V_IN_SBID,V_OUT_STATUS,V_OUT_MESSAGE);
  EXCEPTION
    WHEN OTHERS THEN
      V_OUT_STATUS :='51';
      V_OUT_MESSAGE:='代办退税明细自检出错：'||SQLCODE||' - '||SQLERRM;
      RETURN;
  END;

  BEGIN
    PROC_XXBD_WZF_TSSB_BGD(V_IN_NSRDZDAH,V_IN_DJXH,V_IN_SBYWBDM,V_IN_SSSQ,V_IN_SBPC,V_IN_SBID,V_OUT_STATUS,V_OUT_MESSAGE);
  EXCEPTION
    WHEN OTHERS THEN
      V_OUT_STATUS :='52';
  ...
  BEGIN
    PROC_XXBD_WZF_TSSB_ZZSFP(V_IN_NSRDZDAH,V_IN_DJXH,V_IN_SBYWBDM,V_IN_SSSQ,V_IN_SBPC,V_IN_SBID,V_OUT_STATUS,V_OUT_MESSAGE);
  EXCEPTION
    WHEN OTHERS THEN
      V_OUT_STATUS :='53';
      V_OUT_MESSAGE:='代办退税发票自检出错：'||SQLCODE||' - '||SQLERRM;
      RETURN;
  END;

  BEGIN
    PROC_XXBD_WZF_CKSH(V_IN_NSRDZDAH,V_IN_DJXH,V_IN_SBYWBDM,V_IN_SSSQ,V_IN_SBPC,V_IN_SBID,V_OUT_STATUS,V_OUT_MESSAGE);
  EXCEPTION
    WHEN OTHERS THEN
      V_OUT_STATUS :='62';
  ...
  BEGIN
    PROC_XXBD_WZF_HGSPTZ(V_IN_NSRDZDAH,V_IN_DJXH,V_IN_SBYWBDM,V_IN_SSSQ,V_IN_SBPC,V_IN_SBID,V_OUT_STATUS,V_OUT_MESSAGE);
  EXCEPTION
    WHEN OTHERS THEN
      V_OUT_STATUS :='63';
