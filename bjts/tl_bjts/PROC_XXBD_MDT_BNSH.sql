CREATE OR REPLACE PROCEDURE PROC_XXBD_MDT_BNSH
/*
  编制人:严国平
  编制日期:202009
  功能:信息比对（生产企业免抵退税）
  ...
  BEGIN
    INSERT INTO CKTS_XXBD_SHQ_YDXX(DJXH,SBYWB_DM,SSSQ,SBPC,SBID,ERR_LY,XH,ERR_OBJ,GLYWB1,GLYWB2,GLYWB3,GLYWZ1,GLYWZ2,YDCODE,ERR_LEV,ERR_MSG,PASS_FLAG,CRTIME)
         SELECT V_IN_DJXH,V_IN_SBYWBDM,V_IN_SSSQ,V_IN_SBPC,V_IN_SBID,'1',SEQ_XXBD_SHQ_YDID.NEXTVAL,LC_YDOBJECT,
                'CKTS_SB_MDT_BNSH_LSB','CKTS_SB_MDT_TSSB_LSB',NULL,T.SBXH,NULL,'MDT_BJGX_SHZL_CKHW_CKPZ','E','出口货物报关单号[' || T.CKBGDH || ']未申报免退税出口明细！','N',SYSDATE
           FROM CKTS_SB_MTS_BNSH_LSB T
          WHERE T.SBID=V_IN_SBID
            AND T.CKBGDH IS NOT NULL
            AND NOT EXISTS (SELECT 1 FROM CKTS_SB_MDT_TSSB_LSB S WHERE S.SBID=T.SBID AND NVL(S.CKBGDH,S.DLCKHWZMHM)=T.CKBGDH);
    COMMIT;
  END;
END;
/
