create or replace function Func_Get_Qxswjg(swjg_dm varchar2) return varchar2 is
  v_qxSwjg varchar2(11);
  i integer;
begin
    ...
    begin
      ...
      while i> 1 loop
        ...
        if substr(swjg_dm,(i*2),2)<>'00' then
          begin
            v_qxSwjg:=substr(swjg_dm,1,(i*2)+1);
            exit;
          end;
        end if;
        i:=i-1;
      end loop;
      if (i=1) then v_qxSwjg := substr(swjg_dm,1,3); end if;
