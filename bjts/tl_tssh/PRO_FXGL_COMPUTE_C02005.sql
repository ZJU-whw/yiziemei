CREATE OR REPLACE PROCEDURE PRO_FXGL_COMPUTE_C02005
/*
 * 小规模期间供货开13%票
  ...
  LOOP
    BEGIN
      INSERT INTO FXGL_DATA_ZXZB(ID,TSSWJG_DM,DJXH,NSRSBH,NSRMC,SMLX,SMRQ,ZBID,ZBCS,SMJG)
           SELECT SEQ_FXGL_DATA_ZXZB.NEXTVAL,DJ.SWJGDM,DJ.DJXH_JS,NVL(DJ.SHXYNO,DJ.NSRDJNO),DJ.NSRMC,NVL(P_SMLX,'0'),SYSDATE,'C02005',
             ...
             FROM (SELECT DJXH,SSQPC,GHFNSRSBH_1,DJRQ,YBNSRRDRQ,ZSSL,MIN(CKRQ_1) AS MIN_CKRQ,MAX(CKRQ_1) AS MAX_CKRQ,SUM(TSE) AS TSE
                     FROM (SELECT DJXH,SSQ||SBPC AS SSQPC,GHFNSRSBH_1,CKRQ_1,ZSSL,TSE,
                                  ...
                                  (SELECT MIN(GH.YBNSRRDRQ)
                                    ...
                                    WHERE GH.NSRSBH=SB.GHFNSRSBH_1
                                      AND GH.NSRZT_DM NOT IN ('09','10')
                                      AND NVL(GH.ZXRQ,SYSDATE)>SB.KPRQ) AS YBNSRRDRQ
