...
routine_body: BEGIN
  DECLARE o_wp TYPE_TB_SHZS_RWWP DEFAULT TYPE_TB_SHZS_RWWP();
    ...
    select IFNULL(dj.zs_swjg_dm, 'NONE'),dj.tsjsfs_dm,dk.kzxx
      ...
      left join gs_dj_cktmsdab_kz dk
        on (dj.nsrdzdah = dk.nsrdzdah and dk.kzlx = 'FLGLCD' and dk.flag = '1' and
           CURRENT_TIMESTAMP between dk.st_date and dk.end_date and rownum = 1)
...
--DBMS_OUTPUT.put_line('随机分单=找到在岗接单人');
    ...
    --  2）判断接单人是否在岗和在线。
          with RY as
           (select t.sfdm, t.swrymc
             ...
             inner join sys_cfg_czry_fpgl s
               ...
               and (s.flgl_set is null or s.flgl_set like '%' || v_FLGLCD ||'%')
               and (s.jsmode_set is null or s.jsmode_set like '%' || v_JSMODE ||'%')
               and (IFNULL(s.zsjg_dm_set, 'NOFZ') like '%' || v_QYFZDM ||'%')
             where t.swjgdm = p_swjg_dm
               and t.gwdm = p_gwdm
               and t.status = '1')
          select SFDM, SWRYMC into v_WTDXSFDM,v_WTDXMC
        ...
        BEGIN
          SET v_WTDXSFDM ='';
          SET v_WTDXMC ='';
          SET v_ErrMsg ='税务机关'||p_swjg_dm||'未设置接单方式！';
        END;
        END IF;
      END IF;
