...
routine_body: BEGIN
  ...
  BEGIN
    ...
    SELECT COUNT(1)
     ...
     WHERE SBID=V_IN_SBID
       ...
       AND (((STR_TO_DATE(DATE_FORMAT(CURRENT_TIMESTAMP, '%Y')||'0420', '%Y%m%d')<DATE(CURRENT_TIMESTAMP)) AND (KPRQ<TRUNCATE(CURRENT_TIMESTAMP, 'YY'))) OR (KPRQ<DATE_ADD(TRUNCATE(CURRENT_TIMESTAMP, 'YY'), INTERVAL -12 MONTH)));
  EXCEPTION
    WHEN OTHERS THEN
      SET V_OUT_STATUS ='06';
  ...
  EXCEPTION
    WHEN OTHERS THEN
      SET V_OUT_STATUS ='51';
      SET V_OUT_MESSAGE ='出口非自产货物退消费税明细自检出错：'||SQLCODE||' - '||SQLERRM;
      LEAVE routine_body;
  END;
  ...
  EXCEPTION
    WHEN OTHERS THEN
      SET V_OUT_STATUS ='54';
      SET V_OUT_MESSAGE ='海关进口消费税缴款书自检出错：'||SQLCODE||' - '||SQLERRM;
      LEAVE routine_body;
  END;
