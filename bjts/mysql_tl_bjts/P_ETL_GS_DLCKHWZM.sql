...
routine_body: BEGIN
  ...
  --注销纳税人不导
  DECLARE pc_nsr CURSOR FOR SELECT DISTINCT WT_CPCODE FROM TB_CKTS_HISTORY_ZJDLZM WHERE TBPC = 6 and WT_CPCODE IS NOT NULL;
  ...
  DBMS_OUTPUT.put_line('同步代理出口货物证明表同步开始 ['||CAST(pv_cnt AS CHAR)||'] 户');
  IF pv_cnt = 0 THEN
     LEAVE routine_body;
  END IF;
  ...
  LOOP
    ...
    SET pv_qyhgdm = rs_nsr.wt_cpcode;
    ...
    BEGIN
      ...
      COMMIT;
      DBMS_OUTPUT.put_line(pv_qyhgdm || '同步成功' || CAST(pv_cnt AS CHAR) || '条数据');
