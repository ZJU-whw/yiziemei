...
routine_body: BEGIN
      ...
      BEGIN
        INSERT INTO CKTS_XXBD_SHQ_YDXX(DJXH,SBYWB_DM,SSSQ,SBPC,SBID,ERR_LY,XH,ERR_OBJ,GLYWB1,GLYWB2,GLYWB3,GLYWZ1,GLYWZ2,YDCODE,ERR_LEV,ERR_MSG,PASS_FLAG,CRTIME)
             SELECT V_IN_DJXH,V_IN_SBYWBDM,V_IN_SSSQ,V_IN_SBPC,V_IN_SBID,'1',SEQ_NEXTVAL('SEQ_XXBD_SHQ_YDID'),LC_YDOBJECT,
                    'CKTS_SB_FZC_SBMX_LSB','CKTS_SB_FZC_CKSH_LSB',NULL,T.SBXH,NULL,'XFS_BJGX_CKMX_SHZL_CKPZ','E','出口货物报关单号/代理出口货物证明号[' || T.CKBGDH || T.DLCKHWZMHM || ']未申报收汇资料！','N',CURRENT_TIMESTAMP
               FROM CKTS_SB_FZC_SBMX_LSB T
              WHERE T.SBID=V_IN_SBID
      ...
      BEGIN
        INSERT INTO CKTS_XXBD_SHQ_YDXX(DJXH,SBYWB_DM,SSSQ,SBPC,SBID,ERR_LY,XH,ERR_OBJ,GLYWB1,GLYWB2,GLYWB3,GLYWZ1,GLYWZ2,YDCODE,ERR_LEV,ERR_MSG,PASS_FLAG,CRTIME)
             SELECT V_IN_DJXH,V_IN_SBYWBDM,V_IN_SSSQ,V_IN_SBPC,V_IN_SBID,'1',SEQ_NEXTVAL('SEQ_XXBD_SHQ_YDID'),LC_YDOBJECT,
               ...
               FROM CKTS_SB_FZC_SBMX_LSB T
              WHERE T.SBID=V_IN_SBID
                AND (((STR_TO_DATE(DATE_FORMAT(CURRENT_TIMESTAMP, '%Y')||'0419', '%Y%m%d')<CURRENT_TIMESTAMP) AND (KPRQ<TRUNCATE(CURRENT_TIMESTAMP, 'YY'))) OR (KPRQ<DATE_ADD(TRUNCATE(CURRENT_TIMESTAMP, 'YY'), INTERVAL -12 MONTH)))
                AND FUNC_XXBD_CHECK_NEEDSH(',' || T.CKTMSYWLXDMJH || ',') = 1
                AND NOT EXISTS (SELECT 1 FROM CKTS_SB_FZC_CKSH_LSB S WHERE S.SBID=T.SBID AND S.CKBGDH||S.DLCKHWZMHM=T.CKBGDH||T.DLCKHWZMHM);
        COMMIT;
      END;
  ...
  BEGIN
    INSERT INTO CKTS_XXBD_SHQ_YDXX(DJXH,SBYWB_DM,SSSQ,SBPC,SBID,ERR_LY,XH,ERR_OBJ,GLYWB1,GLYWB2,GLYWB3,GLYWZ1,GLYWZ2,YDCODE,ERR_LEV,ERR_MSG,PASS_FLAG,CRTIME)
         SELECT V_IN_DJXH,V_IN_SBYWBDM,V_IN_SSSQ,V_IN_SBPC,V_IN_SBID,'1',SEQ_NEXTVAL('SEQ_XXBD_SHQ_YDID'),LC_YDOBJECT,
                'CKTS_SB_FZC_SBMX_LSB',NULL,NULL,T.SBXH,NULL,'XFS_CKMX_CKYWLX_QYGTS','E','出口货物报关单号[' || IFNULL(T.CKBGDH, T.DLCKHWZMHM) || ']为启运港业务，需确认国内启运方式及海关总署认证企业类型！','N',CURRENT_TIMESTAMP
           FROM CKTS_SB_FZC_SBMX_LSB T
          WHERE T.SBID=V_IN_SBID
