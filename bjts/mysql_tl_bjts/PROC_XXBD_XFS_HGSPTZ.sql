  ...
  BEGIN
    INSERT INTO CKTS_XXBD_SHQ_YDXX(DJXH,SBYWB_DM,SSSQ,SBPC,SBID,ERR_LY,XH,ERR_OBJ,GLYWB1,GLYWB2,GLYWB3,GLYWZ1,GLYWZ2,YDCODE,ERR_LEV,ERR_MSG,PASS_FLAG,CRTIME)
         SELECT V_IN_DJXH,V_IN_SBYWBDM,V_IN_SSSQ,V_IN_SBPC,V_IN_SBID,'1',SEQ_NEXTVAL('SEQ_XXBD_SHQ_YDID'),LC_YDOBJECT,
                'CKTS_SB_FZC_HGSPTZ_LSB','CKTS_SB_FZC_SBMX_LSB',NULL,T.SBXH,NULL,'XFS_BJGX_DMTZ_CKMX_CKPZ','E','出口货物报关单号/代理出口货物证明号[' || IFNULL(T.DLCKHWZMHM, T.CKBGDH) || ']未申报退消费税明细！','N',CURRENT_TIMESTAMP
           FROM CKTS_SB_FZC_HGSPTZ_LSB T
          WHERE T.SBID=V_IN_SBID
    ...
    INSERT INTO CKTS_XXBD_SHQ_YDXX(DJXH,SBYWB_DM,SSSQ,SBPC,SBID,ERR_LY,XH,ERR_OBJ,GLYWB1,GLYWB2,GLYWB3,GLYWZ1,GLYWZ2,YDCODE,ERR_LEV,ERR_MSG,PASS_FLAG,CRTIME)
         SELECT V_IN_DJXH,V_IN_SBYWBDM,V_IN_SSSQ,V_IN_SBPC,V_IN_SBID,'1',SEQ_NEXTVAL('SEQ_XXBD_SHQ_YDID'),LC_YDOBJECT,
                'CKTS_SB_FZC_HGSPTZ_LSB','CKTS_SB_FZC_SBMX_LSB',NULL,T.SBXH,NULL,'XFS_BJGX_DMTZ_CKMX_SPDM','E','出口货物报关单号/代理出口货物证明号[' || IFNULL(T.DLCKHWZMHM, T.CKBGDH) || ']调整前商品代码与退消费税明细表中不一致！','N',CURRENT_TIMESTAMP
           FROM CKTS_SB_FZC_HGSPTZ_LSB T
    ...
    INSERT INTO CKTS_XXBD_SHQ_YDXX(DJXH,SBYWB_DM,SSSQ,SBPC,SBID,ERR_LY,XH,ERR_OBJ,GLYWB1,GLYWB2,GLYWB3,GLYWZ1,GLYWZ2,YDCODE,ERR_LEV,ERR_MSG,PASS_FLAG,CRTIME)
         SELECT V_IN_DJXH,V_IN_SBYWBDM,V_IN_SSSQ,V_IN_SBPC,V_IN_SBID,'1',SEQ_NEXTVAL('SEQ_XXBD_SHQ_YDID'),LC_YDOBJECT,
                'CKTS_SB_FZC_HGSPTZ_LSB',NULL,NULL,T.SBXH,NULL,'XFS_BJGX_DMTZ_CKMX_ZSLV','E','出口货物报关单号[' || IFNULL(T.DLCKHWZMHM, T.CKBGDH) || ']调整后商品代码在对应出口日期的退税率文库中的消费税税率与退消费税明细表中不一致！','N',CURRENT_TIMESTAMP
           FROM CKTS_SB_FZC_HGSPTZ_LSB T
          INNER JOIN CKTS_SB_FZC_SBMX_LSB S ON S.SBID=T.SBID AND IFNULL(S.DLCKHWZMHM, S.CKBGDH)=IFNULL(T.DLCKHWZMHM, T.CKBGDH)
