var sptslfb = require("./sptslfb.html");
var chart;
avalon.component('sptslfb', {
    template: sptslfb,
    defaults: {
        params: {},
        act: 1,
        ...
        tableCol: [],
        tableArr: [
            { name: "", label: "退税率", index: "", hidden: false, sortable: false, align: "center", width: 50 },
            {
                name: "", label: "出口额(美元)", index: "", hidden: false, sortable: false, align: "right", width: 150, formatter: function (cellvalue, options, rowObject) {
        ...
        onReady: function () {
            ...
            $('.sptslfb .datepicker.date-day').datepicker({
                dateFormat: 'yy-mm-dd'
            });
            //统计表初始化
            $('#sptslfb-chart').width($('.sptslfb').width()).height($('.sptslfb .form').height());
            // self.chartOption.title.subtext="申报年月:"+self.searchData.ssny;
            chart = echarts.init(document.getElementById('sptslfb-chart'));
            ...
            if (this.params.tjtj && this.params.tjtj.parseData === '1') {
                ...
                var zTree = $.fn.zTree.getZTreeObj("sptslfbTree2")
                var text = ''
        ...
        fleshChart: function (data) {
            ...
            for (var rowNum = 0; rowNum < data.rows.length; rowNum++) {
                ...
                tsbzData.push({ value: row[self.tableArr[3].name], name: row[self.tableArr[0].name] })
            }
        ...
        createTable: function (arr) {
            ...
            $("#sptslfb-table").jqGrid({
                datatype: "local",
                gridview: true,
                colModel: cm,
                viewrecords: true,
                rownumbers: true,
                pager: '#sptslfb-tablePager',
                shrinkToFit: true,
                width: "100%",
                multiselect: false,
                multiselectWidth: "30",
                ...
                height: (function () { return $(".sptslfb .form").height() - 90; })(),
                beforeSelectRow: function (rowid, e) {
                    if ($(e.target).hasClass('dzzd')) {
                        var b = getCellData("sptslfb-table", rowid, 'taxpayerCode')
                        vm.addTab({ title: "企业信息采集", component: "qyxxcj", params: { taxpayerCode: b } });
                        return false;
                    } else if ($(e.target).hasClass('aqm')) {
                        var url = getCellData("sptslfb-table", rowid, 'securityCodeUrl')
                        if (!url) { tools.info('链接不存在'); return false; }
                        window.open(url)
                        return false;
                    } else if ($(e.target).hasClass('zcwj')) {
                        var url = getCellData("sptslfb-table", rowid, 'zcmUrl')
                        if (!url) { tools.info('链接不存在'); return false; }
                        window.open(url);
                        return false;
                ...
                gridComplete: function () {
                    ...
                    for (var i = 0; i < self.tableArr.length; i++) {
                        if (i == 1 || i == 3) {
                            var sum = 0;
                            $("#sptslfb-table").getCol(self.tableArr[i].name, false).map(function (a) { sum += (a.replace(/\,/g, '') - 0) });
                    ...
                    $("#sptslfb-table").footerData('set', sumData);
                },
                ...
                onPaging: function (pgButton) {
                    var pageNo = tools.getPageNo(pgButton, "sptslfb-table");
                    self.search(pageNo);
                }

            });
            this.searchData.pageSize = $(".ui-pg-selbox", $('.sptslfb')).val();
            $("#sptslfb-table").setGridWidth($('.sptslfb').width())
        },
        search: function (page) {
            ...
            $("#sptslfb-table").jqGrid('clearGridData')
            ajax("POST", "/bjtssw/tjfx/loaddata", params).done(function (res) {
                if (res.code == '0') {
                    self.tableData = res.data;
                    $("#sptslfb-table").resetSelection();
                    $("#sptslfb-table")[0].addJSONData(res.data);
        ...
        initTree2: function () {
            ...
            $.fn.zTree.init($("#sptslfbTree2"), setting, data);
        },
        getCheckedChildNodes: function (type) {
            var res = { gbcode: [], dqcode: [] };
            var ckmygMc = "";
            var nodes = $.fn.zTree.getZTreeObj("sptslfbTree2").getCheckedNodes(true);
        ...
        showSelect: function (e) {
            ...
            $('.sptslfb .page').on('click', function (e) {
                var e = e || window.event;
                if ($('.select-container').find($(e.target)).length <= 0) {
                    $(".select-container").hide();
                    $('.sptslfb .page').off('click');
                }
            })
        },
        ...
        initTree: function () {
            ...
            ajax("POST", "/cxfw/export/readtree", {nodeType:"3"}).done(function (res) {
                if (res.code == '0') {
                    tree = $.fn.zTree.init($("#sptslfbTree"), setting, res.data);
                } else {
                    tools.info(res.msg)
                }
            }).fail(function (err) {
        ...
        exform: function () {
            var self = this;
            if ($('#sptslfb-table').jqGrid('getRowData').length <= 0) {
                tools.info("请先查询列表");
                return;
            }
            if (this.searchData.qylx.length == 0) {
