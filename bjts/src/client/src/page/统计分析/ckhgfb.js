var ckhgfb = require("./ckhgfb.html");
var chart;
avalon.component('ckhgfb', {
    template: ckhgfb,
    defaults: {
        params: {},
        act: 1,
        ...
        tableCol: [],
        tableArr: [
            { name: "", label: "关别代码", index: "", hidden: false, sortable: false, align: "center", width: 100 },
            { name: "", label: "海关关别", index: "", hidden: false, sortable: false, align: "left", width: 150 },
            {
                name: "", label: "本年累计出口额", index: "", hidden: false, sortable: false, align: "right", width: 150, formatter: function (cellvalue, options, rowObject) {
                    return avalon.filters.number(cellvalue, 2);
                }
            },
            { name: "", label: "占比", index: "", hidden: false, sortable: false, align: "center", width: 50 },
            {
                name: "", label: "上年同期出口额", index: "", hidden: false, sortable: false, align: "right", width: 150, formatter: function (cellvalue, options, rowObject) {
                    return avalon.filters.number(cellvalue, 2);
                }
            },
        ...
        onReady: function () {
            ...
            $('.ckhgfb .datepicker.date-day').datepicker({
                dateFormat: 'yy-mm-dd'
            });
            //统计表初始化
            $('#ckhgfb-chart').width($('.ckhgfb').width()).height($('.ckhgfb .form').height());
            // self.chartOption.title.subtext="申报年月:"+self.searchData.ssny;
            chart = echarts.init(document.getElementById('ckhgfb-chart'));
            ...
            if (this.params.tjtj && this.params.tjtj.parseData === '1') {
                ...
                var zTree = $.fn.zTree.getZTreeObj("ckhgfbTree2")
                var text = ''
        ...
        createTable: function (arr) {
            ...
            $("#ckhgfb-table").jqGrid({
                ...
                height: (function () { return $(".ckhgfb .form").height() - 90; })(),
                beforeSelectRow: function (rowid, e) {
                    if ($(e.target).hasClass('dzzd')) {
                        var b = getCellData("ckhgfb-table", rowid, 'taxpayerCode')
                        vm.addTab({ title: "企业信息采集", component: "qyxxcj", params: { taxpayerCode: b } });
                        return false;
                    } else if ($(e.target).hasClass('aqm')) {
                        var url = getCellData("ckhgfb-table", rowid, 'securityCodeUrl')
                        if (!url) { tools.info('链接不存在'); return false; }
                        window.open(url)
                        return false;
                    } else if ($(e.target).hasClass('zcwj')) {
                        var url = getCellData("ckhgfb-table", rowid, 'zcmUrl')
                        if (!url) { tools.info('链接不存在'); return false; }
                        window.open(url);
                        return false;
                ...
                gridComplete: function () {
                    ...
                    for (var i = 0; i < self.tableArr.length; i++) {
                        if (i == 2 || i == 4) {
                            var sum = 0;
                            $("#ckhgfb-table").getCol(self.tableArr[i].name, false).map(function (a) { sum += (a.replace(/\,/g, '') - 0) });
                    ...
                    $("#ckhgfb-table").footerData('set', sumData);
                },
                ...
                onPaging: function (pgButton) {
                    var pageNo = tools.getPageNo(pgButton, "ckhgfb-table");
                    self.search(pageNo);
                }

            });
            this.searchData.pageSize = $(".ui-pg-selbox", $('.ckhgfb')).val();
            $("#ckhgfb-table").setGridWidth($('.ckhgfb').width())
        },
        search: function (page) {
            ...
            $("#ckhgfb-table").jqGrid('clearGridData')
            ajax("POST", "/bjtssw/tjfx/loaddata", params).done(function (res) {
                if (res.code == '0') {
                    self.tableData = res.data;
                    $("#ckhgfb-table").resetSelection();
                    $("#ckhgfb-table")[0].addJSONData(res.data);
        ...
        initTree2: function () {
            ...
            $.fn.zTree.init($("#ckhgfbTree2"), setting, avalonRoot.ztreeNodes.gjdata);
        },
        getCheckedChildNodes: function (type) {
            var res = { gbcode: [], dqcode: [] };
            var ckmygMc = "";
            var nodes = $.fn.zTree.getZTreeObj("ckhgfbTree2").getCheckedNodes(true);
        ...
        showSelect: function (e) {
            ...
            $('.ckhgfb .page').on('click', function (e) {
                var e = e || window.event;
                if ($('.select-container').find($(e.target)).length <= 0) {
                    $(".select-container").hide();
                    $('.ckhgfb .page').off('click');
                }
            })
        },
        ...
        handleChange: function () {
            ...
            this.hideTree();
            this.ckmygMc = "";
            var tjlx = $(".ckhgfb .tjlxSelect").val();
            ...
            $.fn.zTree.getZTreeObj("ckhgfbTree2").destroy();
            var data;
            if (tjlx == "1") {
            ...
            $.fn.zTree.init($("#ckhgfbTree2"), setting, data);
        },
        reset: function () {
            var self = this;
            self.searchData = {
        ...
        initTree: function () {
            ...
            tools.getCachedSwjg(avalonRoot, ajax).done(function (data) {
                $.fn.zTree.init($("#ckhgfbTree"), setting, data);
            }).fail(function (err) {
                tools.info(err);
            });
        ...
        exform: function () {
            var self = this;
            if ($("#ckhgfb-table").jqGrid('getRowData').length <= 0) {
                tools.info("请先查询列表");
                return;
            }
            if (this.searchData.qylx.length == 0) {
