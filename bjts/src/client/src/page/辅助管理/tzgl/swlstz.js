var swlstz = require("./swlstz.html");
avalon.component("swlstz",{
    template: swlstz,
    defaults: {
        act:"1",
        swjgmc:"",
        ...
        onReady:function(){
	        ...
	        $(".swlstz .datepicker.date-day").datepicker({
		        dateFormat:"yy-mm-dd"
	        });
            this.initTree();
            this.createTable();
        ...
        createTable:function(){
            ...
            var columns = [
                ...
                // { name: "release_swjgdm", label: "发布机关", index: "release_swjgdm",width: 140, align:"left",sortable: true },
            ...
            $("#swlstz-table").jqGrid({
                ...
                width:"100%",
                height:(function(){
                    return $(".swlstz .form").height() -60;
                })(),
                beforeSelectRow:function(rowid,e){
                    var rowObject = $("#swlstz-table").jqGrid("getRowData",rowid);
                    if ($(e.target).hasClass("operation")) {
                        avalonRoot.addTab({title:"税务通知内容查看",component:"swtzbj",params:{type:"check",noticeid:rowObject.id}});
                ...
                onPaging:function(pgButton){
                    var pageNo=tools.getPageNo(pgButton,"swlstz-table");
                    self.search(pageNo);
                }
            });
        ...
        search:function(pageNo){
            var self=this;
            this.searchData.pageSize = $(".ui-pg-selbox", $('.swlstz')).val() || 20;
            var params=tools.clone(self.searchData);
            params.pageNo=pageNo;
            $("#swlstz-table").jqGrid('clearGridData')
            ajax("POST","/glfw/swlstzcx/select/first",params).done(function(res){
                if(res.code=='0'){
                    $("#swlstz-table")[0].addJSONData(res.data);
                    self.closeHyper()
                }else{
                    tools.info(res.msg);
                }
        ...
        initTree:function() {
            ...
            tools.getCachedSwjg(avalonRoot,ajax).done(function (data) {
                $.fn.zTree.init($("#swlstzZtree1"), setting1,data);
                $.fn.zTree.init($("#swlstzZtree2"), setting2,data);
            }).fail(function (err) {
                tools.info(err);
            });
        },
        showHyper:function(){
            $('.swlstz .select-sub').toggle();
            $('.swlstz .select-wrapper .icon').toggleClass("active");
            if ($('.swlstz .select-wrapper .icon').attr("title").slice(0,2) === "展开") {
                $('.swlstz .select-wrapper .icon').attr("title","收起查询条件");
            } else {
                $('.swlstz .select-wrapper .icon').attr("title","展开查询条件")
            }
        },closeHyper:function(){
            $('.swlstz .select-sub').hide();
            $('.swlstz .select-wrapper .icon').removeClass('active');
            $('.swlstz .select-wrapper .icon').attr("title","展开查询条件")
        },
        showTree:function(e){
            var self=this;
            $(".treeDiv",$(e.target).parent()).show();
            $('.swlstz').on('click',function(e){
        ...
        exform:function(){
            var self=this;
            if($('#swlstz-table').jqGrid('getRowData').length<=0){
                tools.info("请先查询列表");
                return ;
            }
            var params = tools.clone(self.searchData)
            ...
            form.attr("action", "/glfw/export/tzgl/swlstzcx");
            var input1 = $("<input>");
