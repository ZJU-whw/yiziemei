var tktzs_sc=require("./tktzs_sc.html");
avalon.component('tktzs_sc', {
	template:tktzs_sc,
	defaults: {
		params:{},
		act:1,
		...
		onReady:function(){
			...
			this.createTable();
			self.initTree();
			$('.tktzs_sc .datepicker.date-day').datepicker({
				dateFormat: 'yy-mm-dd'
			});
			$('.tktzs_sc .datepicker.date-month').datepicker({
				dateFormat: 'yymm'
			});
		},
        ...
        createTable:function(){
            ...
            $("#tktzs_sc-table").jqGrid({
                datatype: "local",
                gridview: true,
                colModel: columns,
                viewrecords: true,
                rownumbers:true,
                pager: '#tktzs_sc-tablePager',
                shrinkToFit: false,
                autowidth:true,
                altRows: true,
				width:"100%",
                ...
                height:(function(){
                    return $(".tktzs_sc .form").height() -60 -30;
                })(),
                beforeSelectRow:function(rowid,e){
                    if($(e.target).hasClass('openMx')){
                    	//获取清单操作类型（列表中的）
                        var yjflagOld = getCellData("tktzs_sc-table", rowid, 'yjflag');
                        var yjflagCNOld = getCellData("tktzs_sc-table", rowid, 'yjflagCN');
                        var no = getCellData("tktzs_sc-table", rowid, 'no');
                        var yjmsgOld = getCellData("tktzs_sc-table", rowid, 'yjmsg');
                        if(yjmsgOld){
                            ...
                            button: [
                                {
                                    ...
                                    callback: function () {
                                        ...
                                        ajax("POST","/glfw/tktzsqd/setYjflag",params).done(function(res){
                                            if(res.code=='0'){
                                                self.search(1);
                                            }else{
                                                tools.info(res.msg);
                ...
                gridComplete: function(){
                    ...
                    sumData["qyhgdm"]="合计";
                    $("#tktzs_sc-table").footerData('set', sumData);
                },
                onPaging:function(pgButton){
                    var pageNo=tools.getPageNo(pgButton,"tktzs_sc-table");
                    self.search(pageNo);
                }
            });
		...
		search:function(pageNo){
			var self=this;
			this.searchData.pageSize = $(".ui-pg-selbox", $('.tktzs_sc')).val() || 20;
			var params=tools.clone(self.searchData);
			params.pageNo=pageNo;
            $("#tktzs_sc-table").jqGrid('clearGridData')
			ajax("POST","/glfw/tktzsqd/select",params).done(function(res){
				if(res.code=='0'){
					self.tableData=res.data;
					$("#tktzs_sc-table").resetSelection();
					$("#tktzs_sc-table")[0].addJSONData(res.data);
                    self.closeHyper();
				}else{
					tools.info(res.msg);
				}
        ...
        generate:function(){
            var self=this;
            if($('#tktzs_sc-table').jqGrid('getRowData').length<=0){
                tools.info("请先查询列表");
                return ;
            }
            var params=tools.clone(self.searchData);
            ajax("POST", "/glfw/tktzsqd/generate", params).done(function (res) {
                if (res.code == '0') {
                    tools.info("清单数据已生成，请前往菜单【调库通知书清单查询】中查看！");
                    self.search(1);
                } else {
                    tools.info(res.msg);
                }
		...
		showHyper:function(){
			$('.tktzs_sc .select-sub').toggle();
			$('.tktzs_sc .select-wrapper .icon').toggleClass("active");
			if ($('.tktzs_sc .select-wrapper .icon').attr("title").slice(0,2) === "展开") {
				$('.tktzs_sc .select-wrapper .icon').attr("title","收起查询条件");
			} else {
				$('.tktzs_sc .select-wrapper .icon').attr("title","展开查询条件")
			}
		},
		closeHyper:function(){
            $('.tktzs_sc .select-sub').hide();
            $('.tktzs_sc .select-wrapper .icon').removeClass('active');
            $('.tktzs_sc .select-wrapper .icon').attr("title","展开查询条件")
		},
		filDate:function(e){
		...
		initTree:function() {
			...
			tools.getCachedSwjg(avalonRoot,ajax).done(function (data) {
				$.fn.zTree.init($(".tktzs_sc .treeDiv"), setting,data);
			}).fail(function (err) {
				tools.info(err);
			});
		},
