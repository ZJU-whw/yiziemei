var tktzs=require("./tktzs.html");
avalon.component('tktzs', {
	template:tktzs,
	defaults: {
		params:{},
		act:1,
		...
		onReady:function(){
			...
			this.createTable();
			self.initTree();
			$('.tktzs .datepicker.date-day').datepicker({
				dateFormat: 'yy-mm-dd'
			});
			$('.tktzs .datepicker.date-month').datepicker({
				dateFormat: 'yymm'
			});
		},
        ...
        createTable:function(){
            ...
            var columns = [
                ...
                { name: "no", label: "通知书号码", index: "no",width: 100, align:"center",sortable: true },
                { name: "op_date", label: "通知书日期", index: "op_date",width: 80, align:"center",sortable: true },
                ...
                { name: "md_amt", label: "免抵退金额", index: "md_amt",width: 100, align:"right",sortable: true },
            ...
            $("#tktzs-table").jqGrid({
                ...
                height:(function(){
                    return $(".tktzs .form").height() -60 -30;
                })(),
                ...
                gridComplete: function(){
                    ...
                    sumData["qyhgdm"]="合计";
                    $("#tktzs-table").footerData('set', sumData);
                },
                onPaging:function(pgButton){
                    var pageNo=tools.getPageNo(pgButton,"tktzs-table");
                    self.search(pageNo);
                }
            });
		...
		search:function(pageNo){
			var self=this;
			this.searchData.pageSize = $(".ui-pg-selbox", $('.tktzs')).val() || 20;
			var params=tools.clone(self.searchData);
			params.pageNo=pageNo;
            $("#tktzs-table").jqGrid('clearGridData')
			ajax("POST","/glfw/tktzsqd/select",params).done(function(res){
				if(res.code=='0'){
					self.tableData=res.data;
					$("#tktzs-table").resetSelection();
					$("#tktzs-table")[0].addJSONData(res.data);
                    self.closeHyper();
				}else{
					tools.info(res.msg);
				}
        ...
        withdraw:function(){
            var self = this;
            var ids = $("#tktzs-table").jqGrid('getGridParam', 'selarrrow');
            if (ids.length == 0) {
            ...
            for (var i = 0; i < uniqueIds.length; i++) {
                var no = getCellData("tktzs-table", uniqueIds[i], 'no');
                var swjgdm = getCellData("tktzs-table", uniqueIds[i], 'swjg_dm');
                //校验税务机关
                if(swjgdm != avalonRoot.user.swjgDm){
                    tools.info("只能撤回所属【"  + avalonRoot.user.swjgMc + "】下用户的通知书,请重新选择。");
                    return false;
                }
                //校验清单标志（未生成的不能撤回）
                var qdbz = getCellData("tktzs-table", uniqueIds[i], 'qdbz');
                if(qdbz != '已生成'){
                    tools.info("通知书号码【"  + no + "】的清单标志为未生成，不需要撤回，请重新选择。");
                    return false;
                }
                noArr.push(no);
                qdnoArr.push(getCellData("tktzs-table", uniqueIds[i], 'qdno'));
            }

            tools.confirm("是否执行（通知书号码" + noArr.join(',') +"）的清单撤回，相关的调库通知书状态变为[未生成]?","确定",function () {
                ajax("POST","/glfw/tktzsqd/withdraw",{noArr: noArr,qdnoArr:qdnoArr}).done(function(res){
                    if(res.code=='0'){
                        tools.info("操作成功！");
                        self.search(1);
                    }else{
		...
		showHyper:function(){
			$('.tktzs .select-sub').toggle();
			$('.tktzs .select-wrapper .icon').toggleClass("active");
			if ($('.tktzs .select-wrapper .icon').attr("title").slice(0,2) === "展开") {
				$('.tktzs .select-wrapper .icon').attr("title","收起查询条件");
			} else {
				$('.tktzs .select-wrapper .icon').attr("title","展开查询条件")
			}
		},
		closeHyper:function(){
            $('.tktzs .select-sub').hide();
            $('.tktzs .select-wrapper .icon').removeClass('active');
            $('.tktzs .select-wrapper .icon').attr("title","展开查询条件")
		},
		filDate:function(e){
		...
		initTree:function() {
			...
			tools.getCachedSwjg(avalonRoot,ajax).done(function (data) {
				$.fn.zTree.init($(".tktzs .treeDiv"), setting,data);
			}).fail(function (err) {
				tools.info(err);
			});
		},
		...
		exform:function(){
			var self=this;
            if($('#tktzs-table').jqGrid('getRowData').length<=0){
                tools.info("请先查询列表");
                return ;
            }
			var params = tools.clone(self.searchData)
			...
			form.attr("action", "/glfw/export/tktzsqd");
			var input1 = $("<input>");
