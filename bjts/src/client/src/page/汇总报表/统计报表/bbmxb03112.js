var bbmxb03112=require("./bbmxb03112.html");
var chart;
avalon.component('bbmxb03112', {
    template:bbmxb03112,
    defaults: {
        params:{
            bbdm:"",
        ...
        onReady:function(){
            ...
            self.isIE();
            //统计表初始化
            $('#bbmxb03112-chart').width($('.bbmxb03112').width());
            self.chartOption.title.subtext="申报年月:"+self.searchData.ssny;
            chart = echarts.init(document.getElementById('bbmxb03112-chart'));
        ...
        search:function(page){
            ...
            ajax("POST","/bjtssw/tjbb/loaddata",params).done(function(res){
                if(res.code=='0'){
                    ...
                    if(!self.scFlag){
                        $(".bbmxb03112 .table-body-wrapper").scroll(function () {
                            $(".bbmxb03112 .table-head-wrapper").scrollLeft($(".table-body-wrapper").scrollLeft())
                        })
                        self.scFlag=true;
                    }
                    // self.calTable();
        ...
        fleshChart:function(data){
            ...
            for(var rowNum=1;rowNum<data.rows.length;rowNum++){
                ...
                ckbzData.push({value:row['CKE_YEAR'],name:row['SPMC']})
                tsbzData.push({value:row['TSE_YEAR'],name:row['SPMC']})
            }
        ...
        showHyper:function(){
            $('.bbbbmxb03112 .hyper').toggle();
            $('.bbbbmxb03112 .hyperBtn').toggleClass('active');
        },
        closeHyper:function(){
            $('.bbbbmxb03112 .hyper').hide();
            $('.bbbbmxb03112 .hyperBtn').removeClass('active');
        },
        filDate:function(e){
        ...
        showMenu:function(e){
            ...
            $('.bbmxb03112').on('click',function(e){
        ...
        initTree:function() {
            ...
            ajax("POST","/cxfw/export/readtree", {nodeType:"3"}).done(function(res){
                if(res.code=='0'){
                    $.fn.zTree.init($(".bbbbmxb03112 .treeDiv"), setting, res.data);
                }else{
                    tools.info(res.msg)
                }
            }).fail(function(err){
