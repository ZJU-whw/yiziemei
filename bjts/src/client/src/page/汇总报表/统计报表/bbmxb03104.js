var bbmxb03104=require("./bbmxb03104.html");
var chart;
avalon.component('bbmxb03104', {
    template:bbmxb03104,
    defaults: {
        params:{
            bbdm:"",
        ...
        onReady:function(){
            ...
            self.isIE();
            //统计表初始化
            $('#bbmxb03104-chart').width($('.bbmxb03104').width()-20);
            self.chartOption.title.subtext="申报年月:"+self.searchData.ssny;
            chart = echarts.init(document.getElementById('bbmxb03104-chart'));
        ...
        search:function(page){
            ...
            ajax("POST","/bjtssw/tjbb/loaddata",params).done(function(res){
                if(res.code=='0'){
                    ...
                    if(!self.scFlag){
                        $(".bbmxb03104 .table-body-wrapper").scroll(function () {
                            $(".bbmxb03104 .table-head-wrapper").scrollLeft($(".table-body-wrapper").scrollLeft())
                        })
                        self.scFlag=true;
                    }
                    // self.calTable();
        ...
        fleshChart:function(data){
            ...
            for(var rowNum=1;rowNum<data.rows.length;rowNum++){
                ...
                ckbzData.push({value:row['CKE_YEAR'],name:row['SPDL']})
                tsbzData.push({value:row['TSE_YEAR'],name:row['SPDL']})
            }
        ...
        showHyper:function(){
            $('.bbbbmxb03104 .hyper').toggle();
            $('.bbbbmxb03104 .hyperBtn').toggleClass('active');
        },
        closeHyper:function(){
            $('.bbbbmxb03104 .hyper').hide();
            $('.bbbbmxb03104 .hyperBtn').removeClass('active');
        },
        filDate:function(e){
        ...
        showMenu:function(e){
            ...
            $('.bbmxb03104').on('click',function(e){
        ...
        initTree:function() {
            ...
            ajax("POST","/cxfw/export/readtree", {nodeType:"3"}).done(function(res){
                if(res.code=='0'){
                    $.fn.zTree.init($(".bbbbmxb03104 .treeDiv"), setting, res.data);
                }else{
                    tools.info(res.msg)
                }
            }).fail(function(err){
        ...
        showTree:function(e){
            ...
            $('.bbbbmxb03104').on('click',function(e){
                var e=e||window.event;
