var bbmxb03102=require("./bbmxb03102.html");
var chart;
avalon.component('bbmxb03102', {
    template:bbmxb03102,
    defaults: {
        params:{
            bbdm:"",
        ...
        chartOption:{//统计图配置文件
            ...
            legend: {//图例组件
                data:['生产企业退税额','外贸企业退税额','生产退税同比','外贸退税同比','退税额同比'],
                top:'bottom',
                left:'center'
            },
            toolbox: {//工具栏按钮
        ...
        onReady:function(){
            ...
            // self.initTree();
            self.isIE();
            $('.bbmxb03102 .datepicker.date-day').datepicker({
                dateFormat: 'yy-mm-dd'
            });
            $('.bbmxb03102 .datepicker.date-month').datepicker({
                dateFormat: 'yymm'
            });
            //统计表初始化
            $('#bbmxb03102-chart').width($('.bbmxb03102').width());
            self.chartOption.title.subtext="申报年月:"+self.searchData.ssny;
            chart = echarts.init(document.getElementById('bbmxb03102-chart'));
            chart.setOption(tools.clone(self.chartOption));
            // $('#bbbbmxb03102-fileupload').fileupload({
            //     dataType: 'json',
        ...
        search:function(page){
            ...
            ajax("POST","/bjtssw/tjbb/loaddata",params).done(function(res){
                if(res.code=='0'){
                    ...
                    if(!self.scFlag){
                        $(".bbmxb03102 .table-body-wrapper").scroll(function () {
                            $(".bbmxb03102 .table-head-wrapper").scrollLeft($(".table-body-wrapper").scrollLeft())
                        })
                        self.scFlag=true;
                    }
                    // self.calTable();
        ...
        fleshChart:function(data){
            var self=this;
            var barData=[data.rows[0],data.rows[1]];
            var lineData=[data.rows[6],data.rows[7],data.rows[8]];
            var barSer=[[],[]],lineSer=[[],[],[]];
            ...
            self.chartOption.series[2].data=tools.clone(lineSer[0]);
            self.chartOption.series[3].data=tools.clone(lineSer[1]);
            self.chartOption.series[4].data=tools.clone(lineSer[2]);
            chart.setOption(tools.clone(self.chartOption));
        },
        saveBB:function(){
            var self=this;
        ...
        showMenu:function(e){
            ...
            $('.bbmxb03102').on('click',function(e){
