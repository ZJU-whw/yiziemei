var rwlbMx=require("./rwlbMx.html");
avalon.component('rwlbMx', {
    template:rwlbMx,
    defaults: {
        params:{
            bbdldm:"",
        ...
        onReady:function(){
            ...
            this.exClass="rwlbMx"+this.params.bbdldm+this.params.ny;
        ...
        getTableRow:function(){
            ...
            var tableArr=[
                ...
                { name: "", label:"操作", width:180, align:"center", resizable: false, search: false, sortable: false,editable :false,formatter: function(cellvalue, options, rowObject){
                    if(self.TZswitch=='1'){
        ...
        createTable:function(arr){
            ...
            $("#rwlbMx-table"+self.exClass).jqGrid({
                datatype: "local",
                gridview: true,
                colModel: cm,
                viewrecords: true,
                rownumbers:true,
                pager: '#rwlbMx-tablePager'+self.exClass,
                shrinkToFit: true,
                width:"100%",
                multiselect: true,
                multiselectWidth:"30",
                ...
                beforeSelectRow:function(rowid,e){
                    if($(e.target).hasClass('tz')){
                        var params={
                            bbdm:getCellData("rwlbMx-table"+self.exClass, rowid, 'bbdm'),
                            bbid:getCellData("rwlbMx-table"+self.exClass, rowid, 'bbid'),
                            ssny:self.searchData.ssny,
                            TZswitch:self.TZswitch,
                            bbmc:getCellData("rwlbMx-table"+self.exClass, rowid, 'bbmc'),
                            swjgdm:self.swjgdm,
                            swjgmc:self.swjgmc,
                        }
                ...
                onPaging:function(pgButton){
                    var pageNo=tools.getPageNo2(pgButton,"rwlbMx-tablePager"+self.exClass);
                    self.search(pageNo);
                },
            })
        ...
        resetTable:function() {
            var self = this;
            $("#rwlbMx-table"+self.exClass).setGridWidth($('.'+self.exClass+'').width())
        },
        search:function(pageNo){
            ...
            $("#rwlbMx-table"+self.exClass).jqGrid('clearGridData')
            ajax("POST","/bjtssw/tjbb/task/sublist",params).done(function(res){
                if(res.code=='0'){
                    $("#rwlbMx-table"+self.exClass)[0].addJSONData(res.data);
        ...
        bbtj:function(){
            var self=this;
            var incode=[]
            var rowids=$("#rwlbMx-table"+self.exClass).jqGrid("getGridParam", "selarrrow");
            for (var i = 0; i < rowids.length; i++) {
                var b = getCellData("rwlbMx-table"+self.exClass, rowids[i], 'bbdm');
                incode.push(b);
            }
            if(incode.length<=0){
        ...
        exTable:function(type){
            var self=this;
            if($("#rwlbMx-table"+self.exClass).jqGrid('getRowData').length<=0){
                tools.info("请先查询列表");
                return ;
            }
            var params = {
