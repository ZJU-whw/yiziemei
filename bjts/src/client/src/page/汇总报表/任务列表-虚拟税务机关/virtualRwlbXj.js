var virtualRwlbXj=require("./virtualRwlbXj.html");
avalon.component('virtualRwlbXj', {
    template:virtualRwlbXj,
    defaults: {
        params:{
            bbdldm:"",
            bbdlmc:"",
        ...
        onReady:function(){
            ...
            this.exClass="virtualRwlbXj"+this.params.bbdldm;
        ...
        createTable:function(arr){
            ...
            $("#virtualRwlbXj-table"+self.exClass).jqGrid({
                datatype: "local",
                gridview: true,
                colModel: cm,
                viewrecords: true,
                rownumbers:true,
                pager: '#virtualRwlbXj-tablePager'+self.exClass,
                shrinkToFit: true,
                width:"100%",
                // multiselect: true,
                // multiselectWidth:"30",
                ...
                beforeSelectRow:function(rowid,e){
                    if($(e.target).hasClass('openMx')){
                        ...
                        var params ={
                            ...
                            swjgdm:getCellData("virtualRwlbXj-table"+self.exClass, rowid, 'swjgdm'),
                            swjgmc:getCellData("virtualRwlbXj-table"+self.exClass, rowid, 'swjgmc'),
                            TZswitch:0,
                        }
                        ...
                        var b = getCellData("virtualRwlbXj-table"+self.exClass, rowid, 'swjgdm')
                        self.dzch(b);
                        return false;
                ...
                onPaging:function(pgButton){
                    var pageNo=tools.getPageNo2(pgButton,"virtualRwlbXj-tablePager"+self.exClass);
                    self.search(pageNo);
                },
            })
        ...
        resetTable:function() {
            var self = this;
            $("#virtualRwlbXj-table"+self.exClass).setGridWidth($('.'+self.exClass+'').width())
        },
        search:function(pageNo){
            ...
            $("#virtualRwlbXj-table"+self.exClass).jqGrid('clearGridData')
            ajax("POST","/bjtssw/tjbb/task/recivelist",params).done(function(res){
                if(res.code=='0'){
                    self.tableData=res.data;
                    $("#virtualRwlbXj-table"+self.exClass)[0].addJSONData(res.data);
                }else{
                    tools.info(res.msg);
                }
            }).fail(function(err){
