var wmqysbMx=require("./wmqysbMx.html");
avalon.component('wmqysbMx', {
    template:wmqysbMx,
    defaults: {
        params:{
            lcslid:""
        },
        tcode: 'wmqysbxxcxSbsj',
        searchData: {
            shxyno: "",
            sssq: "",
            sbpc: "",
        ...
        createTable:function(cols){
            var self=this;
            $("#wmqysbMx-table"+this.tableNum).jqGrid({
                datatype: "local",
                gridview: true,
                colModel: cols,
                viewrecords: true,
                rownumbers:true,
                pager: '#wmqysbMx-tablePager'+this.tableNum,
                shrinkToFit: false,
                width:"100%",
                autowidth:true,
                altRows: true,
                ...
                onPaging:function(pgButton){
                    var pageNo=tools.getPageNo2(pgButton,"wmqysbMx-tablePager"+self.tableNum);
                    self.search(pageNo);
                }

            });
            this.searchData.pageSize = $(".ui-pg-selbox", $('.wmqysbMx')).val();
            this.search(1)
        },
        //copy bg
        ...
        search:function(pageNo){
            var self = this
            this.searchData.pageSize = $(".ui-pg-selbox", $('.wmqysbMx')).val() || 20;
            var params=tools.clone(self.searchData);
            params.pageNo=pageNo;
            $("#wmqysbMx-table"+this.tableNum).jqGrid('clearGridData')
            ajax("POST","/cxfw/wmqysbxxcx/second/sbsj",params).done(function(res){
                if(res.code=='0'){
                    $("#wmqysbMx-table"+self.tableNum).resetSelection();
                    $("#wmqysbMx-table"+self.tableNum)[0].addJSONData(res.data);
                }else{
                    tools.info(res.msg);
                }
            }).fail(function(err){
