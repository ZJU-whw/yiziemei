var wmqysb=require("./wmqysb.html");
avalon.component('wmqysb', {
    template:wmqysb,
    defaults: {
        params:{},
        act:1,
        ...
        onReady:function(){
            ...
            this.getTableRow();
            self.initTree();
            $('.wmqysb .datepicker.date-day').datepicker({
                dateFormat: 'yy-mm-dd'
            });
            $('.wmqysb .datepicker.date-month').datepicker({
                dateFormat: 'yymm'
            });
            this.getGroup()
        },
        ...
        createTable:function(arr){
            ...
            $("#wmqysb-table").jqGrid({
                ...
                beforeSelectRow:function(rowid,e){
                    if($(e.target).hasClass('openMx')){
                        ...
                        avalonRoot.addTab({title:"外贸企业申报明细",component:"wmqysbMx",sameCheck:false,params:{lcslid:obj.lcslid,shxyno:obj.shxyno,sssq:obj.sb_ym,sbpc:obj.sb_pc}});
                        return false;
                ...
                gridComplete: function(){
                    ...
                    $("#wmqysb-table").footerData('set', sumData);
                },
                onPaging:function(pgButton){
                    var pageNo=tools.getPageNo(pgButton,"wmqysb-table");
                    self.search(pageNo);
                }

            });
            $("#wmqysb-table").jqGrid('setGroupHeaders', {
            ...
            this.searchData.pageSize = $(".ui-pg-selbox", $('.wmqysb')).val();
            // self.search(1)
        },
        ...
        resetTable:function() {
            ...
            for (var i = 0; i < self.tableOption.length; i++) {
                if (self.tableOption[i].show == true) {
                    $("#wmqysb-table").showCol(self.tableOption[i].name)
                } else {
                    $("#wmqysb-table").hideCol(self.tableOption[i].name)
                }
            }
            $("#wmqysb-table").setGridWidth($('.wmqysb').width())
        },normalSearch(){
        ...
        search:function(pageNo){
            ...
            this.searchData.pageSize = $(".ui-pg-selbox", $('.wmqysb')).val()  || 20;
            var params=tools.clone(self.searchData);
            params.pageNo=pageNo
            $("#wmqysb-table").jqGrid('clearGridData')
            ajax("POST","/cxfw/wmqysbxxcx/first",params).done(function(res){
                if(res.code=='0'){
                    self.tableData=res.data;
                    $("#wmqysb-table").resetSelection();
                    $("#wmqysb-table")[0].addJSONData(res.data);
                    self.closeHyper();
                }else{
                    tools.info(res.msg);
                }
        ...
        showHyper:function(){
            $('.wmqysb .select-sub').toggle();
            $('.wmqysb .select-wrapper .icon').toggleClass("active");
            if ($('.wmqysb .select-wrapper .icon').attr("title").slice(0,2) === "展开") {
                $('.wmqysb .select-wrapper .icon').attr("title","收起查询条件");
            } else {
                $('.wmqysb .select-wrapper .icon').attr("title","展开查询条件")
            }
        },
        closeHyper:function(){
            $('.wmqysb .select-sub').hide();
            $('.wmqysb .select-wrapper .icon').removeClass('active');
            $('.wmqysb .select-wrapper .icon').attr("title","展开查询条件");
        },
        filDate:function(e){
        ...
        initTree:function() {
            ...
            tools.getCachedSwjg(avalonRoot,ajax).done(function (data) {
                $.fn.zTree.init($(".wmqysb .treeDiv"), setting, data);
            }).fail(function (err) {
                tools.info(err);
            });
        },
        ...
        exform:function(){
            var self=this;
            if($('#wmqysb-table').jqGrid('getRowData').length<=0){
                tools.info("请先查询列表");
                return ;
            }
            var params = tools.clone(self.searchData)
            ...
            form.attr("action", "/cxfw/export/wmqysbxxcx");
            var input1 = $("<input>");
