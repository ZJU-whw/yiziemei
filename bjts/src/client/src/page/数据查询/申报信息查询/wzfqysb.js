var wzfqysb=require("./wzfqysb.html");
avalon.component('wzfqysb', {
    template:wzfqysb,
    defaults: {
        params:{},
        act:1,
        ...
        onReady:function(){
            ...
            this.getTableRow();
            self.initTree();
            $('.wzfqysb .datepicker.date-day').datepicker({
                dateFormat: 'yy-mm-dd'
            });
            $('.wzfqysb .datepicker.date-month').datepicker({
                dateFormat: 'yymm'
            });
            this.getGroup()
        },
        ...
        createTable:function(arr){
            ...
            $("#wzfqysb-table").jqGrid({
                datatype: "local",
                gridview: true,
                colModel: cm,
                viewrecords: true,
                rownumbers:true,
                pager: '#wzfqysb-tablePager',
                shrinkToFit: false,
                width:"100%",
                // multiselect: true,
                // multiselectWidth:"30",
                ...
                height:(function(){
                    return $(".wzfqysb .form").height() -120;
                })(),
                beforeSelectRow:function(rowid,e){
                    if($(e.target).hasClass('openMx')){
                        ...
                        avalonRoot.addTab({title:"外综服企业申报明细",component:"wzfqysbMx",sameCheck:false,params:{lcslid:obj.lcslid,shxyno:obj.shxyno,sssq:obj.sb_ym,sbpc:obj.sb_pc}});
                        return false;
                ...
                gridComplete: function(){
                    ...
                    $("#wzfqysb-table").footerData('set', sumData);
                },
                onPaging:function(pgButton){
                    var pageNo=tools.getPageNo(pgButton,"wzfqysb-table");
                    self.search(pageNo);
                }

            });
            $("#wzfqysb-table").jqGrid('setGroupHeaders', {
            ...
            this.searchData.pageSize = $(".ui-pg-selbox", $('.wzfqysb')).val();
            // self.search(1)
        },
        ...
        resetTable:function() {
            ...
            for (var i = 0; i < self.tableOption.length; i++) {
                if (self.tableOption[i].show == true) {
                    $("#wzfqysb-table").showCol(self.tableOption[i].name)
                } else {
                    $("#wzfqysb-table").hideCol(self.tableOption[i].name)
                }
            }
            $("#wzfqysb-table").setGridWidth($('.wzfqysb').width())
        },normalSearch(){
        ...
        search:function(pageNo){
            ...
            this.searchData.pageSize = $(".ui-pg-selbox", $('.wzfqysb')).val() || 20;
            var params=tools.clone(self.searchData);
            params.pageNo=pageNo
            $("#wzfqysb-table").jqGrid('clearGridData')
            ajax("POST","/cxfw/wzfqysbcx/first",params).done(function(res){
                if(res.code=='0'){
                    self.tableData=res.data;
                    $("#wzfqysb-table").resetSelection();
                    $("#wzfqysb-table")[0].addJSONData(res.data);
                    self.closeHyper();
                }else{
                    tools.info(res.msg);
                }
        ...
        showHyper:function(){
            $('.wzfqysb .select-sub').toggle();
            $('.wzfqysb .select-wrapper .icon').toggleClass("active");
            if ($('.wzfqysb .select-wrapper .icon').attr("title").slice(0,2) === "展开") {
                $('.wzfqysb .select-wrapper .icon').attr("title","收起查询条件");
            } else {
                $('.wzfqysb .select-wrapper .icon').attr("title","展开查询条件")
            }
        },
        closeHyper:function(){
            $('.wzfqysb .select-sub').hide();
            $('.wzfqysb .select-wrapper .icon').removeClass('active');
            $('.wzfqysb .select-wrapper .icon').attr("title","展开查询条件")
        },
        filDate:function(e){
        ...
        initTree:function() {
            ...
            tools.getCachedSwjg(avalonRoot,ajax).done(function (data) {
                $.fn.zTree.init($(".wzfqysb .treeDiv"), setting, data);
            }).fail(function (err) {
                tools.info(err);
            });
        },
        ...
        exform:function(){
            var self=this;
            if($('#wzfqysb-table').jqGrid('getRowData').length<=0){
                tools.info("请先查询列表");
                return ;
            }
            var params = tools.clone(self.searchData)
            ...
            form.attr("action", "/cxfw/export/wzfqysbcx");
            var input1 = $("<input>");
