var fwmysb=require("./fwmysb.html");
avalon.component('fwmysb', {
    template:fwmysb,
    defaults: {
        params:{},
        act:1,
        ...
        onReady:function(){
            ...
            this.getTableRow();
            self.initTree();
            $('.fwmysb .datepicker.date-day').datepicker({
                dateFormat: 'yy-mm-dd'
            });
            $('.fwmysb .datepicker.date-month').datepicker({
                dateFormat: 'yymm'
            });
            this.getGroup()
        },
        ...
        createTable:function(arr){
            ...
            $("#fwmysb-table").jqGrid({
                ...
                beforeSelectRow:function(rowid,e){
                    if($(e.target).hasClass('openMx')){
                        ...
                        avalonRoot.addTab({title:"服务贸易申报明细",component:"fwmysbMx",sameCheck:false,params:{lcslid:obj.lcslid,tsjsfs:obj.tsjsfs,shxyno:obj.shxyno,sssq:obj.sb_ym}});
                        return false;
                ...
                gridComplete: function(){
                    ...
                    $("#fwmysb-table").footerData('set', sumData);
                },
                onPaging:function(pgButton){
                    var pageNo=tools.getPageNo(pgButton,"fwmysb-table");
                    self.search(pageNo);
                }

            });
            $("#fwmysb-table").jqGrid('setGroupHeaders', {
            ...
            this.searchData.pageSize = $(".ui-pg-selbox", $('.fwmysb')).val();
            // self.search(1)
        },
        ...
        resetTable:function() {
            ...
            for (var i = 0; i < self.tableOption.length; i++) {
                if (self.tableOption[i].show == true) {
                    $("#fwmysb-table").showCol(self.tableOption[i].name)
                } else {
                    $("#fwmysb-table").hideCol(self.tableOption[i].name)
                }
            }
            $("#fwmysb-table").setGridWidth($('.fwmysb').width())
        },normalSearch(){
        ...
        search:function(pageNo){
            ...
            this.searchData.pageSize = $(".ui-pg-selbox", $('.fwmysb')).val() || 20;
            var params=tools.clone(self.searchData);
            params.pageNo=pageNo
            $("#fwmysb-table").jqGrid('clearGridData')
            ajax("POST","/cxfw/fwmysbcx/first",params).done(function(res){
                if(res.code=='0'){
                    self.tableData=res.data;
                    $("#fwmysb-table").resetSelection();
                    $("#fwmysb-table")[0].addJSONData(res.data);
        ...
        showHyper:function(){
            $('.fwmysb .select-sub').toggle();
            $('.fwmysb .select-wrapper .icon').toggleClass("active");
            if ($('.fwmysb .select-wrapper .icon').attr("title").slice(0,2) === "展开") {
                $('.fwmysb .select-wrapper .icon').attr("title","收起查询条件");
            } else {
                $('.fwmysb .select-wrapper .icon').attr("title","展开查询条件")
            }
        },
        closeHyper:function(){
            $('.fwmysb .select-sub').hide();
            $('.fwmysb .select-wrapper .icon').removeClass('active');
            $('.fwmysb .select-wrapper .icon').attr("title","展开查询条件")
        },
        filDate:function(e){
        ...
        initTree:function() {
            ...
            tools.getCachedSwjg(avalonRoot,ajax).done(function (data) {
                $.fn.zTree.init($(".fwmysb .treeDiv"), setting,data);
            }).fail(function (err) {
                tools.info(err);
            });
        },
        ...
        exform:function(){
            var self=this;
            if($('#fwmysb-table').jqGrid('getRowData').length<=0){
                tools.info("请先查询列表");
                return ;
            }
            var params = tools.clone(self.searchData)
            ...
            form.attr("action", "/cxfw/export/fwmysbcx");
            var input1 = $("<input>");
