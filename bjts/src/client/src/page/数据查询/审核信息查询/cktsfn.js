var cktsfn=require("./cktsfn.html");
avalon.component('cktsfn', {
    template:cktsfn,
    defaults: {
        params:{},
        act:1,
        ...
        onReady:function(){
	        ...
	        this.getTableRow();
            self.initTree();
            $('.cktsfn  .datepicker.date-day').datepicker({
                dateFormat: 'yy-mm-dd'
            });
            $('.cktsfn  .datepicker.date-month').datepicker({
                dateFormat: 'yymm'
            });
        },
        ...
        creatChart:function(){
            echarts.init($('.cktsfn .chartDiv')[0]).setOption({
                title: {
                    text: '天气情况统计',
                    subtext: '虚构数据',
                    left: 'center'
	    ...
	    createTable:function(arr){
		    ...
		    $("#cktsfn-table").jqGrid({
			    ...
			    height:(function(){
				    return $(".cktsfn .form").height() -60-30;
			    })(),
			    ...
			    onSortCol: function (index, iCol, sortorder) {
                    ...
                    $("#cktsfn-table").footerData('set', sumData);
                },
			    onPaging:function(pgButton){
				    var pageNo=tools.getPageNo(pgButton,"cktsfn-table");
				    self.search(pageNo);
			    }
		    })
		    this.searchData.pageSize = $(".ui-pg-selbox", $('.cktsfn')).val();
	    },
	    ...
	    resetTable:function() {
		    ...
		    for (var i = 0; i < self.tableOption.length; i++) {
			    if (self.tableOption[i].show == true) {
				    $("#cktsfn-table").showCol(self.tableOption[i].name)
			    } else {
				    $("#cktsfn-table").hideCol(self.tableOption[i].name)
			    }
		    }
		    $("#cktsfn-table").setGridWidth($('.cktsfn').width())
	    },
	    search:function(pageNo){
		    ...
		    if (isValid) {
			    this.searchData.pageSize = $(".ui-pg-selbox", $('.cktsfn')).val() || 20;
			    var params=tools.clone(self.searchData);
			    params.pageNo=pageNo
                $("#cktsfn-table").jqGrid('clearGridData')
			    ajax("POST","/cxfw/fnqkcx",params).done(function(res){
				    if(res.code=='0'){
				    	self.tableData=res.data;
					    $("#cktsfn-table").resetSelection();
					    $("#cktsfn-table")[0].addJSONData(res.data);
	    ...
	    showHyper:function(){
		    $('.cktsfn .select-sub').toggle();
		    $('.cktsfn .select-wrapper .icon').toggleClass("active");
		    if ($('.cktsfn .select-wrapper .icon').attr("title").slice(0,2) === "展开") {
			    $('.cktsfn .select-wrapper .icon').attr("title","收起查询条件");
		    } else {
			    $('.cktsfn .select-wrapper .icon').attr("title","展开查询条件")
		    }
	    },
	    closeHyper:function(){
            $('.cktsfn .select-sub').hide();
            $('.cktsfn .select-wrapper .icon').removeClass('active');
            $('.cktsfn .select-wrapper .icon').attr("title","展开查询条件")
	    },
        filDate:function(e){
        ...
        initTree:function() {
            ...
            tools.getCachedSwjg(avalonRoot,ajax).done(function (data) {
                $.fn.zTree.init($(".cktsfn .treeDiv"), setting, data);
            }).fail(function (err) {
                tools.info(err);
            });
        },
        ...
        exform:function(){
	        var self=this;
            if($('#cktsfn-table').jqGrid('getRowData').length<=0){
                tools.info("请先查询列表");
                return ;
            }
	        var params = tools.clone(self.searchData)
	        ...
	        form.attr("action", "/cxfw/export/cktsfncx");
	        var input1 = $("<input>");
