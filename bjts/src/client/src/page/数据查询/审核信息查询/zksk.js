var zksk=require("./zksk.html");
avalon.component('zksk', {
    template:zksk,
    defaults: {
        params:{},
        act:1,
        ...
        onReady:function(){
            ...
            this.getTableRow();
            self.initTree();
            $('.zksk .datepicker.date-day').datepicker({
                dateFormat: 'yy-mm-dd'
            });
            $('.zksk .datepicker.date-month').datepicker({
                dateFormat: 'yymm'
            });
        ...
        creatChart:function(){
            echarts.init($('.zksk .chartDiv')[0]).setOption({
                title: {
                    text: '天气情况统计',
                    subtext: '虚构数据',
                    left: 'center'
        ...
        createTable:function(arr){
            ...
            $("#zksk-table").jqGrid({
                ...
                height:(function(){
                    return $(".zksk .form").height() -100;
                })(),
                beforeSelectRow:function(rowid,e){
                    if($(e.target).hasClass('openMx')){
                        var b = getCellData("zksk-table", rowid, 'taxpayerCode')
                        avalonRoot.addTab({title:"暂扣税款明细",component:"zkskMx",sameCheck:false,params:{lcslid:self.tableData.rows[rowid-1].lcslid}});
                        return false;
                ...
                onPaging:function(pgButton){
                    var pageNo=tools.getPageNo(pgButton,"zksk-table");
                    self.search(pageNo);
                },
                // footerrow:true,
            ...
            this.searchData.pageSize = $(".ui-pg-selbox", $('.zksk')).val();
            // self.search(1)
        },
        ...
        resetTable:function() {
            ...
            for (var i = 0; i < self.tableOption.length; i++) {
                if (self.tableOption[i].show == true) {
                    $("#zksk-table").showCol(self.tableOption[i].name)
                } else {
                    $("#zksk-table").hideCol(self.tableOption[i].name)
                }
            }
            $("#zksk-table").setGridWidth($('.zksk').width())
        },
        search:function(pageNo){
	        ...
	        var isValid = tools.validate("zksk-form",fields);
	        if (isValid) {
		        this.searchData.pageSize = $(".ui-pg-selbox", $('.zksk')).val() || 20;
		        var params=tools.clone(self.searchData);
		        params.pageNo=pageNo
                $("#zksk-table").jqGrid('clearGridData')
		        ajax("POST","/cxfw/zkskcx",params).done(function(res){
			        if(res.code=='0'){
				        $("#zksk-table").resetSelection();
				        $("#zksk-table")[0].addJSONData(res.data);
	    ...
	    showHyper:function(){
		    $('.zksk .select-sub').toggle();
		    $('.zksk .select-wrapper .icon').toggleClass("active");
		    if ($('.zksk .select-wrapper .icon').attr("title").slice(0,2) === "展开") {
			    $('.zksk .select-wrapper .icon').attr("title","收起查询条件");
		    } else {
			    $('.zksk .select-wrapper .icon').attr("title","展开查询条件")
		    }
	    },
	    closeHyper:function(){
            $('.zksk .select-sub').hide();
            $('.zksk .select-wrapper .icon').removeClass('active');
            $('.zksk .select-wrapper .icon').attr("title","展开查询条件")
	    },
        filDate:function(e){
        ...
        initTree:function() {
            ...
            tools.getCachedSwjg(avalonRoot,ajax).done(function (data) {
                $.fn.zTree.init($(".zksk .treeDiv"), setting,data);
            }).fail(function (err) {
                tools.info(err);
            });
        },
        ...
        exform:function(){
            var self=this;
            if($('#zksk-table').jqGrid('getRowData').length<=0){
                tools.info("请先查询列表");
                return ;
            }
            var params = tools.clone(self.searchData)
            ...
            form.attr("action", "/cxfw/export/zkskcx");
            var input1 = $("<input>");
