var lljg=require("./lljg.html");
avalon.component('lljg', {
    template:lljg,
    defaults: {
        params:{},
        act:1,
        ...
        onReady:function(){
            ...
            this.getTableRow();
            self.initTree();
            $('.lljg .datepicker.date-day').datepicker({
                dateFormat: 'yy-mm-dd'
            });
            $('.lljg .datepicker.date-month').datepicker({
                dateFormat: 'yymm'
            });
        ...
        creatChart:function(){
            echarts.init($('.lljg .chartDiv')[0]).setOption({
                title: {
                    text: '天气情况统计',
                    subtext: '虚构数据',
                    left: 'center'
        ...
        createTable:function(arr){
            ...
            $("#lljg-table").jqGrid({
                ...
                height:(function(){
                    return $(".lljg .form").height() -100;
                })(),
                beforeSelectRow:function(rowid,e){
                    if($(e.target).hasClass('openMx')){
                        var b = getCellData("lljg-table", rowid, 'taxpayerCode')
                        avalonRoot.addTab({title:"来料加工手册核销明细",component:"lljgMx",sameCheck:false,params:{lcslid:self.tableData.rows[rowid-1].lcslid}});
                        return false;
                ...
                onPaging:function(pgButton){
                    var pageNo=tools.getPageNo(pgButton,"lljg-table");
                    self.search(pageNo);
                },
                // footerrow:true,
            ...
            this.searchData.pageSize = $(".ui-pg-selbox", $('.lljg')).val();
            // self.search(1)
        },
        ...
        resetTable:function() {
            ...
            for (var i = 0; i < self.tableOption.length; i++) {
                if (self.tableOption[i].show == true) {
                    $("#lljg-table").showCol(self.tableOption[i].name)
                } else {
                    $("#lljg-table").hideCol(self.tableOption[i].name)
                }
            }
            $("#lljg-table").setGridWidth($('.lljg').width())
        },
        search:function(pageNo){
	        ...
	        if (isValid) {
		        this.searchData.pageSize = $(".ui-pg-selbox", $('.lljg')).val() || 20;
		        var params=tools.clone(self.searchData);
		        params.pageNo=pageNo
                $("#lljg-table").jqGrid('clearGridData')
		        ajax("POST","/cxfw/lljgzmcx",params).done(function(res){
			        if(res.code=='0'){
				        $("#lljg-table").resetSelection();
				        $("#lljg-table")[0].addJSONData(res.data);
        ...
        showHyper:function(){
            $('.lljg .select-sub').toggle();
            $('.lljg .select-wrapper .icon').toggleClass("active");
            if ($('.lljg .select-wrapper .icon').attr("title").slice(0,2) === "展开") {
                $('.lljg .select-wrapper .icon').attr("title","收起查询条件");
            } else {
                $('.lljg .select-wrapper .icon').attr("title","展开查询条件")
            }
        },
        closeHyper:function(){
            $('.lljg .select-sub').hide();
            $('.lljg .select-wrapper .icon').removeClass('active');
            $('.lljg .select-wrapper .icon').attr("title","展开查询条件");
        },
        filDate:function(e){
        ...
        initTree:function() {
            ...
            tools.getCachedSwjg(avalonRoot,ajax).done(function (data) {
                $.fn.zTree.init($(".lljg .treeDiv"), setting,data);
            }).fail(function (err) {
                tools.info(err);
            });
        },
        ...
        exform:function(){
            var self=this;
            if($('#lljg-table').jqGrid('getRowData').length<=0){
                tools.info("请先查询列表");
                return ;
            }
            var params = tools.clone(self.searchData)
            ...
            form.attr("action", "/cxfw/export/lljgzmcx");
            var input1 = $("<input>");
