var wmckyyzl=require("./wmckyyzl.html");
avalon.component('wmckyyzl', {
    template:wmckyyzl,
    defaults: {
      params:{},
      act:1,
      tcode:"wmckyyzl",
        ...
        qylxOptions: [
          { value: '1', name: '生产型出口企业' },
          { value: '2', name: '流通型出口企业' },
          { value: '3', name: '市场采购贸易' },
          { value: '4', name: '跨境电商无票免税' }
        ],
        tableData:{},
        onReady:function(){
            try {
        ...
        initDate: function(){
          $('.wmckyyzl .datepicker.date-month').datetimepicker({
            language:'zh-CN',
            format: 'yyyymm',
            weekStart: 1,
            // todayBtn: true,
          ...
          $('.wmckyyzl .datepicker.date-day').datepicker({
            dateFormat: 'yy-mm-dd'
          });
        },
        createTable:function(){
            ...
            $("#wmckyyzl-table").jqGrid({
              datatype: "local",
              gridview: true,
              colModel: columns,
              viewrecords: true,
              rownumbers:true,
              pager: '#wmckyyzl-tablePager',
              shrinkToFit: false,
              width:"100%",
              // multiselect: true,
              // multiselectWidth:"30",
              ...
              height:(function(){
                  return $(".wmckyyzl .form").height() -60;
              })(),
              beforeSelectRow:function(rowid,e){
                var row = $('#wmckyyzl-table').getRowData(rowid)
              ...
              onPaging:function(pgButton){
                var pageNo=tools.getPageNo(pgButton,"wmckyyzl-table");
                self.search(pageNo);
              }
            })
            this.searchData.pageSize = $(".ui-pg-selbox", $('.wmckyyzl')).val();
            self.search(1)
        },
        search:function(pageNo){
          ...
          this.searchData.pageSize = $(".ui-pg-selbox", $('.wmckyyzl')).val() || 20;
          var params=tools.clone(self.searchData);
          params.pageNo=pageNo
          $("#wmckyyzl-table").jqGrid('clearGridData')
          ajax("POST","/dzba/apply/list",params).done(function(res){
            if(res.code=='0'){
              $("#wmckyyzl-table").resetSelection();
              $("#wmckyyzl-table")[0].addJSONData(res.data);
      ...
      exform:function(){
          if($('#wmckyyzl-table').jqGrid('getRowData').length<=0){
              tools.info("请先查询列表");
              return ;
          }
          var self=this;
      ...
      initTree:function() {
        ...
        ajax("POST","/cxfw/export/readtree",{nodeType:"3"}).done(function(res){
            if(res.code=='0'){
              $.fn.zTree.init($(".wmckyyzl .treeDiv"), setting, res.data);
            }else{
                tools.info(res.msg)
            }
        }).fail(function(err){
