var shrzcx = require("./shrzcx.html");
avalon.component("shrzcx", {
  template: shrzcx,
  defaults: {
    params: {},
    act: 1,
    ...
    onReady: function () {
      ...
      //   self.searchData.rqlx = self.params.rqlx?self.params.rqlx:''
      ...
      $(".shrzcx .datepicker.date-day").datepicker({
        dateFormat: "yy-mm-dd",
      });
      $(".shrzcx .datepicker.date-month").datepicker({
        dateFormat: "yymm",
      });
    },
    ...
    //   cm.push(op)
      $("#shrzcx-table").jqGrid({
        ...
        height: (function () {
          return $(".shrzcx .form").height() - 70;
        })(),
        beforeSelectRow: function (rowid, e) {
          var row = $("#shrzcx-table").jqGrid("getRowData", rowid);
        ...
        onPaging: function (pgButton) {
          var pageNo = tools.getPageNo(pgButton, "shrzcx-table");
          self.search(pageNo);
        },
        ...
        //       $("#shrzcx-table").footerData("set", sumData);
        //     }, 200);
        //   } catch (error) {
        //     console.log(error);
        //   }
    ...
    //   $("#shrzcx-table").jqGrid("setGroupHeaders", {
    //     useColSpanStyle: true,
    //     groupHeaders: [
    //       {
    //         startColumnName: "djxh",
    //         numberOfColumns: 4,
    //         titleText: "企业基本信息",
    //       },
    //       {
    ...
    //   });
      ...
      //   ]
      // });
      this.searchData.pageSize = $(".ui-pg-selbox", $(".shrzcx")).val();
    },
    ...
    resetTable: function () {
      ...
      for (var i = 0; i < self.tableOption.length; i++) {
        if (self.tableOption[i].show == true) {
          $("#shrzcx-table").showCol(self.tableOption[i].name);
        } else {
          $("#shrzcx-table").hideCol(self.tableOption[i].name);
        }
      }
      $("#shrzcx-table").setGridWidth($(".shrzcx").width());
    },
    normalSearch() {
    ...
    search: function (pageNo) {
      var self = this;
      var dataValid = [
        { start: "fqsjQ", end: "fqsjZ", msg: "查询日期" },
      ...
      this.searchData.pageSize =
        $(".ui-pg-selbox", $(".shrzcx")).val() || 20;
      var params = tools.clone(self.searchData);
      params.pageNo = pageNo;
      $("#shrzcx-table").jqGrid("clearGridData");
      // ajax("POST", "/cxfw/fxgl/hdtj/mx", params)
      ajax("POST", "/cxfw/cqwsb/shmx/tj/list", params)
        .done(function (res) {
          if (res.code == "0") {
            self.tableData = res.data;
            $("#shrzcx-table").resetSelection();
            $("#shrzcx-table")[0].addJSONData(res.data);
            self.closeHyper();
          } else {
            tools.info(res.msg);
          }
    ...
    showHyper: function () {
      $(".shrzcx .select-sub").toggle();
      $(".shrzcx .select-wrapper .icon").toggleClass("active");
      if (
        $(".shrzcx .select-wrapper .icon").attr("title").slice(0, 2) ===
        "展开"
      ) {
        $(".shrzcx .select-wrapper .icon").attr("title", "收起查询条件");
      } else {
        $(".shrzcx .select-wrapper .icon").attr("title", "展开查询条件");
      }
    },
    closeHyper: function () {
      $(".shrzcx .select-sub").hide();
      $(".shrzcx .select-wrapper .icon").removeClass("active");
      $(".shrzcx .select-wrapper .icon").attr("title", "展开查询条件");
    },
    filDate: function (e) {
    ...
    initTree: function () {
      ...
      tools
        ...
        .done(function (data) {
          $.fn.zTree.init($(".shrzcx .treeDiv"), setting, data);
        })
        .fail(function (err) {
          tools.info(err);
        });
    ...
    exform: function () {
      var self = this;
      if ($("#shrzcx-table").jqGrid("getRowData").length <= 0) {
        tools.info("请先查询列表");
        return;
      }
      var params = tools.clone(self.searchData);
    ...
    showModel: function (title) {
      ...
      $(".model").show();
      $(".shrzcx .add-page-model").show();
    },
    hideModel: function () {
      $(".model").hide();
      $(".shrzcx .add-page-model").hide();
    },
    saveModel: function () {
      var self = this;
