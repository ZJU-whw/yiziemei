var dshywclEdit = require("./dshywclEdit.html");
avalon.component("dshywclEdit", {
  template: dshywclEdit,
  defaults: {
    params: {
      lcslid: "",
      shxyno: "",
    ...
    searchTimer: null,
    onReady: function () {
      $(".dshywclEdit .datepicker.date-month").datepicker({
        dateFormat: "yymm",
      });
      $(".dshywclEdit .datepicker.date-day").datepicker({
        dateFormat: "yy-mm-dd",
      });
      if (this.params.row) {
    ...
    goBack() {
      ...
      window.DSHYWCL_SEARCH && window.DSHYWCL_SEARCH();
    },
    save() {
      var title = "";
      if (this.params.title == "查看") {
    ...
    searchJsData() {
      ...
      ajax("POST", "/cxfw/cqwsb/wsh/shmx/list", params).done(function (res) {
        if (res.code == "0") {
    ...
    createTableAddCkyw: function () {
      ...
      $("#dshywclEdit-ckyw-table").jqGrid({
        ...
        // multiselect: true,
        viewrecords: true,
        pager: "#dshywclEdit-ckyw-tablePager",
        // multiselectWidth:"30",
        altRows: true,
        ...
        width: (function () {
          return $(".dshywclEdit .table-content").width();
        })(),
        height: 300,
        onPaging: function (pgButton) {
          var pageNo = tools.getPageNo(pgButton, "dshywclEdit-ckyw-table");
          self.searchCkyw(pageNo);
        },
      });
      this.htrzData.pageSize = $(
        ".ui-pg-selbox",
        $(".dshywclEdit-ckyw-table")
      ).val();
    },
    ...
    <img src="${icon}" class="${sdzt}" style="vertical-align: middle; margin-left: 5px; width: 16px; height: 16px;cursor:pointer;">
      ...
      $("#dshywclEdit-fjlb-table").jqGrid({
        datatype: "local",
        gridview: true,
        colModel: wwcColumns,
        viewrecords: true,
        rownumbers: true,
        pager: "#dshywclEdit-fjlb-tablePager",
        shrinkToFit: true,
        width: "100%",
        ...
        beforeSelectRow: function (rowid, e) {
          var row = $("#dshywclEdit-fjlb-table").jqGrid("getRowData", rowid);
        ...
        // },
        onPaging: function (pgButton) {
          var pageNo = tools.getPageNo(pgButton, "dshywclEdit-fjlb-table");
          self.search(pageNo);
        },
        gridComplete: function () {
          var ids = $("#dshywclEdit-fjlb-table").getDataIDs();
          for (var i = 0; i < ids.length; i++) {
            var rowData = $("#dshywclEdit-fjlb-table").getRowData(ids[i]);
            if (!rowData.dycs) {
              // 有效标志=N的指标记录用浅灰背景色
              $("#dshywclEdit-fjlb-table " + "#" + ids[i])
                .find("td")
    ...
    search: function (pageNo) {
      ...
      var params = {
        ...
        pageSize: $(".ui-pg-selbox", $(".dshywclEdit-fjlb-table")).val() || 20,
        pageNo: pageNo,
      };
      $("#dshywclEdit-fjlb-table").jqGrid("clearGridData");
      ajax("POST", "/cxfw/cqwsb/doc/list", params)
        .done(function (res) {
          if (res.code == "0") {
            ...
            // $("#dshywclEdit-fjlb-table").resetSelection();
            if ($("#dshywclEdit-fjlb-table")[0]) {
              $("#dshywclEdit-fjlb-table")[0].addJSONData(res.data);
            }
          } else {
            tools.info(res.msg);
          }
      ...
      ajax("POST", "/bjtssw/sbxx/doc/view", params)
        .done(function (res) {
          if (res.code == "0" && res.data) {
            $(".model").show();
            $(".dshywclEdit .page-model-pdf").show();
            var pdfSrc = res.data;
    ...
    searchCkyw(pageNo) {
      try {
        ...
        $("#dshywclEdit-ckyw-table").jqGrid("clearGridData");
        ajax("POST", "/cxfw/cqwsb/shmx/htrz/list", params)
          .done(function (res) {
            if (res.code == "0") {
              $("#dshywclEdit-ckyw-table").resetSelection();
              $("#dshywclEdit-ckyw-table")[0].addJSONData(res.data);
            } else {
              tools.info(res.msg);
            }
          })
    ...
    showModel: function () {
      $(".model").show();
      $(".dshywclEdit .hscl-page-model").show();
    },
    hideModel: function () {
      $(".model").hide();
      $(".dshywclEdit .hscl-page-model").hide();
    },
    toggleCollapse() {
    ...
    createPdf: function (url) {
      ...
      var myPDF = PDFObject.embed(url, "#sb-dshywclEdit-pdf", options);

      var el = document.querySelector("#sb-dshywclEdit-results");
      el.setAttribute("class", myPDF ? "success" : "fail");
      el.innerHTML = myPDF ? "" : "Uh-oh, the embed didn't work.";
    },
    hideModelPdf: function () {
      $(".model").hide();
      $(".dshywclEdit .page-model-pdf").hide();
    },
    isRead(params) {
      var self = this;
    ...
    showFjModel: function (title) {
      this.addTitle = title;
      $(".model").show();
      $(".dshywclEdit .add-page-model").show();
    },
    hideFjModel: function () {
      $(".model").hide();
      $(".dshywclEdit .add-page-model").hide();
      this.modelData = {
        id: "",
        nsrsbh: "",
        isRead: "",
