var shrwtj = require("./shrwtj.html");
avalon.component("shrwtj", {
  template: shrwtj,
  defaults: {
    params: {},
    act: 1,
    ...
    onReady: function () {
      ...
      $(".shrwtj .datepicker.date-day").datepicker({
        dateFormat: "yy-mm-dd",
      });
      $(".shrwtj .datepicker.date-month").datepicker({
        dateFormat: "yymm",
      });
    },
    ...
    createTable: function (arr) {
      ...
      for (var i = 0; i < arr.length; i++) {
        ...
        const regex = /\（.*?\）/
        const result = cm[i].label.replace(regex, '');
        cm[i].label = result
      }
    //   let op = {
    //     name: "op",
    ...
    //       !hasHsPermission
    //           ? ""
    //           : "</div><div class='btn sh' style='float: none;display: inline-block;' title='审核'>审核</div>";
    //       return (
    ...
    //   };
    //   cm.push(op)
    let id = { name: "id", label: "主键id", index: "id",hidden:true };
    cm.unshift(id);
      $("#shrwtj-table").jqGrid({
        datatype: "local",
        gridview: true,
        ...
        height: (function () {
          return $(".shrwtj .form").height() - 90;
        })(),
        beforeSelectRow: function (rowid, e) {
          var row = $("#shrwtj-table").jqGrid("getRowData", rowid);
        ...
        onPaging: function (pgButton) {
          var pageNo = tools.getPageNo(pgButton, "shrwtj-table");
          self.search(pageNo);
        },
        ...
        //       sumData["fahdswjgmc"] = "合计";
        //       $("#shrwtj-table").footerData("set", sumData);
        //     }, 200);
        //   } catch (error) {
        //     console.log(error);
        //   }
      ...
      $("#shrwtj-table").jqGrid("setGroupHeaders", {
        useColSpanStyle: true,
        groupHeaders: [
          {
            startColumnName: "rwfqQyhs",
            numberOfColumns: 4,
            titleText: "任务发起",
          },
          {
            startColumnName: "wclQyhs",
            numberOfColumns: 4,
            titleText: "未处理",
          },
          {
            startColumnName: "yclQyhs",
            numberOfColumns: 4,
            titleText: "已处理",
          },
          {
            startColumnName: "zsQyhs",
            numberOfColumns: 6,
            titleText: "适用征税",
          },
          {
            startColumnName: "msQyhs",
            numberOfColumns: 6,
            titleText: "适用免税",
          },
          {
            startColumnName: "ysbQyhs",
            numberOfColumns: 6,
            titleText: "已申报",
          },
          {
            startColumnName: "dsbQyhs",
            numberOfColumns: 4,
            titleText: "待申报",
          },
        ],
      ...
      //   ]
      // });
      this.searchData.pageSize = $(".ui-pg-selbox", $(".shrwtj")).val();
    },
    ...
    resetTable: function () {
      ...
      for (var i = 0; i < self.tableOption.length; i++) {
        if (self.tableOption[i].show == true) {
          $("#shrwtj-table").showCol(self.tableOption[i].name);
        } else {
          $("#shrwtj-table").hideCol(self.tableOption[i].name);
        }
      }
      $("#shrwtj-table").setGridWidth($(".shrwtj").width());
    },
    normalSearch() {
    ...
    search: function (pageNo) {
      var self = this;
      var dataValid = [
        { start: "fqsjQ", end: "fqsjZ", msg: "创建日期" },
      ...
      this.searchData.pageSize =
        $(".ui-pg-selbox", $(".shrwtj")).val() || 20;
      var params = tools.clone(self.searchData);
      params.pageNo = pageNo;
      $("#shrwtj-table").jqGrid("clearGridData");
      // ajax("POST", "/cxfw/fxgl/hdtj/mx", params)
      ajax("POST", "/cxfw/cqwsb/shmx/tj/list", params)
        .done(function (res) {
          if (res.code == "0") {
            self.tableData = res.data;
            $("#shrwtj-table").resetSelection();
            $("#shrwtj-table")[0].addJSONData(res.data);
            self.closeHyper();
          } else {
            tools.info(res.msg);
          }
    ...
    showHyper: function () {
      $(".shrwtj .select-sub").toggle();
      $(".shrwtj .select-wrapper .icon").toggleClass("active");
      if (
        $(".shrwtj .select-wrapper .icon").attr("title").slice(0, 2) ===
        "展开"
      ) {
        $(".shrwtj .select-wrapper .icon").attr("title", "收起查询条件");
      } else {
        $(".shrwtj .select-wrapper .icon").attr("title", "展开查询条件");
      }
    },
    closeHyper: function () {
      $(".shrwtj .select-sub").hide();
      $(".shrwtj .select-wrapper .icon").removeClass("active");
      $(".shrwtj .select-wrapper .icon").attr("title", "展开查询条件");
    },
    filDate: function (e) {
    ...
    initTree: function () {
      ...
      tools
        ...
        .done(function (data) {
          $.fn.zTree.init($(".shrwtj .treeDiv"), setting, data);
        })
        .fail(function (err) {
          tools.info(err);
        });
    ...
    exform: function () {
      var self = this;
      if ($("#shrwtj-table").jqGrid("getRowData").length <= 0) {
        tools.info("请先查询列表");
        return;
      }
      var params = tools.clone(self.searchData);
      ...
      form.attr("action", "/cxfw/cqwsb/shmx/tj/list/export");
      var input1 = $("<input>");
    ...
    showModel: function (title) {
      ...
      $(".model").show();
      $(".shrwtj .add-page-model").show();
    },
    hideModel: function () {
      $(".model").hide();
      $(".shrwtj .add-page-model").hide();
    },
    saveModel: function () {
      var self = this;
    ...
    rqSearchTypeChg(){
      this.searchData.fqsjQ = ''
      this.searchData.fqsjZ = ''
