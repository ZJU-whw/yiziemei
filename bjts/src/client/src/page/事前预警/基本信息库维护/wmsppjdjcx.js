var wmsppjdjcx=require("./wmsppjdjcx.html");
avalon.component('wmsppjdjcx', {
    template:wmsppjdjcx,
    defaults: {
        params:{
            type:""
        ...
        onReady:function(){
            ...
            this.getTableRow();
            $('.wmsppjdjcx .datepicker.date-day').datepicker({
                dateFormat: 'yy-mm-dd'
            });
        },
        ...
        createTable:function(arr){
            ...
            $("#wmsppjdjcx-table").jqGrid({
                datatype: "local",
                gridview: true,
                colModel: cm,
                viewrecords: true,
                rownumbers:true,
                pager: '#wmsppjdjcx-tablePager',
                shrinkToFit: true,
                width:"100%",
                autowidth:true,
                altRows: true,
                ...
                height:(function(){
                    return $(".wmsppjdjcx .form").height() -60;
                })(),
                beforeSelectRow:function(rowid,e){
                    if($(e.target).hasClass('openMx')){
                        var b = getCellData("wmsppjdjcx-table", rowid, 'taxpayerCode')
                        avalonRoot.addTab({title:"不予退税明细",component:"wmsppjdjcxMx",sameCheck:false,params:{lcslid:self.tableData.rows[rowid-1].lcslid}});
                        return false;
                ...
                onPaging:function(pgButton){
                    var pageNo=tools.getPageNo(pgButton,"wmsppjdjcx-table");
                    self.search(pageNo);
                },
            })
            this.searchData.pageSize = $(".ui-pg-selbox", $('.wmsppjdjcx')).val();
            self.search(1)
        },
        ...
        resetTable:function() {
            $("#wmsppjdjcx-table").setGridWidth($('.wmsppjdjcx').width())
        },
        search:function(pageNo){
            ...
            $("#wmsppjdjcx-table").jqGrid('clearGridData')
            ajax("POST","/bjtssw/yj/cksppjj",params).done(function(res){
                if(res.code=='0'){
                    $("#wmsppjdjcx-table").resetSelection();
                    $("#wmsppjdjcx-table")[0].addJSONData(res.data);
        ...
        initTree:function() {
            ...
            ajax("POST","/cxfw/export/readtree", {nodeType:"3"}).done(function(res){
                if(res.code=='0'){
                    $.fn.zTree.init($(".wmsppjdjcx .treeDiv"), setting, res.data);
                    $.fn.zTree.init($(".wmsppjdjcx .treeDiv2"), setting2, res.data);
                }else{
                    tools.info(res.msg)
                }
            }).fail(function(err){
        ...
        showTree:function(e){
            ...
            $('.wmsppjdjcx').on('click',function(e){
        ...
        exform:function(){
            if($('#wmsppjdjcx-table').jqGrid('getRowData').length<=0){
                tools.info("请先查询列表");
                return ;
            }
            var self=this;
