var zxjdzbcssz=require("./zxjdzbcssz.html");
avalon.component('zxjdzbcssz', {
	template:zxjdzbcssz,
	defaults: {
		params:{},
		act:1,
      ...
      yxbz:"",
		...
		createTable:function(){
			var self=this;
			var columns = [
				{ name: "op2", label:"操作", width: 100, frozen: true, align:"center", resizable: false, sortable: false,hidden:true,formatter: function(cellvalue, options, rowObject){
          ...
          if (rowObject.yxbz == 'Y') {
            ...
            return rowObject.zbCname
				...
				{ name: "op", label: "操作", width: 100, align: "center", resizable: false, search: false, sortable: false,formatter: function(cellvalue, options, rowObject){
					return "<div class='btn toMx "+(rowObject.yxbz == 'N' ? 'disabled' : '')+"' style='float: none;display: inline-block;' title='配置'>配置</div>";
				}},
			];
			if (!this.isAdmin) {
			...
			$("#zxjdzbcssz-table").jqGrid({
				datatype: "local",
				gridview: true,
				colModel: columns,
				viewrecords: true,
				rownumbers:true,
				pager: '#zxjdzbcssz-tablePager',
				shrinkToFit: false,
				autowidth:true,
				altRows: true,
				// multiselect: true,
				...
				width:"100%",
				height:(function(){
					return $(".zxjdzbcssz .form").height() -60;
				})(),
				beforeSelectRow:function(rowid,e){
					var row = $("#zxjdzbcssz-table").jqGrid("getRowData", rowid)
          ...
          if($(e.target).hasClass('toMx')){
						...
						avalonRoot.addTab({title:"专项监管指标参数详情",component:"zxjdzbcsszMx",params:params});
						return false;
            ...
            return true;
				...
				onPaging:function(pgButton){
					var pageNo=tools.getPageNo(pgButton,"zxjdzbcssz-table");
					self.search(pageNo);
				},
        gridComplete: function(){
          var ids = $("#zxjdzbcssz-table").getDataIDs();
          for (var i = 0; i<ids.length; i++) {
            var rowData = $("#zxjdzbcssz-table").getRowData(ids[i]);
			...
			if (this.isAdmin) {
				// $("#zxjdzbcssz-table").jqGrid('setFrozenColumns');
				// tools.HeiKjNoSel('zxjdzbcssz', 'zxjdzbcssz-table');
			}
			self.search(1);
		},
    showHyper:function(){
			$('.zxjdzbcssz .select-sub').toggle();
			$('.zxjdzbcssz .select-wrapper .icon').toggleClass("active");
			if ($('.zxjdzbcssz .select-wrapper .icon').attr("title").slice(0,2) === "展开") {
				$('.zxjdzbcssz .select-wrapper .icon').attr("title","收起查询条件");
			} else {
				$('.zxjdzbcssz .select-wrapper .icon').attr("title","展开查询条件")
			}
		},
    closeHyper:function(){
      $('.zxjdzbcssz .select-sub').hide();
      $('.zxjdzbcssz .select-wrapper .icon').removeClass('active');
      $('.zxjdzbcssz .select-wrapper .icon').attr("title","展开查询条件");
    },
		search:function(pageNo){
			var self=this;
			this.searchData.pageSize = $(".ui-pg-selbox", $('.zxjdzbcssz')).val() || 20;
			var params=tools.clone(self.searchData);
			params.pageNo=pageNo;
			$("#zxjdzbcssz-table").jqGrid('clearGridData')
			ajax("POST","/sszj/zxjg/zbgl/zb/list",params).done(function(res){
				if(res.code=='0'){
					$("#zxjdzbcssz-table").resetSelection();
					$("#zxjdzbcssz-table")[0].addJSONData(res.data);
					if (self.isAdmin) {
						tools.HeiKjNoSel('zxjdzbcssz', 'zxjdzbcssz-table');
					}
          self.closeHyper();
    ...
    exform:function(){
			var self=this;
			if($('#zxjdzbcssz-table').jqGrid('getRowData').length<=0){
				tools.info("请先查询列表");
				return ;
			}
			var params = tools.clone(self.searchData)
		...
		showTree:function(e){
			...
			$('.zxjdzbcssz').on('click',function(e){
