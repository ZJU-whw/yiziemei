var zxjdzbpz=require("./zxjdzbpz.html");
avalon.component('zxjdzbpz', {
	template:zxjdzbpz,
	defaults: {
		params:{},
		act:1,
    ...
    addTitle:"新增",
		...
		createTable:function(){
			var self=this;
			var columns = [
				{ name: "op2", label:"操作", width: 0, frozen: true, align:"center", resizable: false, sortable: false,hidden:true,formatter: function(cellvalue, options, rowObject){
				...
				{ name: "op", label: "操作", width: 160, align: "center", resizable: false, search: false, sortable: false,formatter: function(cellvalue, options, rowObject){
					return "<div class='btn edit' style='float: none;display: inline-block;' title='编辑'>编辑</div><div class='btn del' style='float: none;display: inline-block;' title='删除'>删除</div>";
				}},
			];
			if (!this.isAdmin) {
			...
			$("#zxjdzbpz-table").jqGrid({
				datatype: "local",
				gridview: true,
				colModel: columns,
				viewrecords: true,
				rownumbers:true,
				pager: '#zxjdzbpz-tablePager',
				shrinkToFit: false,
				autowidth:true,
				altRows: true,
				// multiselect: true,
				...
				width:"100%",
				height:(function(){
					return $(".zxjdzbpz .form").height() -60;
				})(),
				beforeSelectRow:function(rowid,e){
					var row = $("#zxjdzbpz-table").jqGrid("getRowData", rowid)
          if($(e.target).hasClass('link')){
						...
						avalonRoot.addTab({title:"指标配置详情",component:"zxjdzbpzmx",params:params});
						return false;
              ...
              var params = {
                zbId: row.zbId
							...
							ajax("POST","/sszj/zxjg/zbgl/zb/del",params).done(function(res){
								if(res.code=='0'){
									self.search(1);
								}else{
									tools.info(res.msg);
            ...
            return true;
				...
				onPaging:function(pgButton){
					var pageNo=tools.getPageNo(pgButton,"zxjdzbpz-table");
					self.search(pageNo);
				}
			});
			
			// $("#zxjdzbpz-table").jqGrid('setFrozenColumns');
			if (this.isAdmin) {
				// tools.HeiKjNoSel('zxjdzbpz', 'zxjdzbpz-table');
			}
      ...
      this.addTitle = operation == '1' ? '新增' : '编辑'
			...
			$('.model').show();
			$('.zxjdzbpz .add-page-model').show();
		},
		hideModel: function(){
			$('.model').hide();
			$('.zxjdzbpz .add-page-model').hide();
      this.modelData = {
        zbId: '',
        zbCname: '',
        zbSname: '',
    ...
    showHyper:function(){
			$('.zxjdzbpz .select-sub').toggle();
			$('.zxjdzbpz .select-wrapper .icon').toggleClass("active");
			if ($('.zxjdzbpz .select-wrapper .icon').attr("title").slice(0,2) === "展开") {
				$('.zxjdzbpz .select-wrapper .icon').attr("title","收起查询条件");
			} else {
				$('.zxjdzbpz .select-wrapper .icon').attr("title","展开查询条件")
			}
		},
    closeHyper:function(){
      $('.zxjdzbpz .select-sub').hide();
      $('.zxjdzbpz .select-wrapper .icon').removeClass('active');
      $('.zxjdzbpz .select-wrapper .icon').attr("title","展开查询条件");
    },
		search:function(pageNo){
			var self=this;
			this.searchData.pageSize = $(".ui-pg-selbox", $('.zxjdzbpz')).val() || 20;
			var params=tools.clone(self.searchData);
			params.pageNo=pageNo;
			$("#zxjdzbpz-table").jqGrid('clearGridData')
			ajax("POST","/sszj/zxjg/zbgl/zb/list",params).done(function(res){
				if(res.code=='0'){
					$("#zxjdzbpz-table").resetSelection();
					$("#zxjdzbpz-table")[0].addJSONData(res.data);
					if (self.isAdmin) {
						tools.HeiKjNoSel('zxjdzbpz', 'zxjdzbpz-table');
					}
          self.closeHyper();
    ...
    exform:function(){
			var self=this;
			if($('#zxjdzbpz-table').jqGrid('getRowData').length<=0){
				tools.info("请先查询列表");
				return ;
			}
			var params = tools.clone(self.searchData)
			...
			form.attr("action", "/sszj/export/zxjg/zbgl/zb");
			var input1 = $("<input>");
    ...
    saveModel: function(){
      ...
      ajax("POST","/sszj/zxjg/zbgl/zb/update",params).done(function(res){
				if(res.code=='0'){
					tools.info('保存成功！');
          self.hideModel();
          self.search(1);
		...
		getYwfl: function(){
			var self = this
			ajax("POST","/sszj/zxjg/zbgl/zb/getYwfl",{}).done(function(res){
				if(res.code=='0'){
