var sdjcsjtj = require("./sdjcsjtj.html");
avalon.component("sdjcsjtj", {
  template: sdjcsjtj,
  defaults: {
    params: {},
    act: 1,
    ...
    onReady: function () {
      ...
      $(".sdjcsjtj .datepicker.date-day").datepicker({
        dateFormat: "yy-mm-dd",
      });
      $(".sdjcsjtj .datepicker.date-month").datepicker({
        dateFormat: "yymm",
      });
    },
    ...
    createTable: function (arr) {
      ...
      var wwcColumns = [
        ...
        { name: "swjgjc", label: "税务机关简称", index: "swjgjc" },
        {
          ...
          formatter: function (cellvalue, options, rowObject) {
            return (
              "<span style='color:#0000ff;text-decoration: underline;cursor:pointer;' class='hcsl'>" +
              cellvalue +
              "</span>"
            );
          },
        },
        {
          ...
          formatter: function (cellvalue, options, rowObject) {
            return (
              "<span style='color:#0000ff;text-decoration: underline;cursor:pointer;' class='hczsl'>" +
              cellvalue +
              "</span>"
            );
          },
        },
        {
          ...
          formatter: function (cellvalue, options, rowObject) {
            return (
              "<span style='color:#0000ff;text-decoration: underline;cursor:pointer;' class='hcywtsl'>" +
              cellvalue +
              "</span>"
            );
          },
        },
        {
          ...
          formatter: function (cellvalue, options, rowObject) {
            return (
              "<span style='color:#0000ff;text-decoration: underline;cursor:pointer;' class='hcwwtsl'>" +
              cellvalue +
              "</span>"
            );
          },
      ...
      $("#sdjcsjtj-table").jqGrid({
        datatype: "local",
        gridview: true,
        colModel: wwcColumns,
        viewrecords: true,
        rownumbers: true,
        pager: "#sdjcsjtj-tablePager",
        shrinkToFit: false,
        width: "100%",
        // multiselect: true,
        // multiselectWidth:"30",
        ...
        height: (function () {
          return $(".sdjcsjtj .form").height() - 70;
        })(),
        beforeSelectRow: function (rowid, e) {
          var row = $("#sdjcsjtj-table").jqGrid("getRowData", rowid);
          var obj = self.tableData.rows[rowid - 1];
          if ($(e.target).hasClass("hcsl")) {
            avalonRoot.addTab({
              title: "实地核查数据统计明细",
            ...
            return false;
          } else if ($(e.target).hasClass("hczsl")) {
            avalonRoot.addTab({
              title: "实地核查数据统计明细",
              component: "sdjcsjtjMx",
              sameCheck: true,
            ...
            return false;
          } else if ($(e.target).hasClass("hcywtsl")) {
            avalonRoot.addTab({
              title: "实地核查数据统计明细",
              component: "sdjcsjtjMx",
              sameCheck: true,
            ...
            return false;
          } else if ($(e.target).hasClass("hcwwtsl")) {
            avalonRoot.addTab({
              title: "实地核查数据统计明细",
              component: "sdjcsjtjMx",
              sameCheck: true,
        ...
        onPaging: function (pgButton) {
          var pageNo = tools.getPageNo(pgButton, "sdjcsjtj-table");
          self.search(pageNo);
        },
      });
      this.searchData.pageSize = $(".ui-pg-selbox", $(".sdjcsjtj")).val();
    },
    ...
    resetTable: function () {
      ...
      for (var i = 0; i < self.tableOption.length; i++) {
        if (self.tableOption[i].show == true) {
          $("#sdjcsjtj-table").showCol(self.tableOption[i].name);
        } else {
          $("#sdjcsjtj-table").hideCol(self.tableOption[i].name);
        }
      }
      $("#sdjcsjtj-table").setGridWidth($(".sdjcsjtj").width());
    },
    normalSearch() {
    ...
    search: function (pageNo) {
      ...
      this.searchData.pageSize = $(".ui-pg-selbox", $(".sdjcsjtj")).val() || 20;
      var params = tools.clone(self.searchData);
      params.pageNo = pageNo;
      $("#sdjcsjtj-table").jqGrid("clearGridData");
      ajax("POST", "/cxfw/fxgl/sdhctj", params)
        .done(function (res) {
          if (res.code == "0") {
            self.tableData = res.data;
            $("#sdjcsjtj-table").resetSelection();
            $("#sdjcsjtj-table")[0].addJSONData(res.data);
            self.closeHyper();
            if(self.swjgList.indexOf(params.swjgdm)==-1){
              $("#sdjcsjtj-table").jqGrid('setLabel', 'swjgdm', '企业税号');
              $("#sdjcsjtj-table").jqGrid('setLabel', 'swjgjc', '企业名称');
            }
          } else {
            tools.info(res.msg);
          }
    ...
    showHyper: function () {
      $(".sdjcsjtj .select-sub").toggle();
      $(".sdjcsjtj .select-wrapper .icon").toggleClass("active");
      if (
        $(".sdjcsjtj .select-wrapper .icon").attr("title").slice(0, 2) ===
        "展开"
      ) {
        $(".sdjcsjtj .select-wrapper .icon").attr("title", "收起查询条件");
      } else {
        $(".sdjcsjtj .select-wrapper .icon").attr("title", "展开查询条件");
      }
    },
    closeHyper: function () {
      $(".sdjcsjtj .select-sub").hide();
      $(".sdjcsjtj .select-wrapper .icon").removeClass("active");
      $(".sdjcsjtj .select-wrapper .icon").attr("title", "展开查询条件");
    },
    filDate: function (e) {
    ...
    initTree: function () {
      ...
      tools
        ...
        .done(function (data) {
          $.fn.zTree.init($(".sdjcsjtj .treeDiv"), setting, data);
        })
        .fail(function (err) {
          tools.info(err);
        });
    ...
    exform: function () {
      var self = this;
      if ($("#sdjcsjtj-table").jqGrid("getRowData").length <= 0) {
        tools.info("请先查询列表");
        return;
      }
      var params = tools.clone(self.searchData);
      ...
      form.attr("action", "/cxfw/fxgl/sdhctj/export");
      var input1 = $("<input>");
    ...
    showModel: function (title) {
      ...
      $(".model").show();
      $(".sdjcsjtj .add-page-model").show();
    },
    hideModel: function () {
      $(".model").hide();
      $(".sdjcsjtj .add-page-model").hide();
    },
    saveModel: function () {
      var self = this;
