var spfxghwh = require("./spfxghwh.html");
avalon.component("spfxghwh", {
  template: spfxghwh,
  defaults: {
    params: {},
    act: 1,
    ...
    onReady: function () {
      ...
      // this.normalSearch()
      // self.initTree();
      $(".spfxghwh .datepicker.date-day").datepicker({
        dateFormat: "yy-mm-dd",
      });
      $(".spfxghwh .datepicker.date-month").datepicker({
        dateFormat: "yymm",
      });
    },
    ...
    createTable: function (arr) {
      ...
      let op = {
        ...
        formatter: function (cellvalue, options, rowObject) {
          ...
          if(self.activeIndex == '0'){
            returnBtn = "<div class='btn edit' style='float: none;display: inline-block;' title='编辑'>编辑</div><div class='btn del' style='float: none;display: inline-block;' title='删除'>删除</div>"
          }else{
            returnBtn = "<div class='btn edit' style='float: none;display: inline-block;' title='查看'>查看</div>"
          }
          return (
            returnBtn
          );
      ...
      cm.push(op);
      $("#spfxghwh-table").jqGrid({
        datatype: "local",
        gridview: true,
        colModel: cm,
        viewrecords: true,
        rownumbers: true,
        pager: "#spfxghwh-tablePager",
        shrinkToFit: false,
        width: "100%",
        // multiselect: true,
        // multiselectWidth:"30",
        ...
        height: (function () {
          return $(".spfxghwh .form").height() - 120;
        })(),
        beforeSelectRow: function (rowid, e) {
          // if($(e.target).hasClass('disabled')) return false;
          var row = $("#spfxghwh-table").jqGrid("getRowData", rowid);
          if ($(e.target).hasClass("openMx")) {
            ...
            let gzFwDataList = [
              {value:'1',label:'发票电子信息'},
              {value:'2',label:'申报进货明细'},
              {value:'3',label:'同时适用'},
            ]
            ...
            tools.confirm("是否确定删除该条数据？", "确定", function () {
              ...
              ajax("POST", "/cxfw/fxgl/fxgys/del", params)
                .done(function (res) {
                  if (res.code == "0") {
                    self.search(1);
                  } else {
        ...
        onPaging: function (pgButton) {
          var pageNo = tools.getPageNo(pgButton, "spfxghwh-table");
          self.search(pageNo);
        },
      });
      this.searchData.pageSize = $(".ui-pg-selbox", $(".spfxghwh")).val();
    },
    ...
    resetTable: function () {
      ...
      for (var i = 0; i < self.tableOption.length; i++) {
        if (self.tableOption[i].show == true) {
          $("#spfxghwh-table").showCol(self.tableOption[i].name);
        } else {
          $("#spfxghwh-table").hideCol(self.tableOption[i].name);
        }
      }
      $("#spfxghwh-table").setGridWidth($(".spfxghwh").width());
    },
    normalSearch() {
    ...
    search: function (pageNo) {
      var self = this;
      this.searchData.pageSize = $(".ui-pg-selbox", $(".spfxghwh")).val() || 20;
      var params = tools.clone(self.searchData);
      params.pageNo = pageNo;
      params.fwlx = self.searchData.fwlx
      let url;
      if (self.activeIndex == "0") {
        url = "/cxfw/fxgl/fxgyslist";
      } else if (self.activeIndex == "1") {
        url = "/cxfw/fxgl/fxgyslist4Other";
      }
      self.url = url
      $("#spfxghwh-table").jqGrid("clearGridData");
      ajax("POST", url, params)
        .done(function (res) {
          if (res.code == "0") {
            self.tableData = res.data;
            $("#spfxghwh-table").resetSelection();
            $("#spfxghwh-table")[0].addJSONData(res.data);
            self.closeHyper();
          } else {
            tools.info(res.msg);
          }
    ...
    showHyper: function () {
      $(".spfxghwh .select-sub").toggle();
      $(".spfxghwh .select-wrapper .icon").toggleClass("active");
      if (
        $(".spfxghwh .select-wrapper .icon").attr("title").slice(0, 2) ===
        "展开"
      ) {
        $(".spfxghwh .select-wrapper .icon").attr("title", "收起查询条件");
      } else {
        $(".spfxghwh .select-wrapper .icon").attr("title", "展开查询条件");
      }
    },
    closeHyper: function () {
      $(".spfxghwh .select-sub").hide();
      $(".spfxghwh .select-wrapper .icon").removeClass("active");
      $(".spfxghwh .select-wrapper .icon").attr("title", "展开查询条件");
    },
    filDate: function (e) {
    ...
    initTree: function () {
      ...
      tools
        ...
        .done(function (data) {
          $.fn.zTree.init($(".spfxghwh .treeDiv"), setting, data);
        })
        .fail(function (err) {
          tools.info(err);
        });
    ...
    exform: function () {
      var self = this;
      if ($("#spfxghwh-table").jqGrid("getRowData").length <= 0) {
        tools.info("请先查询列表");
        return;
      }
      var params = tools.clone(self.searchData);
      ...
      form.attr("action", "/cxfw/fxgl/fxgyslist/export");
      var input1 = $("<input>");
    ...
    showModel: function (title) {
      ...
      $(".model").show();
      $(".spfxghwh .add-page-model").show();
    },
    hideModel: function () {
      $(".model").hide();
      $(".spfxghwh .add-page-model").hide();
    },
    saveModel: function () {
      var self = this;
      if (!/^\d+(\.\d{1,2})?$/.test(this.modelData.gzDjq)&&this.modelData.gzDjq) {
        tools.info("请输入正确的单价范围起");
        return;
      }
      if (!/^\d+(\.\d{1,2})?$/.test(this.modelData.gzDjz)&&this.modelData.gzDjz) {
        tools.info("请输入正确的单价范围止");
        return;
      }
      var valid = this.checkValid(this.modelData);
      if (!valid) return;
      api.updateFxgys(this.modelData).done(function (res) {
        if (res.code == "0") {
          tools.info("保存成功！");
          self.hideModel();
          self.search(1);
