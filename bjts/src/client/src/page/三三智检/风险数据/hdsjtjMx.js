var hdsjtjMx = require("./hdsjtjMx.html");
avalon.component("hdsjtjMx", {
  template: hdsjtjMx,
  defaults: {
    params: {},
    act: 1,
    ...
    onReady: function () {
      ...
      $(".hdsjtjMx .datepicker.date-day").datepicker({
        dateFormat: "yy-mm-dd",
      });
      $(".hdsjtjMx .datepicker.date-month").datepicker({
        dateFormat: "yymm",
      });
    },
    ...
    createTable: function (arr) {
      ...
      $("#hdsjtjMx-table").jqGrid({
        datatype: "local",
        gridview: true,
        colModel: cm,
        viewrecords: true,
        rownumbers: true,
        pager: "#hdsjtjMx-tablePager",
        shrinkToFit: false,
        width: "100%",
        // multiselect: true,
        // multiselectWidth:"30",
        ...
        height: (function () {
          return $(".hdsjtjMx .form").height() - 70;
        })(),
        beforeSelectRow: function (rowid, e) {
          var row = $("#hdsjtjMx-table").jqGrid("getRowData", rowid);
        ...
        onPaging: function (pgButton) {
          var pageNo = tools.getPageNo(pgButton, "hdsjtjMx-table");
          self.search(pageNo);
        },
        gridComplete: function () {
          try {
            setTimeout(() => {
              ...
              $("#hdsjtjMx-table").footerData("set", sumData);
            }, 200);
          } catch (error) {
            console.log(error);
          }
      ...
      this.searchData.pageSize = $(".ui-pg-selbox", $(".hdsjtjMx")).val();
    },
    ...
    resetTable: function () {
      ...
      for (var i = 0; i < self.tableOption.length; i++) {
        if (self.tableOption[i].show == true) {
          $("#hdsjtjMx-table").showCol(self.tableOption[i].name);
        } else {
          $("#hdsjtjMx-table").hideCol(self.tableOption[i].name);
        }
      }
      $("#hdsjtjMx-table").setGridWidth($(".hdsjtjMx").width());
    },
    normalSearch() {
    ...
    search: function (pageNo) {
      ...
      this.searchData.pageSize =
        $(".ui-pg-selbox", $(".hdsjtjMx")).val() || 20;
      var params = tools.clone(self.searchData);
      params.pageNo = pageNo;
      $("#hdsjtjMx-table").jqGrid("clearGridData");
      ajax("POST", "/cxfw/fxgl/hdtj/mx", params)
        .done(function (res) {
          if (res.code == "0") {
            self.tableData = res.data;
            $("#hdsjtjMx-table").resetSelection();
            $("#hdsjtjMx-table")[0].addJSONData(res.data);
            self.closeHyper();
    ...
    showHyper: function () {
      $(".hdsjtjMx .select-sub").toggle();
      $(".hdsjtjMx .select-wrapper .icon").toggleClass("active");
      if (
        $(".hdsjtjMx .select-wrapper .icon").attr("title").slice(0, 2) ===
        "展开"
      ) {
        $(".hdsjtjMx .select-wrapper .icon").attr("title", "收起查询条件");
      } else {
        $(".hdsjtjMx .select-wrapper .icon").attr("title", "展开查询条件");
      }
    },
    closeHyper: function () {
      $(".hdsjtjMx .select-sub").hide();
      $(".hdsjtjMx .select-wrapper .icon").removeClass("active");
      $(".hdsjtjMx .select-wrapper .icon").attr("title", "展开查询条件");
    },
    filDate: function (e) {
    ...
    initTree: function () {
      ...
      tools
        ...
        .done(function (data) {
          $.fn.zTree.init($(".hdsjtjMx .treeDiv"), setting, data);
        })
        .fail(function (err) {
          tools.info(err);
        });
    ...
    exform: function () {
      var self = this;
      if ($("#hdsjtjMx-table").jqGrid("getRowData").length <= 0) {
        tools.info("请先查询列表");
        return;
      }
      var params = tools.clone(self.searchData);
      ...
      form.attr("action", "/cxfw/fxgl/hdtj/mx/export");
      var input1 = $("<input>");
    ...
    showModel: function (title) {
      ...
      $(".model").show();
      $(".hdsjtjMx .add-page-model").show();
    },
    hideModel: function () {
      $(".model").hide();
      $(".hdsjtjMx .add-page-model").hide();
    },
    saveModel: function () {
      var self = this;
