var fxsjtj = require("./fxsjtj.html");
avalon.component("fxsjtj", {
  template: fxsjtj,
  defaults: {
    params: {},
    act: 1,
    ...
    onReady: function () {
      ...
      if (self.params.swjgMc) {
        ...
        this.hasHsPermission =
          this.swjgList.indexOf(self.params.swjgdm) == -1;
      }else{
      ...
      $(".fxsjtj .datepicker.date-day").datepicker({
        dateFormat: "yy-mm-dd",
      });
      $(".fxsjtj .datepicker.date-month").datepicker({
        dateFormat: "yymm",
      });
    },
    ...
    createTable: function (arr) {
      ...
      var wwcColumns = [
        ...
        {
          name: "fxdzxx",
          label: "风险出口电子信息（户数/条数）",
          index: "fxdzxx",
          align: "center",
          width:'180',
          formatter: function (cellvalue, options, rowObject) {
            return (
              "<span style='color:#0000ff;text-decoration: underline;cursor:pointer;' class='fxdzxx'>" +
              cellvalue +
              "</span>"
            );
          },
        ...
        {
          name: "fxdzsbxx",
          label: "风险出口申报信息（户数/条数）",
          index: "fxdzsbxx",
          align: "center",
          width:'180',
          formatter: function (cellvalue, options, rowObject) {
            return (
              "<span style='color:#0000ff;text-decoration: underline;cursor:pointer;' class='fxdzsbxx'>" +
              cellvalue +
              "</span>"
            );
          },
        ...
        {
          name: "fxghxx",
          label: "风险供货电子信息（户数/条数）",
          index: "fxghxx",
          align: "center",
          width:'180',
          formatter: function (cellvalue, options, rowObject) {
            return (
              "<span style='color:#0000ff;text-decoration: underline;cursor:pointer;' class='fxghxx'>" +
              cellvalue +
              "</span>"
            );
          },
        ...
        {
          name: "fxghsbxx",
          label: "风险供货申报信息（户数/条数）",
          index: "fxghsbxx",
          align: "center",
          width:'180',
          formatter: function (cellvalue, options, rowObject) {
            return (
              "<span style='color:#0000ff;text-decoration: underline;cursor:pointer;' class='fxghsbxx'>" +
              cellvalue +
              "</span>"
            );
          },
      ...
      var newCm = [
        { name: "swjgdm", label: "企业社会信用代码", index: "swjgdm" },
        { name: "swjgjc", label: "企业名称", index: "swjgjc" },
        {
          name: "fxdzxx",
          label: "风险出口电子信息",
          index: "fxdzxx",
      ...
      $("#fxsjtj-table").jqGrid({
        datatype: "local",
        gridview: true,
        colModel: self.hasHsPermission?newCm:wwcColumns,
        viewrecords: true,
        rownumbers: true,
        pager: "#fxsjtj-tablePager",
        shrinkToFit: false,
        width: "100%",
        // multiselect: true,
        // multiselectWidth:"30",
        ...
        height: (function () {
          return $(".fxsjtj .form").height() - 70;
        })(),
        beforeSelectRow: function (rowid, e) {
          var row = $("#fxsjtj-table").jqGrid("getRowData", rowid);
          var obj = self.tableData.rows[rowid - 1];
          if ($(e.target).hasClass("fxdzxx")) {
            console.log(self.searchData.endDate);
            console.log(tools.getMonthLast('2024-08-01'));
            avalonRoot.addTab({
              title: "事前预警信息综合查询",
              component: "yjxxzhcx",
              sameCheck: true,
            ...
            return false;
          } else if ($(e.target).hasClass("fxdzsbxx")) {
            avalonRoot.addTab({
              title: "事前预警信息综合查询",
              component: "yjxxzhcx",
              sameCheck: true,
            ...
            return false;
          } else if ($(e.target).hasClass("fxghxx")) {
            avalonRoot.addTab({
              title: "事前预警信息综合查询",
              component: "yjxxzhcx",
              sameCheck: true,
            ...
            return false;
          } else if ($(e.target).hasClass("fxghsbxx")) {
            avalonRoot.addTab({
              title: "事前预警信息综合查询",
              component: "yjxxzhcx",
              sameCheck: true,
        ...
        onPaging: function (pgButton) {
          var pageNo = tools.getPageNo(pgButton, "fxsjtj-table");
          self.search(pageNo);
        },
      });
      this.searchData.pageSize = $(".ui-pg-selbox", $(".fxsjtj")).val();
    },
    ...
    resetTable: function () {
      ...
      for (var i = 0; i < self.tableOption.length; i++) {
        if (self.tableOption[i].show == true) {
          $("#fxsjtj-table").showCol(self.tableOption[i].name);
        } else {
          $("#fxsjtj-table").hideCol(self.tableOption[i].name);
        }
      }
      $("#fxsjtj-table").setGridWidth($(".fxsjtj").width());
    },
    normalSearch() {
    ...
    search: function (pageNo) {
      ...
      this.searchData.pageSize = $(".ui-pg-selbox", $(".fxsjtj")).val() || 20;
      var params = tools.clone(self.searchData);
      params.pageNo = pageNo;
      $("#fxsjtj-table").jqGrid("clearGridData");
      ajax("POST", "/cxfw/fxgl/fxtj", params)
        .done(function (res) {
          if (res.code == "0") {
            self.tableData = res.data;
            $("#fxsjtj-table").resetSelection();
            $("#fxsjtj-table")[0].addJSONData(res.data);
            self.closeHyper();
            if(self.swjgList.indexOf(params.swjgdm)==-1){
              $("#fxsjtj-table").jqGrid('setLabel', 'swjgdm', '企业税号');
              $("#fxsjtj-table").jqGrid('setLabel', 'swjgjc', '企业名称');
            }
          } else {
            tools.info(res.msg);
          }
    ...
    showHyper: function () {
      $(".fxsjtj .select-sub").toggle();
      $(".fxsjtj .select-wrapper .icon").toggleClass("active");
      if (
        $(".fxsjtj .select-wrapper .icon").attr("title").slice(0, 2) === "展开"
      ) {
        $(".fxsjtj .select-wrapper .icon").attr("title", "收起查询条件");
      } else {
        $(".fxsjtj .select-wrapper .icon").attr("title", "展开查询条件");
      }
    },
    closeHyper: function () {
      $(".fxsjtj .select-sub").hide();
      $(".fxsjtj .select-wrapper .icon").removeClass("active");
      $(".fxsjtj .select-wrapper .icon").attr("title", "展开查询条件");
    },
    filDate: function (e) {
    ...
    initTree: function () {
      ...
      tools
        ...
        .done(function (data) {
          $.fn.zTree.init($(".fxsjtj .treeDiv"), setting, data);
        })
        .fail(function (err) {
          tools.info(err);
        });
    ...
    exform: function () {
      var self = this;
      if ($("#fxsjtj-table").jqGrid("getRowData").length <= 0) {
        tools.info("请先查询列表");
        return;
      }
      var params = tools.clone(self.searchData);
      ...
      form.attr("action", "/cxfw/fxgl/fxtj/export");
      var input1 = $("<input>");
    ...
    showModel: function (title) {
      ...
      $(".model").show();
      $(".fxsjtj .add-page-model").show();
    },
    hideModel: function () {
      $(".model").hide();
      $(".fxsjtj .add-page-model").hide();
    },
    saveModel: function () {
      var self = this;
