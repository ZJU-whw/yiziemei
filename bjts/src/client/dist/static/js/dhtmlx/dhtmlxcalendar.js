function dhtmlXCalendarObject(inps, skin, options) {
	this.inputObj = inps;
	// parse inputs
	this.i = {};
	...
	li.innerHTML = "<div class='dhtmlxcalendar_month_arrow dhtmlxcalendar_month_arrow_left' onmouseover='this.className=\"dhtmlxcalendar_month_arrow dhtmlxcalendar_month_arrow_left_hover\";' onmouseout='this.className=\"dhtmlxcalendar_month_arrow dhtmlxcalendar_month_arrow_left\";'></div>"+
			"<span class='dhtmlxcalendar_month_label_year'><input style='width:28px; height:12px; text-align:center; margin-right:3px;' class='dhtmlxcalendar_month_label_year' />年</span><span class='dhtmlxcalendar_month_label_month'><input style='width:12px; height:12px; text-align:center; margin-right:3px; margin-left:5px;' class='dhtmlxcalendar_month_label_month' />月</span>"+
	...
	li.onclick = function(e) {
		...
		// show month selector
		if (t.className && t.className == "dhtmlxcalendar_month_label_month" && $(t).prop("readonly") == false) {
			e.cancelBubble = true;
			
			var selLeft = 65 + ($(that.contMonth).width() - 182) / 2;
			that._showSelector("month",selLeft,21,"selector_month",true);
			return;
		}
		// show year selector
		if (t.className && t.className == "dhtmlxcalendar_month_label_year" && $(t).prop("readonly") == false) {
			e.cancelBubble = true;
			var selLeft = 1 + ($(that.contMonth).width() - 182) / 2;
			that._showSelector("year",selLeft,21,"selector_year",true);
			return;
		}
	...
	//输入年份时的处理
	$(li).children(".dhtmlxcalendar_month_label_year").children("input").change(function() {
		var yearVal = $.trim($(this).val());
		if(yearVal.length == 0) {
			that.callEvent("onChange",[null,true]);
			return;
		}
		...
		that._hideSelector();
		that._drawMonth(new Date(year, that._activeMonth.getMonth(), 1, 0, 0, 0, 0));
		that._doOnSelectorChange();
	});

	//输入月份时的处理
	$(li).children(".dhtmlxcalendar_month_label_month").children("input").change(function() {
		var monthVal = $.trim($(this).val());
		...
		that._drawMonth(new Date(that._activeMonth.getFullYear(), month, 1, 0, 0, 0, 0));
		that._doOnSelectorChange();
	});
	...
	this.contDates.onclick = function(e){
		...
		if (t._date != null && !t._css_dis) {
			...
			var t3 = that._activeDate.getSeconds();
			
			// cjeck if allow to modify input
			if (that.checkEvent("onBeforeChange")) {
				if (!that.callEvent("onBeforeChange",[new Date(t._date.getFullYear(),t._date.getMonth(),t._date.getDate(),t1,t2,t3)])) return;
			}
			...
			that._activeDate = new Date(t._date.getFullYear(),t._date.getMonth(),t._date.getDate(),t1,t2,t3);
	...
	this.hideDate = function() {
		...
		if($(this.inputObj).prop("tagName") != "INPUT") {
			this.base.style.backgroundImage = "none";
			
			//隐藏年月的左右箭头
			$("ul li div", this.contMonth).hide();
			
			$(this.base).width(116);
			
			//修改年、月头的属性
			$(this.contMonth).width(110);
			$("ul", this.contMonth).width(110);
			$("ul li", this.contMonth).width(110);
			$("ul li", this.contMonth).css("background-image", "none");
			
			//修改时、分、秒的属性
			$(this.contTime).width(110);
			$("ul", this.contTime).width(110);
			$("ul li", this.contTime).width(110);
			$("ul li", this.contTime).css("background-image", "none");
			$("ul li .dhtmlxcalendar_time_label", this.contTime).hide(); 
		}
	}
	
	this.hideMonth = function() {
	...
	this._isShowSecond = options && options.isShowSecond === true ? true : false; //是否显示秒
	this._isMsStepFive = options && options.isMsStepFive === false ? false : true; //分和秒选择器是否按5增长

	var li = document.createElement("LI");
	li.className = "dhtmlxcalendar_cell dhtmlxcalendar_time_hdr";
	var inhtml = "<div class='dhtmlxcalendar_time_label'></div><input style='width:12px; height:12px; text-align:center; margin-right:2px; margin-left:2px;' class='dhtmlxcalendar_label_hours'><span class='dhtmlxcalendar_label_colon'>:</span><input style='width:12px; height:12px; text-align:center; margin-right:2px; margin-left:2px;' class='dhtmlxcalendar_label_minutes'><span class='dhtmlxcalendar_label_colon'";
	if(!this._isShowSecond) { //不显示秒
		inhtml += " style='display:none;'";
	}
	inhtml += ">:</span><input style='width:12px; height:12px; text-align:center; margin-right:2px; margin-left:2px;";
	if(!this._isShowSecond) { //不显示秒
		inhtml += "display:none;";
	}
	inhtml += "' class='dhtmlxcalendar_label_seconds'>";
//alert(inhtml);	
	li.innerHTML = inhtml;
	ul.appendChild(li);
	...
	li.onclick = function(e) {
		...
		var pos = $(t).position();
		var height = $(t).height();
		var baseWidth = $(that.base).width();
		var baseHeight = $(that.base).height();
		var selWidth; // = $(that._sel).width();
		var selHeight; // = $(that._sel).height();
		var selTop, selLeft;
	//alert("" + baseHeight + "," + pos.top + "," + selWidth + "," + selHeight);
		// show hours selector
		if (t.className && t.className == "dhtmlxcalendar_label_hours" && $(t).prop("readonly") == false) {
			e.cancelBubble = true;
			selWidth = 165
			selHeight = 79;
			selLeft = pos.left - selWidth / 2 + 13;
			if(baseHeight > 50) {
				selTop = pos.top - selHeight + baseHeight - height - 12;
			}
			else {
				selTop = pos.top + height;
			}
			
			that._showSelector("hours", selLeft, selTop,"selector_hours",true);
			return;
		}
		// show minutes selector
		if (t.className && t.className == "dhtmlxcalendar_label_minutes" && $(t).prop("readonly") == false) {
			e.cancelBubble = true;
			if(that._isMsStepFive) {
				selWidth = 81
				selHeight = 80;
			}
			else {
			...
			that._showSelector("minutes", selLeft, selTop,"selector_minutes",true);
			return;
		}
		// show seconds selector
		if (t.className && t.className == "dhtmlxcalendar_label_seconds" && $(t).prop("readonly") == false) {
			e.cancelBubble = true;
			if(that._isMsStepFive) {
				selWidth = 81
				selHeight = 80;
			...
			that._showSelector("seconds",selLeft, selTop,"selector_seconds",true);
			return;
		}
		// hide selector if it visible
		that._hideSelector();
	...
	//输入小时的处理
	$(li).children("input.dhtmlxcalendar_label_hours").change(function() {
		var v = $.trim($(this).val());
		if(v.length == 0) {
			that.callEvent("onChange",[null,true]);
			return;
		...
		var iV = parseInt(v);
		if(isNaN(iV) || iV < 0 || iV > 23) {
			iV = 0;
		}
		
		that._hideSelector();
		that._activeDate.setHours(iV);
	...
	//输入分钟的处理
	$(li).children("input.dhtmlxcalendar_label_minutes").change(function() {
		var v = $.trim($(this).val());
		if(v.length == 0) {
			that.callEvent("onChange",[null,true]);
			return;
		...
		var iV = parseInt(v);
		if(isNaN(iV) || iV < 0 || iV > 59) {
			iV = 0;
		}
		
		that._hideSelector();
		that._activeDate.setMinutes(iV);
	...
	//输入秒的处理
	$(li).children("input.dhtmlxcalendar_label_seconds").change(function() {
		var v = $.trim($(this).val());
		if(v.length == 0) {
			that.callEvent("onChange",[null,true]);
			return;
		...
		that._hideSelector();
		that._activeDate.setSeconds(iV);
		that._updateActiveSeconds();
		that._updateVisibleSeconds();
		that._doOnSelectorChange();
	});
	...
	this._drawMonth = function(d) {
		...
		this.contMonth.firstChild.firstChild.childNodes[2].childNodes[0].value = this.langData[this.lang].monthesFNames[d.getMonth()];
		this.contMonth.firstChild.firstChild.childNodes[1].childNodes[0].value = d.getFullYear();
	...
	this._initSelector = function(type,css) {
		...
		//初始化分、秒。按照分、秒设置的步长，生成选择器
		var step = this._isMsStepFive ? 5 : 1;
		var rows =  this._isMsStepFive ? 4 : 6;
		var cols =  this._isMsStepFive ? 3 : 10;
		// init minutes
		if (type == "minutes") {
			...
			for (var q=0; q<rows; q++) {
				var ul = document.createElement("UL");
				ul.className = "dhtmlxcalendar_selector_line";
				ul.style.width = this._isMsStepFive ? "81px" : "270px";
		...
		if (type == "seconds") {
			...
			this.rsCont.onclick = function(e) {
				...
				if (t._seconds != null) {
					that._hideSelector();
					that._activeDate.setSeconds(t._seconds);
					that._updateActiveSeconds();
					that._updateVisibleSeconds();
					that._doOnSelectorChange();
				}
	...
	this._showSelector = function(type,x,y,css,autoHide) {
		...
		//	alert(y);
		//$(this._sel).offset({top: y, left: x});
		this._sel.style.display = "";
		//this._sel.className = "dhtmlxcalendar_selector_obj dhtmlxcalendar_"+css;
		if((type == "hours" || type == "minutes" || type == "seconds") && $(this.base).height() > 50) {
			this._sel.className = "dhtmlxcalendar_selector_obj dhtmlxcalendar_"+css + " dhtmlxcalendar_selector_above";
		}
		else {
			this._sel.className = "dhtmlxcalendar_selector_obj dhtmlxcalendar_"+css;
		}
	...
	this._doOnSelectorShow = function(type) {
		...
		if (type == "seconds") this._updateActiveSeconds();
	}
	...
	this._updateVisibleHours = function() {
		this.contTime.firstChild.firstChild.childNodes[1].value = this._fixLength(this._activeDate.getHours(),2);
	}
	...
	this._updateVisibleMinutes = function() {
		this.contTime.firstChild.firstChild.childNodes[3].value = this._fixLength(this._activeDate.getMinutes(),2);
	}
	
	/* seconds selector */
	
	// update seconds in selector
	this._updateActiveSeconds = function() {
		if (typeof(this._rsActive) != "undefined" && typeof(this._rsCells[this._rsActive]) != "undefined") this._rsCells[this._rsActive].className = "dhtmlxcalendar_selector_cell";
		this._rsActive = this._activeDate.getSeconds();
		if (typeof(this._rsCells[this._rsActive]) != "undefined") this._rsCells[this._rsActive].className = "dhtmlxcalendar_selector_cell dhtmlxcalendar_selector_cell_active";
	}
	
	// update seconds in calendar
	this._updateVisibleSeconds = function() {
		this.contTime.firstChild.firstChild.childNodes[5].value = this._fixLength(this._activeDate.getSeconds(),2);
	}
	...
	*/	
		...
		var mf = (format||this._dateFormat).replace("%Y", "YYYY").replace("%m", "MM").replace("%d", "DD").replace("%H", "HH").replace("%i", "mm").replace("%s", "ss");
		var mday = moment(val, mf);
		if (getSet) {
			var i = {Y:mday.year(), m:mday.month() + 1, d:mday.date(), H:mday.hours(), i:mday.minutes(), s:mday.seconds()};
			return i;
		}
		else {
			return mday.toDate();
	...
	//取当前活动日期时间格式化后的值
	this.getActiveFormatedDate = function(format) {
		if(!$(this.contDates).is(":visible")) {
			if($(this.contMonth).is(":visible")) { //仅选择年月
				if($.trim($(this.contMonth).find("input.dhtmlxcalendar_month_label_year").val()).length == 0 || $.trim($(this.contMonth).find("input.dhtmlxcalendar_month_label_month").val()).length == 0 ) {
					return "";
				}
			}
			else { //仅选择时间
				if($.trim($(this.contTime).find("input.dhtmlxcalendar_label_hours").val()).length == 0 || $.trim($(this.contTime).find("input.dhtmlxcalendar_label_minutes").val()).length == 0
					 || ($(this.contTime).find("input.dhtmlxcalendar_label_seconds").is(":visible") && $.trim($(this.contTime).find("input.dhtmlxcalendar_label_seconds").val()).length == 0) ) {
					return "";
				}
			}
		...
		var date = new Date(this._activeMonth.getFullYear(), this._activeMonth.getMonth(), this._activeDate.getDate(), this._activeDate.getHours(), this._activeDate.getMinutes(), this._activeDate.getSeconds());
		return this._dateToStr(date, format);
	}
	...
	this.unload = function() {
		...
		this._isMsStepFive; //分和秒选择器是否按5增长。默认true
		this._isSelectorVisible = null;
		this._isShowSecond; //是否显示秒。默认false
		...
		this._updateActiveSeconds = null;
		...
		this._updateVisibleSeconds = null;
		this._updateYearsList = null;
		this.getActiveFormatedDate = null;
		this.hide = null;
		this.hideDate = null;
		this.hideMonth = null;
		this.hideTime = null;
...
dhtmlXCalendarObject.prototype.langData = {
	...
	"zh": {
		...
		monthesSNames: ["一月","二月","三月","四月","五月","六月","七月","八月","九月","十月","十一","十二"],
