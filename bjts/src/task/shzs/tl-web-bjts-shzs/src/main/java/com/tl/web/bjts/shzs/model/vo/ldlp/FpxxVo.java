...
public class FpxxVo implements Serializable{
    ...
    public String getFpzt() {
        ...
        if("0".equals(fpzt)){
            return ConstUtil.FPZT_NORMAL;
        }
        if("1".equals(fpzt)){
            return ConstUtil.FPZT_OUT_OF_CONTROL;
        }
        if("2".equals(fpzt)){
            return ConstUtil.FPZT_INVALID;
        }
        if("3".equals(fpzt) ||"4".equals(fpzt)||"5".equals(fpzt)){
            return ConstUtil.FPZT_ABNORMAL;
        }
        if("6".equals(fpzt)){
            return ConstUtil.FPZT_DEFICIT_REDUCE;
        }
        return "";
    }
