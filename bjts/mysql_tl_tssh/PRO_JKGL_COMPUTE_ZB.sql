...
routine_body: BEGIN
  ...
  SET V_NDBGQQ = STR_TO_DATE(V_ZBYEAR||'-01-01', '%Y-%m-%d');
  SET V_NDBGQZ = STR_TO_DATE(V_ZBYEAR||'-12-31', '%Y-%m-%d');
  ...
  LOOP
    ...
    BEGIN
      SELECT T.BGQID
       ...
       WHERE T.DJXH=CUR_BGQ.DJXH AND T.BGQ_Q=DATE_ADD(V_NDBGQQ, INTERVAL -12 MONTH) AND T.BGQ_Z=V_NDBGQQ-1
         AND T.SXZT='2';
    EXCEPTION
      WHEN OTHERS THEN
        SET V_NDJQID =0;
    ...
    BEGIN
      SELECT T.BGQID
        INTO V_JDJQID
        FROM JKGL_DATA_BGQ T
       WHERE T.DJXH=CUR_BGQ.DJXH AND T.BGQ_Q>=DATE_ADD(V_JDBGQQ, INTERVAL -3 MONTH) AND T.BGQ_Z=V_JDBGQQ-1
         AND T.SXZT='3';
    EXCEPTION
      WHEN OTHERS THEN
        SET V_JDJQID =0;
    ...
    BEGIN
      SELECT T.BGQID
       ...
       WHERE T.QYLX=(CASE CUR_BGQ.JSMODE WHEN '1' THEN '1' ELSE '2' END)
