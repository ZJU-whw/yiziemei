  ...
  LOOP
    ...
    BEGIN
      SELECT T.BGQID
        INTO V_NDJQID
        FROM JKGL_DATA_BGQ T
       WHERE T.DJXH=CUR_BGQ.DJXH AND T.BGQ_Q=DATE_ADD(CUR_BGQ.BGQ_Q, INTERVAL -12 MONTH) AND T.BGQ_Z=CUR_BGQ.BGQ_Q-1
         AND T.SXZT='2';
    EXCEPTION
      WHEN OTHERS THEN
        SET V_NDJQID =0;
    ...
    SELECT COUNT(1)
     ...
     WHERE T.BGQID=CUR_BGQ.BGQID AND ROWNUM=1;
    IF V_ROWS=0 THEN
      INSERT INTO JKGL_DATA_TJ_ZBU_KZ(BGQID) VALUES (CUR_BGQ.BGQID);
      COMMIT;
    END IF;
    ...
    MERGE INTO JKGL_DATA_TJ_ZBU_QYXX A
         USING (SELECT CUR_BGQ.BGQID AS BGQID, T.FDDBRMC  AS FR_XM, T.FRZJHM AS FR_ZJHM, 
                       ...
                       CASE WHEN IFNULL(T.SCCKRQ, DATE'2100-12-31')>V_BGQ_Z THEN NULL 
    ...
    COMMIT;
    DELETE FROM JKGL_DATA_TJ_ZBU_TSGH T
     WHERE T.BGQID=CUR_BGQ.BGQID
       AND NOT EXISTS (SELECT 1 
                         FROM CKTS_SB_MTS_JHMX S 
