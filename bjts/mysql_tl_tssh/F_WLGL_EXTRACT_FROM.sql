...
BEGIN
  ...
  SET v_result = REGEXP_SUBSTR(p_text, v_pattern, 1, 1, 'i', 3);
  IF v_result IS NULL OR LENGTH(v_result)<2 THEN
    SET v_pattern = '(货源地|产地)[:：;；[:space:]]*\s*([^A-Z0-9,，;；：.．。、/[:space:]]+)';
    SET v_result = REGEXP_SUBSTR(p_text, v_pattern, 1, 1, 'i', 2);
  END IF;
  ...
  IF v_result IS NULL OR LENGTH(v_result)<2 THEN
    ...
    SET v_result = REGEXP_SUBSTR(p_text, v_pattern, 1, 1, 'i', 1);
  END IF;
