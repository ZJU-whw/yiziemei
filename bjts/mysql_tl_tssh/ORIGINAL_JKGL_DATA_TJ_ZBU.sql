...
routine_body: BEGIN
  ...
  LOOP
    ...
    BEGIN
      ...
      IF CUR_BGQ.BGQ_Q = DATE_ADD(CAST(DATE_FORMAT(CUR_BGQ.BGQ_Z, '%Y-%m-01') AS DATETIME), INTERVAL -11 MONTH) THEN
      ...
      IF EXTRACT(MONTH FROM CUR_BGQ.BGQ_Q) IN (1,4,7,10) AND EXTRACT(MONTH FROM CUR_BGQ.BGQ_Z) IN (3,6,9,12) THEN
        ...
        SELECT IFNULL(SUM(ZC_YSZK), 0), IFNULL(SUM(ZC_YUFZK), 0), IFNULL(SUM(ZC_CH), 0), IFNULL(SUM(ZC_LDZC), 0), 
          ...
          FROM (SELECT (CASE B.EWBHXH WHEN 4 THEN B.QMYE_ZC ELSE 0 END) AS ZC_YSZK, 
                 ...
                 WHERE A.DJXH=CUR_BGQ.DJXH 
                   AND A.SSQQ>=DATE_ADD(CUR_BGQ.BGQ_Q, INTERVAL -3 MONTH) AND A.SSQZ=CUR_BGQ.BGQ_Q-1
        ...
        SELECT IFNULL(SUM(ZC_YSZK), 0), IFNULL(SUM(ZC_YUFZK), 0), IFNULL(SUM(ZC_CH), 0), IFNULL(SUM(ZC_LDZC), 0), 
          ...
          FROM (SELECT (CASE B.EWBHXH WHEN 4 THEN B.QMYE_ZC ELSE 0 END) AS ZC_YSZK, 
                 ...
                 INNER JOIN HX_SB.SB_CWBB_QYKJZZYBQY_ZCFZB B ON B.ZLBSCJUUID=A.ZLBSCJUUID
                 WHERE A.DJXH=CUR_BGQ.DJXH 
                   AND A.SSQQ>=CAST(DATE_FORMAT(DATE_ADD(CUR_BGQ.BGQ_Z, INTERVAL -2 MONTH), '%Y-%m-01') AS DATETIME) AND A.SSQZ=CUR_BGQ.BGQ_Z
                   AND A.ZFBZ_1 IS NULL
                 UNION ALL
                SELECT (CASE B.EWBHXH WHEN 6 THEN B.QMS_ZC ELSE 0 END) AS ZC_YSZK, 
                       (CASE B.EWBHXH WHEN 8 THEN B.QMS_ZC ELSE 0 END) AS ZC_YUFZK, 
                 ...
                 INNER JOIN HX_SB.SB_CWBB_QYKJZZ_ZCFZB B ON B.ZLBSCJUUID=A.ZLBSCJUUID
                 WHERE A.DJXH=CUR_BGQ.DJXH 
                   AND A.SSQQ>=CAST(DATE_FORMAT(DATE_ADD(CUR_BGQ.BGQ_Z, INTERVAL -3 MONTH), '%Y-%m-01') AS DATETIME) AND A.SSQZ=CUR_BGQ.BGQ_Z
                   AND A.ZFBZ_1 IS NULL
                 UNION ALL
                SELECT (CASE B.EWBHXH WHEN 4 THEN B.QMYE_ZC ELSE 0 END) AS ZC_YSZK, 
                       (CASE B.EWBHXH WHEN 5 THEN B.QMYE_ZC ELSE 0 END) AS ZC_YUFZK, 
        ...
        SELECT IFNULL(SUM(LR_YYSR), 0), IFNULL(SUM(LR_YYCB), 0), IFNULL(SUM(LR_SJJFJ), 0), IFNULL(SUM(LR_XSFY), 0), 
          ...
          FROM (-- +期末数
                SELECT (CASE B.EWBHXH WHEN 1 THEN B.BNLJS ELSE 0 END) AS LR_YYSR, 
                 ...
                 WHERE A.DJXH=CUR_BGQ.DJXH 
                   AND A.SSQQ>=CAST(DATE_FORMAT(DATE_ADD(CUR_BGQ.BGQ_Z, INTERVAL -2 MONTH), '%Y-%m-01') AS DATETIME) 
                ...
                SELECT (CASE B.EWBHXH WHEN 1 THEN B.BNLJJE ELSE 0 END) AS LR_YYSR, 
                 ...
                 WHERE A.DJXH=CUR_BGQ.DJXH 
                   AND A.SSQQ>=CAST(DATE_FORMAT(DATE_ADD(CUR_BGQ.BGQ_Z, INTERVAL -3 MONTH), '%Y-%m-01') AS DATETIME) 
                   AND A.SSQZ=CUR_BGQ.BGQ_Z
                   AND A.ZFBZ_1 IS NULL
                 UNION ALL
    ...
    MERGE INTO JKGL_DATA_TJ_ZBU_CKGB A
         USING (SELECT CUR_BGQ.BGQID AS BGQID, T.MYGDQSZ_DM AS GB_DM, SUM(T.MYLAJ) AS CKEUSD, SUM(T.RMBLAJ) AS CKERMB,
          ...
          WHEN NOT MATCHED THEN
            ...
            VALUES (SEQ_NEXTVAL('SEQ_JKGL_DATA_TJ_ZBU_CKGB'),B.BGQID,B.GB_DM,B.CKEUSD,B.CKERMB,B.KASL,B.SBDWSL,B.YSFSSL);
    COMMIT;
    ...
    MERGE INTO JKGL_DATA_TJ_ZBU_CKKA A
         USING (SELECT CUR_BGQ.BGQID AS BGQID, T.HGGQKA_DM AS KA_DM, SUM(T.MYLAJ) AS CKEUSD, SUM(T.RMBLAJ) AS CKERMB,
          ...
          WHEN NOT MATCHED THEN
            ...
            VALUES (SEQ_NEXTVAL('SEQ_JKGL_DATA_TJ_ZBU_CKKA'),B.BGQID,B.KA_DM,B.CKEUSD,B.CKERMB,B.SBDWSL);
    COMMIT;
    ...
    MERGE INTO JKGL_DATA_TJ_ZBU_CKSP A
         USING (SELECT CUR_BGQ.BGQID AS BGQID, SUBSTR(T.CKSP_DM,1,8) AS SP8_DM, SUM(T.MYLAJ) AS CKEUSD, SUM(T.RMBLAJ) AS CKERMB,
          ...
          WHEN NOT MATCHED THEN
            ...
            VALUES (SEQ_NEXTVAL('SEQ_JKGL_DATA_TJ_ZBU_CKSP'),B.BGQID,B.SP8_DM,B.CKEUSD,B.CKERMB,B.CKSL,B.CKDJ);
    COMMIT;
    UPDATE JKGL_DATA_TJ_ZBU_CKSP A
       SET A.SPDL=SUBSTR(A.SP8_DM,1,2),
           A.SPMC=(SELECT B.CKSP_MC FROM JCFX_DM_CKSP B WHERE B.CKSP_DM=A.SP8_DM AND ROWNUM=1)
    ...
    MERGE INTO JKGL_DATA_TJ_ZBU_QYXX A
         USING (SELECT CUR_BGQ.BGQID AS BGQID, T.FDDBRMC  AS FR_XM, T.FRZJHM AS FR_ZJHM, 
                 ...
                 WHERE T.CPCODE=CAST(CUR_BGQ.DJXH AS CHAR)) B
            ON (A.BGQID=B.BGQID)
          WHEN MATCHED THEN
    ...
    UPDATE JKGL_DATA_TJ_ZBU_QYXX
       SET FLGLCD =
           (SELECT S.KZXX 
             ...
             WHERE T.CPCODE=CAST(CUR_BGQ.DJXH AS CHAR) 
    ...
    MERGE INTO JKGL_DATA_TJ_ZBU_TSGH A
         USING (SELECT CUR_BGQ.BGQID AS BGQID, S.GHFNSRSBH_1 AS GHFSH, SUM(S.JSJE) AS JHJE, SUM(S.TSE) AS TSE
          ...
          WHEN NOT MATCHED THEN
            ...
            VALUES (SEQ_NEXTVAL('SEQ_JKGL_DATA_TJ_ZBU_TSGH'),B.BGQID,B.GHFSH,B.JHJE,B.TSE);
    COMMIT;
    UPDATE JKGL_DATA_TJ_ZBU_TSGH T
       SET T.FXQYBZ='1'
     WHERE T.BGQID=CUR_BGQ.BGQID
       AND EXISTS (SELECT 1
                     FROM JKGL_GY_FXQY_SWJG S
                    WHERE S.QYSBH=T.GHFSH
                      AND IFNULL(S.YXQ_Z, CURRENT_TIMESTAMP)>CUR_BGQ.BGQ_Q);
    COMMIT;
    UPDATE JKGL_DATA_TJ_ZBU_TSGH T
       SET T.FXDQBZ='1'
     WHERE T.BGQID=CUR_BGQ.BGQID
    ...
    MERGE INTO JKGL_DATA_TJ_ZBU_TSSP A
         USING (SELECT CUR_BGQ.BGQID AS BGQID, SUBSTR(T.CKSP_DM,1,8) AS SP8_DM, SUM(T.MYLAJ) AS CKEUSD, SUM(T.RMBLAJ) AS CKERMB,
          ...
          WHEN NOT MATCHED THEN
            ...
            VALUES (SEQ_NEXTVAL('SEQ_JKGL_DATA_TJ_ZBU_TSSP'),B.BGQID,B.SP8_DM,B.CKEUSD,B.CKERMB,B.CKERMB_STZC,
                    B.CKSL,B.SBTMSE,B.JHJE,SUBSTR(B.SP8_DM,1,2),B.MMYLR,B.CKDJ,B.JHDJ);
    COMMIT;
    UPDATE JKGL_DATA_TJ_ZBU_TSSP T
       SET T.MGBZ='1'
     WHERE T.BGQID=CUR_BGQ.BGQID
       AND EXISTS (SELECT 1
    ...
    MERGE INTO JKGL_DATA_TJ_ZBU_DZGF A
         USING (SELECT CUR_BGQ.BGQID AS BGQID, GFSBH,GFMC,FS,JE,SE,ZYSPMC,ZYSPJE
          ...
          WHEN NOT MATCHED THEN
            ...
            VALUES (SEQ_NEXTVAL('SEQ_JKGL_DATA_TJ_ZBU_DZGF'),B.BGQID,B.GFSBH,B.GFMC,B.FS,B.JE,B.SE,B.ZYSPMC,B.ZYSPJE);
    COMMIT;
    ...
    MERGE INTO JKGL_DATA_TJ_ZBU_DZXF A
         USING (SELECT CUR_BGQ.BGQID AS BGQID, XFSBH,XFMC,FS,JE,SE,ZYSPMC,ZYSPJE
          ...
          WHEN NOT MATCHED THEN
            ...
            VALUES (SEQ_NEXTVAL('SEQ_JKGL_DATA_TJ_ZBU_DZXF'),B.BGQID,B.XFSBH,B.XFMC,B.FS,B.JE,B.SE,B.ZYSPMC,B.ZYSPJE);
    COMMIT;
    ...
    MERGE INTO JKGL_DATA_TJ_ZBU_DZSPJX A
         USING (SELECT CUR_BGQ.BGQID AS BGQID, SPSM, SPMC, JE, SE
          ...
          WHEN NOT MATCHED THEN
            ...
            VALUES (SEQ_NEXTVAL('SEQ_JKGL_DATA_TJ_ZBU_DZSPJX'),B.BGQID,B.SPMC,B.SPSM,B.JE,B.SE);
    COMMIT;
    ...
    MERGE INTO JKGL_DATA_TJ_ZBU_DZSPXX A
         USING (SELECT CUR_BGQ.BGQID AS BGQID, SPSM, SPMC, JE, SE
          ...
          WHEN NOT MATCHED THEN
            INSERT (ID,BGQID,SPMC,SPSM,JE,SE)
            VALUES (SEQ_NEXTVAL('SEQ_JKGL_DATA_TJ_ZBU_DZSPXX'),B.BGQID,B.SPMC,B.SPSM,B.JE,B.SE);
     COMMIT;
