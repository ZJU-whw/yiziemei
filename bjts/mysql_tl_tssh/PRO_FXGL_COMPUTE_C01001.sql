...
routine_body: BEGIN
  ...
  FOR CUR_CKQY IN (SELECT T.DJXH,TRUNCATE(MIN(T.QDSJ), 0) AS SCSBRQ
                     FROM CKTS_LC_SBXX T
    ...
    IF V_MYLAJ>=V_MYLAJ_LX3M THEN
      INSERT INTO FXGL_DATA_ZXZB(ID,TSSWJG_DM,DJXH,NSRSBH,NSRMC,SMLX,SMRQ,ZBID,ZBCS,SMJG)
           SELECT SEQ_NEXTVAL('SEQ_FXGL_DATA_ZXZB'),TT.SWJGDM,TT.DJXH_JS,IFNULL(TT.SHXYNO, TT.NSRDJNO),TT.NSRMC,IFNULL(P_SMLX, '0'),CURRENT_TIMESTAMP,'C01001',
                  V_PARAMS,'[d]出口企业首次申报日期=' || DATE_FORMAT(CUR_CKQY.SCSBRQ, '%Y%m%d')||'|最近三个月美元出口额=' ||V_MYLAJ
  ...
  LOOP
    ...
    IF V_MYLAJ>=V_MYLAJ_LX3M THEN
      INSERT INTO FXGL_DATA_ZXZB(ID,TSSWJG_DM,DJXH,NSRSBH,NSRMC,SMLX,SMRQ,ZBID,ZBCS,SMJG)
           SELECT SEQ_NEXTVAL('SEQ_FXGL_DATA_ZXZB'),TT.SWJGDM,TT.DJXH_JS,IFNULL(TT.SHXYNO, TT.NSRDJNO),TT.NSRMC,IFNULL(P_SMLX, '0'),CURRENT_TIMESTAMP,'C01001',
                  V_PARAMS,'[d]中断后复出口申报日期=' || DATE_FORMAT(CUR_CKQY.SCSBRQ, '%Y%m%d')||'|最近三个月美元出口额=' ||V_MYLAJ
             FROM GLXT_BB_SHXT_DJXX TT
            WHERE TT.DJXH_JS=CUR_CKQY.DJXH;
      COMMIT;
    END IF;
  ...
  LOOP
    ...
    IF V_MYLAJ>=V_MYLAJ_LX3M THEN
      INSERT INTO FXGL_DATA_ZXZB(ID,TSSWJG_DM,DJXH,NSRSBH,NSRMC,SMLX,SMRQ,ZBID,ZBCS,SMJG)
           SELECT SEQ_NEXTVAL('SEQ_FXGL_DATA_ZXZB'),TT.SWJGDM,TT.DJXH_JS,IFNULL(TT.SHXYNO, TT.NSRDJNO),TT.NSRMC,IFNULL(P_SMLX, '0'),CURRENT_TIMESTAMP,'C01001',
                  V_PARAMS,'[d]外贸迁移后首次申报日期=' || DATE_FORMAT(CUR_CKQY.SCSBRQ, '%Y%m%d')||'|最近三个月美元出口额=' ||V_MYLAJ
             FROM GLXT_BB_SHXT_DJXX TT
            WHERE TT.DJXH_JS=CUR_CKQY.DJXH;
      COMMIT;
    END IF;
