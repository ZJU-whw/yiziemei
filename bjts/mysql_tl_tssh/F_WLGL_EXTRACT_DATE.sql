...
BEGIN
  ...
  SET v_pattern = '((Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Mon|Nov|Dec)[-/\.,]\d{1,2}[-/\.,]\d{4}|'
...
;
  ...
  ELSEIF REGEXP_LIKE(v_date_str, '^\d{2}[-/\.]\d{1,2}[-/\.]\d{1,2}$') THEN
    SELECT STR_TO_DATE(REGEXP_REPLACE('20'||v_date_str, '[-/\.]', '-'), '%Y-%m-%d')
      INTO v_result
;
  -- 情况2.3: 2023年1月15日
  ELSEIF REGEXP_LIKE(v_date_str, '^\d{4}年\d{1,2}月\d{1,2}日$') THEN
    SELECT STR_TO_DATE(v_date_str, '%Y"年"%m"月"%d"日"')
      INTO v_result
;
  -- 情况2.4: 23年1月15日
  ELSEIF REGEXP_LIKE(v_date_str, '^\d{2}年\d{1,2}月\d{1,2}日$') THEN
    SELECT STR_TO_DATE('20'||v_date_str, '%Y"年"%m"月"%d"日"')
      INTO v_result
;
  -- 情况2.5: 1月15日
  ELSEIF REGEXP_LIKE(v_date_str, '^\d{1,2}月\d{1,2}日$') THEN
    SELECT STR_TO_DATE(DATE_FORMAT(p_date, '%Y')||'年'||v_date_str, '%Y"年"%m"月"%d"日"')
      INTO v_result
;
  -- 情况3: 纯8位数字 20230115
...
;
  ...
  ELSEIF REGEXP_LIKE(v_date_str, '^\d{6}$') THEN
    SELECT STR_TO_DATE('20'||v_date_str, '%Y%m%d')
      INTO v_result
;
  -- 情况4.2: 纯4位数字 0115
  ELSEIF REGEXP_LIKE(v_date_str, '^\d{4}$') THEN
    SELECT STR_TO_DATE(DATE_FORMAT(p_date, '%Y')||v_date_str, '%Y%m%d')
      INTO v_result
;
  -- 情况5: 01-15 / 01/15 / 01.15
  ELSEIF REGEXP_LIKE(v_date_str, '^\d{1,2}[-/\.]\d{1,2}$') THEN
    SELECT STR_TO_DATE(DATE_FORMAT(p_date, '%Y')||'-'||REGEXP_REPLACE(v_date_str, '[-/\.]', '-'), '%Y-%m-%d')
      INTO v_result
;
  END IF;
