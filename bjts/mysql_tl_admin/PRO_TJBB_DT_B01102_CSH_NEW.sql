...
DROP PROCEDURE IF EXISTS PRO_TJBB_DT_B01102_CSH_NEW$$

CREATE PROCEDURE PRO_TJBB_DT_B01102_CSH_NEW
/*************************************************
 * 不予办理退（免）税统计月报表
...
SELECT COUNT(DISTINCT DJXH), COUNT(DISTINCT(CASE WHEN T.SZ='V' THEN DJXH ELSE NULL END)),
           ROUND(IFNULL(SUM(CASE WHEN T.SZ='V' AND T.CKRQ_1< LD_THISYEAR THEN T.TSE ELSE 0 END), 0)/10000,6), -- 不予
           ROUND(IFNULL(SUM(CASE WHEN T.SZ='V' AND T.CKRQ_1>=LD_THISYEAR THEN T.TSE ELSE 0 END), 0)/10000,6), -- 不予
           ROUND(IFNULL(SUM(CASE WHEN T.SZ='C' AND T.CKRQ_1< LD_THISYEAR THEN T.TSE ELSE 0 END), 0)/10000,6), -- 不予
           ROUND(IFNULL(SUM(CASE WHEN T.SZ='C' AND T.CKRQ_1>=LD_THISYEAR THEN T.TSE ELSE 0 END), 0)/10000,6) -- 不予
...
;
    ...
    SELECT COUNT(DISTINCT DJXH),
           ROUND(IFNULL(SUM(CASE WHEN T.CKRQ_1< LD_THISYEAR THEN T.TSE ELSE 0 END), 0)/10000,6), -- 不予
           ROUND(IFNULL(SUM(CASE WHEN T.CKRQ_1>=LD_THISYEAR THEN T.TSE ELSE 0 END), 0)/10000,6) -- 不予
...
SELECT COUNT(DISTINCT DJXH), COUNT(DISTINCT(CASE WHEN T.TSE>0 THEN DJXH ELSE NULL END)),
           ROUND(IFNULL(SUM(CASE WHEN T.CKRQ_1< LD_THISYEAR THEN T.TSE ELSE 0 END), 0)/10000,6), -- 不予
           ROUND(IFNULL(SUM(CASE WHEN T.CKRQ_1>=LD_THISYEAR THEN T.TSE ELSE 0 END), 0)/10000,6), -- 不予
           ROUND(IFNULL(SUM(CASE WHEN T.CKRQ_1< LD_THISYEAR THEN T.MDE ELSE 0 END), 0)/10000,6), -- 不予
           ROUND(IFNULL(SUM(CASE WHEN T.CKRQ_1>=LD_THISYEAR THEN T.MDE ELSE 0 END), 0)/10000,6) -- 不予
      INTO LN_BAHS, LN_SBHS, LN_BY_ZZS_HIS, LN_BY_ZZS_CUR, LN_BY_XFS_HIS, LN_BY_XFS_CUR
...
SELECT TRIM(LPAD(CAST(SUBSTR(T.FORMULA,2,INSTR(T.FORMULA,')')-2) AS CHAR), 2, '0')) AS BBLC,
  ...
  BJTS_CURSOR_LOOP_002: LOOP
    ...
    END IF;
      BEGIN
        SET LC_SQL = ORA_CONCAT(ORA_CONCAT(ORA_CONCAT(ORA_CONCAT(ORA_CONCAT(ORA_CONCAT(ORA_CONCAT(ORA_CONCAT(ORA_CONCAT(ORA_CONCAT(ORA_CONCAT('INSERT INTO TJBB_DT_B01102 (SSNY, BBLC, SWJGDM,
                          HJ_QYHS, HJ_TSE,
                          HH_QYHS, HH_TSE_XJ, HH_TSE_HIS, HH_TSE_CUR,
                          BA_QYHS, BA_TSE_XJ, BA_TSE_HIS, BA_TSE_CUR,
                          PZ_QYHS, PZ_TSE_XJ, PZ_TSE_HIS, PZ_TSE_CUR,
...
END;
  ...
  END;
SET LC_SQL_UPDATEHZ = 'UPDATE TJBB_DT_B01102 SET ';
...
SELECT FNAME FROM TJBB_HEADER_COLS WHERE BBDM='B01102' AND FTYPE='NUMBER' ORDER BY SHOWORDER;
