# 系统去 Oracle 化迁移改造方案（初稿）

> **项目**：出口退税 / 税务风险管理系统（政务内网）
...
按政务部门整体去 Oracle 化要求，本系统数据库由 Oracle 11g 替换为 MySQL 5.7。系统运行于政务内网（物理隔离），数据库层使用多个 schema，并大量使用存储过程、函数、视图等数据库对象承载业务逻辑；应用层为 Java 服务，除自身数据源外还通过 ETL 从第三方平台抽取数据。
...
| 回退时机 | 条件 | 动作 | 预计耗时 |
|---|---|---|---|
| 割接窗口内 | 数据校验不通过 / 冒烟失败 | 应用配置切回 Oracle，Oracle 解除只读 | 30 分钟 |
| T+7 内 | 发现严重功能或性能问题 | 应用切回 Oracle；**MySQL 期间产生的增量数据需人工回补** | 数小时至 1 天 |
| T+7 后 | — | 不再回退，转为问题修复 | — |
...
| 阶段 | 主要工作 | 周期 | 里程碑 |
|---|---|---|---|
| P0 准备 | 立项、组队、环境准备、工具选型 POC、**源码保真度整改（2.5）** | 3 周 | 方案评审通过 |
| P1 摸底评估 | 对象清单、数据画像、代码扫描、工作量复算、与第三方约定时点 | 3 周 | 评估报告 + 详细计划 |
| P2 结构与规范 | 类型映射落地、DDL 生成与复核、改造规范与模板定稿 | 4 周 | 目标库结构就绪 |
| P3 对象改造 | 存储过程/函数/视图改造 + 单测（**最大工作量**） | 14 周 | 对象改造完成 |
| P4 应用改造 | 持久层、数据源、类型映射、异常、分页、存储过程调用 | 12 周（与 P3 并行） | 应用可在 MySQL 运行 |
| P5 ETL 改造 | 抽数改造、双模适配、调度迁移 | 8 周（与 P3 并行） | ETL 全流程跑通 |
| P6 数据迁移 | 全量迁移脚本、校验脚本、多轮演练 | 6 周（与 P4 并行） | 迁移可重复执行且校验通过 |
| P7 测试 | 功能回归、双跑比对、性能测试、专项语义测试 | 8 周 | 达到验收标准 |
| P8 割接 | 演练 + 正式割接 | 2 周 | 投产 |
| P9 稳定期 | 监控优化、C2 集合化改造、`ONLY_FULL_GROUP_BY` 整改、Oracle 下线 | 8 周 | 项目关闭 |

**总周期约 11–12 个月**（P3/P4/P5/P6 大幅并行）。关键路径：**P1 → P3 → P7 → P8**。
...
| **R6** | 表名冲突（合并单库方案） | 中 | 需重命名并连带改应用 | P1 阶段完成冲突检查（A-9），有冲突则重新评估方案 A |
...
| **R12** | 回退时增量数据回补困难 | 中 | 回退代价高于预期 | 割接前与业务方书面确认回退代价；缩短观察期决策链路 |
...
2. schema 合并方案（D-02 方案 B）是否确认？**表名冲突检查（A-9）须在决策前完成**。
